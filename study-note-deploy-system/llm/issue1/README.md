...
**검색어 다듬기**다. 문제는 Ollama(모델을 GPU에 올려 HTTP로 서빙해주는 프로그램)를
그대로 backend에 노출하면:
...
- 이 서버의 핵심 임무는 "모델 출력이 약속한 JSON 모양인지 검증"인데,
  ...
  Ollama에게 보낼 JSON 스키마가 된다 — 한 벌로 세 가지를 해결.
...
- **배경**: 문서와 코드가 다른 이름을 말하면 backend는 어느 쪽을 믿어야 할지 모른다.
  "설계 문서가 계약의 정본"을 지키는 게 핵심. (이 결함은 나(사람)가 아니라
  두 개의 독립 리뷰어가 동시에 잡았다 — 듀얼 리뷰의 값어치.)

### 문제 2 — 모델이 없어도 "건강함"으로 보였다
...
Ollama 앞에 "프롬프트를 소유하는 문지기"를 세웠다. 동시 2건 초과는 7.7ms 만에
