...
### 한글 경로 문서를 조회하니 본문이 빈 문자열

**증상**: 스모크에서 한글 경로 문서를 조회:
...
③이 결정타 — 우리가 rename(problem.md→1-question.md)하기 **이전 커밋에서도 0바이트**.
즉 시스템 어디도 내용을 잃지 않았고, **programmers 94개는 원래부터 안 쓴 플레이스홀더**였다.

**수정**: 없음(결함 아님). 색인 파이프라인은 빈 파일을 "청크 0개 → 건너뜀"으로 이미
정상 처리하고 있었다(①에서 확인).

**배경**: "버그처럼 보이는 것"의 절반은 데이터 실태다. 코드를 고치기 전에
**서버 사본 → 로컬 원본 → git 이력** 순서로 거슬러 확인하면, 멀쩡한 코드를 고치는
헛수고를 막을 수 있다. 특히 ③처럼 "우리 작업 이전에도 그랬나"를 이력으로 확정하는
단계가 중요하다 — 우리 rename이 내용을 날렸을 가능성을 배제해준다.
