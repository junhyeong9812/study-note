...
### 기각 1 — "1/2/3 탭이 항상 3개여야 한다"

issue2의 결정 그대로: 코테는 2-summary가 **의도된 부재**다. 3슬롯 고정 + "작성 전"
패널은 의도된 부재와 깜빡한 부재를 화면에서 구분 불가하게 만든다. `docs[0]` 역참조
위험 지적도 `is_subject = docs.isNotEmpty && children.isEmpty` 정의상 성립 불가. → 기각.
