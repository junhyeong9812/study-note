...
*(원문에는 별도의 「영향과 의의」 절이 없다. 이 편의 결론은 바로 위 「진화의 논리」의 마지막 문장이고, 이 절은 원문이 "지금 무엇을 고르나"로 잇는 대목이다. corpus 문서는 링크가 아니라 이름으로 적고, 원본 repo 기준 경로를 함께 적었다.)*

이 문서가 "역사(왜 이렇게 됐나)"라면, corpus/frontend는 "설계(지금 무엇을 고르나)"다.\
진화의 각 분기점은 corpus의 트레이드오프 문서로 이어진다.

- 단방향 흐름·서버상태 분리·전역 스토어 선택 → corpus 「상태 관리」 (원본 repo 기준 경로: `../local-llm/corpus/frontend/상태-관리.md`)
- SSR/SSG/ISR/스트리밍·RSC·아일랜드의 선택 기준 → corpus 「렌더링 전략」 (원본 repo 기준 경로: `../local-llm/corpus/frontend/렌더링-전략.md`)
- 컴포넌트 합성·컨테이너/프레젠테이셔널 → corpus 「컴포넌트 아키텍처」 (원본 repo 기준 경로: `../local-llm/corpus/frontend/컴포넌트-아키텍처.md`)
- 서버 캐시·무효화·낙관적 업데이트(React Query/SWR) → corpus 「데이터 페칭·캐싱」 (원본 repo 기준 경로: `../local-llm/corpus/frontend/데이터-페칭-캐싱.md`)
- hydration 세금·번들·코드스플리팅·CWV → corpus 「성능·최적화」 (원본 repo 기준 경로: `../local-llm/corpus/frontend/성능-최적화.md`)
- 전체 인덱스 → corpus/frontend INDEX (원본 repo 기준 경로: `../local-llm/corpus/frontend/INDEX.md`)
