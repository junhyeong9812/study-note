...
> ASCII 도식 4개, 「한눈에」의 길·포장 비유(대응표 포함), 3.10 절의 `handle` 함수의 `case` 넷을 옮긴 표 1개, 용어 블록의 「예:」, 「용어 풀이」, 「재서술자 주」 3개는 원문에 없는 보충이다.
...
> **재서술자 주:** 원문 산문은 f-string이 대체한 것을 "`%` 포매팅, `str.format()`, `string.Template`"으로 적는데, 위 코드의 세 줄은 `%`, `.format()`, `.format(n=…)`이다.\
> 셋째 줄은 `string.Template`이 아니라 `str.format()`에 이름을 붙여 쓴 방식이고, `string.Template`은 코드에 나오지 않는다 — 산문의 셋과 코드의 셋은 수만 같고 구성원이 다른 것으로 보인다. 원문은 고치지 않고 이 자리에 표기만 해 둔다.
...
이 문법이 없었다면 다음 해의 `dataclasses`(3.7)는 불가능했다 — 데이터클래스는 바로 이 클래스 변수 어노테이션을 읽어 필드를 만든다.

- 다음 절(3.7)의 `dataclass`가 읽는 것이 위 코드의 `x: int` / `y: int` 같은 모양이다.

> **재서술자 주:** 원문은 같은 `x: int` 줄을 코드 주석에서는 "인스턴스 속성 어노테이션"이라 부르고, 바로 아래 문단에서는 "클래스 변수 어노테이션"이라 부른다.\
> 둘 다 `class` 블록 안에 적은 같은 자리를 가리키는 것으로 보인다 — 어느 한쪽이 틀렸다기보다 부르는 이름이 둘인 것이다.\
> 더 그럴듯한 읽기는 이렇다 — 같은 코드에서 진짜 클래스 변수는 `origin: ClassVar[bool]` 쪽이므로, 코드 주석의 "인스턴스 속성 어노테이션"이 PEP 526의 용어와 맞고, 본문의 "클래스 변수 어노테이션"은 **"클래스 본문에 적은 변수 어노테이션"**을 느슨하게 부른 표현으로 보인다. 원문은 고치지 않고 이 자리에 표기만 해 둔다.
...
> **재서술자 주:** 원문은 위 불릿 바로 앞에서 "주된 기법은 두 가지다"라 적어 놓고 불릿은 셋을 단다.\
> 셋째 불릿("더 빠른 함수 호출")까지 더해 셋을 든 것인데 앞 문장의 수만 둘로 남은 것으로 보인다. 원문은 고치지 않고 이 자리에 표기만 해 둔다 — 외울 것은 "두 가지"가 아니라 위 불릿 셋이다.
