...
- 미리 계산(전처리)에 O(n log n)을 한 번 내면, 그 뒤 어떤 구간 질문이든 O(1) — 표 두 칸만 본다.
- 단, 값이 바뀌면 표 전체가 틀어지므로 **바뀌지 않는(정적) 데이터** 전용이다.
- 겹쳐도 되는 연산(min, max, gcd)만 이 요령이 통한다. 합(sum)은 겹치면 두 번 더해지므로, 겹치지 않게 자르는 변형(DisjointSparseTable)을 쓴다.

```text
질문: [1..6]의 최솟값?          미리 적어둔 길이-4 묶음 두 개로 덮는다

값     5  2  9  1  7  4  6  3
자리   0  1  2  3  4  5  6  7
          |<-- 묶음A [1..4] -->|
                |<-- 묶음B [3..6] -->|
          가운데 [3..4]가 겹치지만 min은 두 번 세어도 그대로
...
```
  ...
  - *2^k*: 2를 k번 곱한 수(1, 2, 4, 8, ...). 이 표의 묶음 길이는 항상 이런 수다.
...
**그림 해설** — 위 그림에서 일어난 일:
1. 길이 4짜리 묶음 [1..4]의 답을, 이미 있는 길이 2짜리 두 개([1..2], [3..4])를 combine 해서 만든다.
2. 두 조각은 딱 맞닿고 겹치지 않으므로, 전처리 단계에서는 어떤 연산이든(합도) 문제없다.
3. 표의 칸 수(n log n개)만큼만 일하니 전처리 = O(n log n).
...
**그림 해설** — 위 조회에서 일어난 일:
1. 구간 길이(6)에 맞는 가장 큰 2의 거듭제곱(4)을 log 표에서 O(1)에 찾는다.
2. 왼쪽 끝에 붙인 묶음과 오른쪽 끝에 붙인 묶음, 두 칸을 combine 한다.
3. 각 묶음이 구간의 절반을 넘게 덮으므로 가운데서 반드시 만난다 — 빈틈이 없다.
4. 겹친 부분은 min이라 두 번 세어도 답이 안 변한다. 이 성질이 없는 합(sum)은 여기서 틀린다.
...
먼저 알아야 할 것 — **이진법과 XOR**:
- 숫자를 0과 1로 적는 것이 이진법. 1 = 001, 6 = 110.
...
**그림 해설** — 위 조회에서 일어난 일:
1. from(1)과 to(6)의 비트가 처음 갈라지는 자리(2번 자리)를 XOR 한 번으로 찾는다 — 그 자리가 층 번호다.
2. 그 층의 중앙선을 사이에 두고 from은 왼쪽, to는 오른쪽에 있다.
3. 왼쪽 칸은 "from부터 중앙 직전까지"의 누적, 오른쪽 칸은 "중앙부터 to까지"의 누적 — 딱 맞물리고 겹치는 칸이 0개다.
4. 겹침이 없으니 합·곱처럼 두 번 세면 틀리는 연산도 칸 두 개 combine으로 끝난다.
...
- **희소 테이블(sparse table)**: 길이 1, 2, 4, 8… 구간의 답을 미리 표로 적어 두고, 아무 구간이든 표 두 칸으로 답하는 구조.
- **전처리(preprocessing)**: 질문을 받기 전에 미리 계산해 두는 것. 여기서는 표 짓기 O(n log n).
- **구간 질의(range query)**: "배열의 from번부터 to번까지"에 대한 질문(최솟값, 합 등).
- **멱등(idempotent)**: 같은 값을 두 번 넣어도 결과가 안 변하는 성질. min(a,a)=a. 합은 아니다.
- **항등원(identity)**: 연산에 넣어도 영향이 없는 값. 합의 0, min의 무한대, gcd의 0.
- **결합법칙**: (a∘b)∘c = a∘(b∘c). 묶는 순서를 바꿔도 되는 성질 — 세그먼트 트리가 요구하는 최소 조건.
- **비가환(non-commutative)**: a∘b ≠ b∘a인 연산(행렬곱 등). 인자 순서를 지켜야 한다.
- **배증(doubling)**: 아래층 답 두 개를 붙여 2배 긴 위층 답을 만드는 쌓기 방식.
- **2^k / floor(log2 x)**: 2를 k번 곱한 수 / x가 2의 몇 제곱 이상인지의 정수 부분.
- **정적(static) 구조**: 만든 뒤 값을 못 바꾸는 구조. 하나라도 바뀌면 전부 다시 짓는다.
- **이진법 / 비트**: 수를 0과 1로 적는 방식 / 그 한 자리.
- **XOR(^)**: 비트 자리별로 "다르면 1"인 연산. from ^ to는 두 수가 갈라지는 자리를 알려준다.
- **i >> 1 (시프트)**: 비트를 오른쪽으로 한 칸 밀기 = 2로 나눈 몫.
- **numberOfLeadingZeros**: 32비트 정수 맨 앞의 연속된 0 개수. 최상위 1의 자리를 찾는 데 쓴다.
- **gcd(최대공약수) / 유클리드 호제법**: 두 수를 모두 나누는 가장 큰 수 / 그것을 나머지 반복으로 빠르게 구하는 법.
- **패딩(padding)**: 크기를 2의 거듭제곱으로 맞추려고 남는 칸을 항등원으로 채우는 것.
- **세그먼트 트리 / 펜윅 트리**: 구간 질의를 O(log n)에 답하면서 값 변경도 되는 트리 구조들(13번·17번).
- **슬라이딩 윈도우**: 고정 크기 창을 한 칸씩 밀며 각 창의 답을 구하는 문제 형태.
- **덱(deque)**: 양쪽 끝에서 넣고 뺄 수 있는 줄. 슬라이딩 윈도우 최솟값을 O(n)에 푸는 도구(04번).
- **O(1), O(n log n) (빅오 표기)**: 입력이 커질 때 비용이 늘어나는 속도. O(1)은 항상 일정, O(n log n)은 n에 log n을 곱한 정도.
