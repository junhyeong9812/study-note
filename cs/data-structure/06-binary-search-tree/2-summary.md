...
**비유: 스무고개(업다운 게임).** 1~100 중 숫자 하나를 맞힐 때 "50보다 커? 작아?"라고 물으면
한 번의 질문으로 후보 절반이 사라진다.

**이진 탐색 트리는 이 게임과 똑같은 구조다.** 질문 하나하나를 아예 칸으로 만들어 둔 게임판이 트리다.

- 모든 칸(노드)이 규칙 하나만 지킨다: **내 왼쪽 아래는 전부 나보다 작고, 오른쪽 아래는 전부 나보다 크다.**
  - *노드(node)*: 트리를 이루는 칸 하나. 값과 왼쪽/오른쪽 연결을 담는다.
- 그래서 찾을 때 한 칸에서 비교 한 번이면 반대쪽 절반은 쳐다볼 필요가 없다.
- 왼쪽 → 자기 → 오른쪽 순서로 읽으면 저절로 **작은 수부터 큰 수 순서**가 나온다. (해시맵은 못 하는 것)
  - *해시맵*: 키를 계산으로 바로 자리에 꽂는 자료구조. 빠르지만 "순서"라는 개념이 없다.
...
**그림에서 일어난 일 (쉽게)**
1. get(40): 갈림길마다 "크다/작다" 한 번 묻고 한쪽으로만 내려갔다. 왔던 길을 되돌아가지 않는다.
2. floorKey(45): 정확히 45가 없어도 된다. 내려가면서 "45 이하인 것 중 지금까지 최고 기록(best)"을 메모해 두고, 길이 끝나면 그 메모가 답이다.
...
**언제 쓰나** — 새 키-값을 넣을 때(put). 이미 있는 키면 값만 바꾼다.
...
**그림에서 일어난 일 (쉽게)**
1. 탐색과 똑같이 내려간다. 다른 점은 하나 — 빈 자리(null)를 만날 때까지 간다.
2. 그 빈 자리에 새 노드를 붙인다. **기존 노드는 하나도 안 움직인다.** 새 식구는 항상 맨 끝(잎)에 들어온다.
   - *잎(leaf)*: 자식이 하나도 없는 맨 끝 노드.
3. 함정: 10, 20, 30, 40처럼 이미 정렬된 순서로 넣으면 매번 같은 방향으로만 붙어서 한 줄이 된다(편향).
   - *편향(skew)*: 트리가 한쪽으로만 길게 늘어진 상태. 이러면 스무고개의 "절반 버리기"가 안 된다.

**비용** — 내려간 길이만큼 = O(높이). 균형이면 O(log n), 편향이면 O(n).

### 동작 — 삭제

**언제 쓰나** — 키 하나를 지울 때(remove). 지운 자리를 어떻게 메우느냐가 문제의 전부다.
...
1. 자식 0개: 그냥 떼어낸다. 부모의 연결만 끊으면(null) 끝.
2. 자식 1개: 그 자식을 한 칸 끌어올려 부모에 직접 잇는다. 줄이 하나뿐이라 순서가 안 깨진다.
3. 자식 2개: 자리를 비우면 안 되니 **대타를 데려온다** — 지울 키 바로 다음으로 큰 키(후속자).
   - *후속자(successor)*: 그 키 바로 다음으로 큰 키. 오른쪽 서브트리에서 왼쪽 끝까지 가면 나온다.
   - 대타의 값을 원래 자리에 복사한 뒤, 대타가 있던 노드를 지운다. 대타는 왼쪽 자식이 없다는 게 보장되니 그 삭제는 다시 1번 또는 2번 케이스다 — 무한 반복이 안 된다.
...
- **노드(node)**: 트리를 이루는 칸 하나. 키·값과 왼쪽/오른쪽 자식으로 가는 연결을 담는다.
- **루트(root)**: 트리의 맨 꼭대기 노드. 모든 연산이 여기서 출발한다.
- **잎(leaf)**: 자식이 하나도 없는 맨 끝 노드.
- **서브트리(subtree)**: 어떤 노드와 그 아래에 매달린 전부를 하나의 작은 트리로 본 것.
- **부모/자식(parent/child)**: 바로 위에서 나를 가리키는 노드가 부모, 내가 가리키는 아래 노드가 자식.
- **불변식(invariant)**: 어떤 조작을 해도 항상 참이어야 하는 규칙. 여기서는 "왼쪽 전체 < 자기 < 오른쪽 전체".
- **참조/포인터(reference/pointer)**: 다른 노드가 "어디 있는지"를 가리키는 값. left/right 필드가 그것이다.
- **중위 순회(in-order traversal)**: 왼쪽 → 자기 → 오른쪽 순서로 트리를 도는 방법. BST에서는 이 순서가 곧 정렬 순서다.
- **재귀(recursion)**: 함수가 자기 자신을 다시 부르는 방식. "서브트리도 똑같은 트리다"라는 성질 덕에 자연스럽다.
- **O(log n) / O(n)**: 데이터가 n개일 때 걸리는 일의 양. O(log n)은 매번 절반씩 버려서 n이 100만이어도 약 20번, O(n)은 전부 다 보는 것.
- **편향(skew)**: 트리가 한쪽으로만 길게 늘어진 상태. 사실상 연결 리스트가 되어 O(n)으로 느려진다.
- **후속자(successor)**: 어떤 키 바로 다음으로 큰 키. 자식 2개인 노드를 지울 때 그 자리를 대신 채우는 후보.
- **floorKey / ceilingKey**: floor는 "그 값 이하 중 가장 큰 키", ceiling은 "그 값 이상 중 가장 작은 키". 정확히 일치하는 키가 없어도 이웃을 찾아 준다.
- **가지치기(pruning)**: 답이 있을 수 없는 가지를 아예 들어가지 않고 통째로 건너뛰는 것. keysInRange가 한다.
- **회전(rotation)**: 불변식을 유지하면서 노드들의 부모-자식 관계를 바꿔 트리 모양을 고치는 조작. 편향을 막는 도구(16장 레드-블랙 트리).
- **해시맵(hash map)**: 키를 해시 함수로 바꿔 자리를 정하는 자료구조. 평균 O(1)로 빠르지만 키의 순서·범위 질의는 못 한다.
- **연결 리스트(linked list)**: 노드가 다음 노드 하나만 가리키며 한 줄로 이어진 구조. 편향된 BST가 이 꼴이 된다.
