...
- 1등은 항상 맨 꼭대기(배열의 0번 칸)에 있다. 물으면(peek) 바로 답한다.
- 규칙은 단 하나: **부모는 자기 두 자식보다 앞선다.** 형제끼리 순서는 신경 안 쓴다.
  - *부모/자식*: 트리에서 바로 위 칸이 부모, 바로 아래 두 칸이 자식.
- 약속을 이만큼만 하니까 넣기/빼기가 둘 다 빠르다(O(log n)). 전부 정렬해 두면 넣기가 느려진다(O(n)).
...
**언제 쓰나** — 새 원소를 넣을 때(insert). "끝에 놓고 → 제자리까지 떠올린다" 두 단계다.
...
2. 부모와 비교해서 내가 더 앞서면 자리를 바꾼다. 물속에서 공기방울이 떠오르듯 위로만 간다(siftUp).
3. 루트에 닿거나, 부모가 이미 나보다 앞서면 멈춘다. 형제나 옆 가지는 한 번도 안 본다.
...
**언제 쓰나** — 1등을 꺼내 갈 때(poll). "루트를 빼고 → 마지막 원소를 루트에 올리고 → 제자리까지 가라앉힌다" 세 단계다.
...
2. 배열 **마지막** 원소(4)를 루트 자리에 올렸다 — 트리 모양을 안 깨고 없앨 수 있는 칸이 마지막 칸뿐이라서다. "지우고 앞으로 당기기"를 하면 모든 부모-자식 계산(2i+1)이 어긋난다.
3. 올라온 4가 규칙을 깨니, **두 자식 중 더 앞선 쪽**과 비교해 지면 자리를 바꾸며 아래로 가라앉힌다(siftDown). 자식이 없거나 두 자식 모두에게 이기면 멈춘다.

**비용** — 내려간 길은 아래로 한 줄 = O(log n). peek은 0번 칸 읽기 = O(1).
...
**그림에서 일어난 일 (쉽게)**
1. insert(4): 책꽂이에 책 한 권 끼워 넣기와 같다 — 자리를 만들려고 뒤의 책들을 한 칸씩 민다. 최악엔 전부 밀어야 하니 O(n).
2. poll(): 배열 끝이 1등이 되도록 정렬해 뒀으니, 끝에서 하나 떼면 끝. 미는 일이 없어 O(1).
3. 교훈: "항상 완전 정렬"의 값을 insert가 치른다. 힙은 완전 정렬을 포기하는 대신 insert/poll 둘 다 O(log n)으로 맞춘다.
