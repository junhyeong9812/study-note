...
> **정점(vertex, V)** — 그래프의 점 하나.\
> 예: 여기서는 이름도 객체도 없이 0, 1, 2... 정수 번호로만 부른다.

> **간선(edge, E)** — 두 정점을 잇는 연결선. 요금(가중치)이 붙을 수 있다.\
> 예: v0과 v1을 잇는 선로 하나가 간선 하나이고, 거기 적힌 4가 가중치다.
...
- 어느 쪽이든 담는 노선도(그래프)는 같다.\
  종이(메모리)와 질문 속도의 거래만 다르다.
- 실무에서 같은 구조: SNS 친구 관계, 내비게이션 도로망, 빌드 의존성 그래프가 전부 이것이다.

> **그래프(graph)** — 점(정점)들과 그 사이 연결(간선)로 관계를 표현하는 자료구조.\
> 예: 노선도·친구 관계·빌드 의존성이 전부 같은 그래프 한 종류다.
...
> **인접 리스트(adjacency list)** — 정점마다 "내가 닿는 곳 목록"을 들고 있는 표현.\
> 예: v0의 목록에 Edge(to=1, w=4)와 Edge(to=3, w=2)가 들어 있으면 v0에서 v1·v3으로 갈 수 있다는 뜻이다.
...
2. 없어서 **양쪽 목록에 하나씩** 적었다.\
   "v0의 옆 역에 v3", "v3의 옆 역에 v0".\
   무방향이라 서로의 목록에 적어야 한다.

> **방향/무방향(directed/undirected)** — 일방통행 길 / 양방향 길.\
> 예: 무방향이면 v0→v3으로도 v3→v0으로도 갈 수 있어서, 간선 하나가 양쪽 목록에 한 줄씩 적힌다.

3. 종이에 적은 줄(Edge 객체)은 2개지만 선로(간선)는 1개 — edges는 1만 올린다.

> **셀프루프(self-loop)** — 자기 자신으로 가는 간선(from == to).\
> 예: 무방향에서 0-0 간선은 같은 목록에 두 번 넣으면 중복이 되므로 Edge를 하나만 넣는다.
...
1. hasEdge/weight: v0의 옆 역 목록을 앞에서부터 하나씩 대조했다.\
   목록이 길수록 오래 걸린다 — 이 표현의 약점.
2. neighbors: 목록에서 역 번호만 뽑아 돌려줬다.\
   **내 옆 역만 보고 남의 목록은 안 본다** — 그래서 BFS/DFS/다익스트라처럼 "이웃 따라 걷는" 알고리즘이 이 표현을 좋아한다.

> **BFS(너비 우선 탐색)** — 가까운 곳부터 물결처럼 퍼지며 방문하는 순회. 큐를 쓴다.\
> 예: v0에서 시작하면 옆 역(v1, v3)을 다 보고 나서야 그 다음 역(v2, v4)으로 넘어간다.

> **DFS(깊이 우선 탐색)** — 한 길로 끝까지 들어갔다가 막히면 되돌아 나오는 순회. 스택(또는 재귀)을 쓴다.\
> 예: v0 → v1 → v2 까지 들어가 막히면 되돌아 나와 v3 쪽을 본다.

**비용** — 전부 그 정점의 목록 길이 = O(deg).\
전체 순회는 O(V+E).
...
> **인접 행렬(adjacency matrix)** — V×V 표를 깔아 두고 (from, to) 칸에 가중치를 적는 표현.\
> 예: 정점이 5개면 간선이 4개뿐이어도 5×5 = 25칸을 늘 잡는다.
...
2. (0, 3) 칸에 2를 적었다.\
   무방향이라 대칭 칸 (3, 0)에도 같이 적었다 — 표가 대각선 기준 거울처럼 되는 이유.
3. 훑는 게 없다.\
   어느 칸인지 좌표로 바로 아니까 **표 읽고 쓰기 한 번** = O(1).
...
1. hasEdge/weight: (0, 3) 칸 하나만 읽으면 끝 = O(1).\
   리스트가 목록을 훑던 자리가 즉답이 됐다.
2. neighbors(0): 0번 행을 왼쪽부터 끝까지 훑으며 채워진 칸의 번호를 모았다.\
   이웃이 하나도 없어도 V칸을 다 봐야 한다 = O(V).
...
> 다만 꺼낼 때 표시하면 같은 정점이 큐에 여러 번 들어가 메모리를 더 쓴다.\
> 정확성이 아니라 비용의 문제다.\
> 시그니처: `static int[] bfsDistances(Graph graph, int start)`

> **큐(queue)** — 먼저 넣은 것이 먼저 나오는 줄.\
> 예: BFS가 "가까운 곳부터" 퍼질 수 있는 이유가, 먼저 발견한 정점이 먼저 나오는 이 순서 때문이다.
...
> 생각할 것 — 스택이 필요하다. 03번에서 만든 것이 이것이다.\
> 재귀를 쓰면 호출 스택이 그 역할을 한다. /\
...
> **스택(stack)** — 나중에 넣은 것이 먼저 나오는 더미.\
> 예: DFS가 "한 길로 끝까지" 들어가는 이유가, 방금 발견한 이웃을 먼저 꺼내는 이 순서 때문이다.

> **재귀(recursion)** — 함수가 자기 자신을 부르는 것.\
> 예: DFS를 재귀로 쓰면 자바의 호출 스택이 스택 역할을 대신하지만, 그래프가 깊으면 그 스택이 넘쳐 StackOverflow 가 난다.
...
> **위상 정렬(topological sort)** — 방향 그래프에서 "모든 화살표가 앞→뒤가 되도록" 정점을 줄 세우는 것.\
> 예: 선수과목이 `0->1` 이면 0을 1보다 앞에 놓아야 한다.

> **순환(cycle)** — 출발한 정점으로 되돌아오는 길.\
> 예: `0->1, 1->0` 이면 어느 쪽을 먼저 놓아도 화살표가 뒤로 가므로 위상 정렬이 아예 불가능하다.
...
> **다익스트라(Dijkstra)** — 가중치 합 기준 최단거리 알고리즘.\
> 예: "지금까지 알아낸 것 중 가장 가까운 정점"을 힙(우선순위 큐)에서 반복해 꺼내며 거리를 확정해 간다.
