...
- 단어를 통째로 저장하지 않는다. "cat"은 c → a → t 세 개의 길로만 존재한다.
- 그래서 cat, car, card는 "ca"까지의 길을 **셋이 나눠 쓴다** — 앞부분이 같으면 저장도 한 번만.
- 찾는 비용은 단어 길이만큼(O(L)). 담긴 단어가 100만 개여도 상관없다.
...
**언제 쓰나** — 단어를 등록할 때(insert). "글자 따라 내려가며 없는 길만 새로 깐다".
...
**그림에서 일어난 일 (쉽게)**
1. contains("car"): 글자 따라 내려가 도착한 노드에 end 도장이 있으면 단어다. "ca"는 노드는 있지만 도장이 없어 거짓.
2. countWithPrefix("ca"): 도착한 노드의 장부(wordsBelow)를 **읽기만** 했다. 세어 보는 대신 넣을 때마다 세어 둔 것 — 이 챕터의 핵심 아이디어.
3. keysWithPrefix("ca"): 그 노드 아래만 훑어 end 도장이 찍힌 곳마다 지나온 글자들을 이어 단어로 만든다. 내려갈 때 글자를 붙이고 돌아 나올 때 떼는 것이 백트래킹이다.
   - *백트래킹(backtracking)*: 갈림길을 다 둘러보고 온 길을 되짚어 나오면서, 가던 길 기록을 원래대로 되돌리는 것.

**비용** — 접두사까지 O(L), 목록 만들기는 O(접두사 길이 + 결과 글자 수). 트라이 전체 크기와 무관.

### 동작 — 삭제

**언제 쓰나** — 단어 하나를 지울 때(remove). 남들과 같이 쓰는 길은 남기고, 나 혼자 쓰던 길만 끊는 게 요점이다.
...
2. 장부가 0이 된 첫 노드 = 이제 아무 단어도 안 지나는 길. 부모에서 그 연결 하나만 끊으면 그 아래 전체가 한꺼번에 버려진다(가비지 컬렉션이 회수).
3. 끝까지 내려가도 0이 안 나오면(남들이 아직 그 길을 쓰면) 아무것도 못 끊는다 — 마지막 노드의 end 도장만 지운다. "car"를 지워도 "card"가 그 길로 다녀야 하니 노드는 남는다.
...
**언제 쓰나** — "ca."처럼 아무 글자나 되는 칸(.)이 섞인 패턴으로 단어를 찾을 때(search).
  - *와일드카드(wildcard)*: 아무 글자와도 맞는 만능 문자. 여기서는 `.` 하나가 글자 하나를 대신한다.
...
2. '.'을 만나자 갈림길의 **모든 자식을 하나씩 다 시도**했다. 하나라도 성공하면 전체가 참.
3. 막힌 길은 false를 돌려주며 호출한 곳으로 되돌아간다 — 이 "돌아감" 자체가 백트래킹이라 되돌리기 코드가 따로 없다.

**비용** — '.'이 없으면 O(L). '.' 하나마다 그 자리의 자식 수만큼 갈래가 곱해진다.
