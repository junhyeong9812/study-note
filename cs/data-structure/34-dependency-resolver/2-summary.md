...
- 어떤 일들은 순서가 정해져 있다: A가 끝나야 B를 할 수 있다("B는 A에 기댄다" = 의존).
- 이 관계를 화살표 그림(그래프)으로 그려 두고, "모두를 만족하는 한 줄 순서"를 뽑는 것이 위상 정렬이다.
- 방법 1(칸/Kahn): "지금 당장 할 수 있는 일"(기다릴 것이 0개인 일)부터 하나씩 해치우고, 끝날 때마다 기다리던 일들의 카운트를 줄인다.
- 방법 2(DFS): 한 일에서 출발해 "이게 끝나야 되는 일"을 끝까지 파고 내려갔다가, 돌아오면서 순서를 적는다.
- 만약 "A는 B를, B는 A를 기다리는" 맞물림(순환)이 있으면 순서가 아예 없다 — 둘 다 그것을 감지하고, DFS는 어느 고리인지도 짚어 준다.

```text
   밥 ----> 볶음밥 ----> 도시락        가능한 순서 = 밥, 볶음밥, 도시락
   (화살표 = "먼저 끝나야 한다")
...
언제 쓰나: "d는 b에 기댄다"는 관계 하나를 그래프에 기록할 때(`dependsOn`). 아래 그림은 전/후 비교 — edges에는 "b 다음에 d"가 담기고, 기대는 쪽(d)의 진입 차수만 1 오른다. 같은 관계를 두 번 걸었을 때 카운트를 두 번 올리면 왜 "가짜 순환"이 생기는지가 이 절의 함정이다.
  ...
  - *간선(edge)*: 그래프에서 두 점을 잇는 화살표 하나. 여기서는 "먼저 → 나중" 관계 하나.
...
언제 쓰나: 순서를 실제로 뽑을 때(`resolve`). 요령은 "지금 기다릴 것이 0개인 일부터 한다"이다. 아래 표는 한 걸음마다 (꺼낸 것 / 대기열 ready / 남은 카운트 remaining / 지금까지의 순서 order)가 어떻게 변하는지를 전부 보여준다 — 하나를 끝낼 때마다 그것을 기다리던 일들의 카운트를 1씩 내리고, 0이 된 일을 대기열에 넣는다.
  ...
  - *위상 정렬(topological sort)*: 모든 화살표(먼저→나중)를 만족하는 한 줄 순서. 답이 여러 개일 수 있다.
...
언제 쓰나: 같은 위상 정렬을 다른 길로 — 한 점에서 끝까지 파고 내려갔다 돌아오며 적는 방식. 미로 탐험에서 갈림길마다 분필로 표시를 남기는 것과 같다: 안 가 본 곳(WHITE), 지금 지나가는 중(GRAY), 다 둘러보고 나온 곳(BLACK). 아래 그림 [1]은 순환 없는 그래프에서 "돌아오면서 적기(후위) → 뒤집기 = 위상 순서"의 전 과정, [2]는 지나가는 중(GRAY)인 곳을 또 만나면 그 자리가 바로 순환이라는 것을 보여준다.
  ...
  - *DFS(깊이 우선 탐색)*: 갈 수 있는 데까지 먼저 내려가고, 막히면 되돌아오는 탐색 순서.
  - *후위(postorder)*: "내 자식들을 전부 끝낸 뒤에" 자기를 적는 기록 순서.
...
- **간선(edge) / 인접 리스트**: 화살표 하나 / "각 점 → 그 점에서 나가는 화살표들의 목록"으로 그래프를 저장하는 방식.
- **진입 차수(in-degree)**: 어떤 점으로 들어오는 화살표 수 = 내가 기다려야 하는 일의 개수. 0이면 당장 시작 가능.
- **위상 정렬(topological sort)**: 모든 "먼저→나중" 화살표를 만족하는 한 줄 순서. 답이 여러 개일 수 있다.
- **순환(cycle)**: 화살표를 따라가면 제자리로 돌아오는 고리. 순환이 있으면 위상 정렬이 불가능하다.
- **칸 알고리즘(Kahn)**: 진입 차수 0인 것부터 꺼내고, 끝날 때마다 이웃의 차수를 내리는 위상 정렬 방법.
- **DFS(깊이 우선 탐색)**: 갈 수 있는 데까지 파고 내려갔다가 막히면 되돌아오는 탐색.
- **후위(postorder)**: 자식들을 전부 끝낸 뒤에 자기를 적는 순서. 뒤집으면 위상 순서가 된다.
- **WHITE / GRAY / BLACK**: 안 봤다 / 지금 내려가는 중 / 다 보고 돌아왔다의 3색 표시. GRAY 재방문 = 순환.
- **back edge**: 지금 내려온 길 위(GRAY)로 되돌아가는 화살표. 순환의 직접 증거.
- **재귀(recursion) / 재귀 스택**: 함수가 자기 자신을 부르는 것 / 그때마다 쌓이는 호출 기록.
- **층(layer)**: "동시에 시작해도 되는 일들"의 묶음. 층 0 = 아무것도 안 기다리는 일들.
- **결정성(determinism)**: 같은 입력이면 항상 같은 답이 나오는 성질. Tree 계열 맵과 우선순위 큐로 만든다.
- **PriorityQueue(우선순위 큐)**: 넣은 순서가 아니라 가장 작은 것부터 꺼내는 줄.
- **TreeMap / TreeSet**: 항상 정렬 순서를 유지하는 자바의 맵/집합. 순회 순서가 정해져 답이 하나로 고정된다.
- **relaxation**: 진입 차수를 1 내리는 동작. 총횟수는 간선 수와 같아야 한다(자가 검증용 계수).
