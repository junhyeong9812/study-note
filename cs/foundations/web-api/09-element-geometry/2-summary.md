...
- ★ **「시간이 안 흔들린다」는 뜻이 아니다** — 이 주제는 **시간을 아예 재지 않는다.** 읽기 비용은 [08번 주제](../08-getcomputedstyle/2-summary.md)와 목록의 **10번 주제**가 잰다.
...
- ★ **읽는 값이 비싸다.** 이 주제의 모든 읽기가 목록의 **10번 주제**가 재는 「강제 동기 레이아웃」의 방아쇠다.
...
비용 — 이 읽기들이 전부 **레이아웃을 강제한다.** 목록의 **10번 주제**가 그 값을 잰다.
...
비용 — `offsetParent` 를 읽는 것도 **레이아웃을 강제한다**(목록의 **10번 주제**에서 실측).
...
비용 — `elementFromPoint` 는 **레이아웃에 히트 테스트까지** 얹는다. 목록의 **10번 주제** 실측에서 **이 주제의 읽기 가운데 가장 비쌌다.**
...
비용 — `getClientRects()` 도 `getBoundingClientRect()` 와 같은 값의 레이아웃 강제다(목록의 **10번 주제** 실측).
...
이 주제의 모든 읽기가 **강제 동기 레이아웃**을 부른다. 고치는 법은 목록의 **10번 주제**가 정본이다.
...
| 여러 요소를 잰다 | 읽기를 **한 묶음으로** 모은다 | 쓰기와 번갈아(목록의 **10번 주제**) |
...
- **강제 동기 레이아웃(forced synchronous layout)** — 쓰기로 더러워진 레이아웃을 읽기가 그 자리에서 다시 계산하게 만드는 것. 이 주제의 모든 읽기가 방아쇠다(목록의 **10번 주제**).
...
- **`scrollTop` 이 소수를 받나** — 목록의 **11번 주제**에서 실측했다. 이 주제에서는 **읽는 쪽만** 다뤘다.
