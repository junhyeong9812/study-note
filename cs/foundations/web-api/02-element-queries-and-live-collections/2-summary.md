...
- 그래서 **정적 컬렉션은 「문서에 없는 노드」를 들고 있을 수 있다.** 이것이 메모리를 붙드는 경로가 되기도 한다([목록의 **20번 주제**](../20-listener-lifetime/)).
...
- 그래서 **정적 스냅샷은 「지운 화면의 잔해」를 붙들 수 있다.** `isConnected` 가 `false` 인 노드를 배열에 담아 두는 것이 누수 경로가 된다([목록의 **20번 주제**](../20-listener-lifetime/)).
...
- [목록의 **20번 주제**](../20-listener-lifetime/)(리스너 수명) — 정적 스냅샷이 떨어진 노드를 붙드는 것이 누수가 되는 경로
