...
- **실무 함의** — `<button>` 에 못 붙는다. 그래서 단추 컴포넌트는 **커스텀 요소**(목록의 **13번 주제**)를 만들고 그 안에 `<button>` 을 넣는다.
