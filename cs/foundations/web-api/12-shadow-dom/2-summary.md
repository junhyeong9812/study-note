...
- ★ **이벤트 전파의 정본은 [목록의 16번 주제](../16-event-propagation-phases/)**(전파 3단계)와 [목록의 **21번 주제**](../21-custom-events/)(`CustomEvent`·`composed`)다. 여기서는 **경계가 무엇을 하는가**까지만 본다.
...
(14)에서 **바깥 네 자리가 전부 `#host`** 였다. `e.composedPath()[0]` 이 답이다. 이벤트 위임이 그림자에서 깨지는 이유가 이것이다([목록의 **18번 주제**](../18-event-delegation/)).
...
- [목록의 **16번 주제**](../16-event-propagation-phases/)(전파 3단계) · [목록의 **18번 주제**](../18-event-delegation/)(이벤트 위임) — 재타기팅이 **위임을 어떻게 깨뜨리나**
- [목록의 **21번 주제**](../21-custom-events/)(`CustomEvent`·`composed`) — ★ **재타기팅과 `composed` 의 정본은 그쪽**이다. 여기서는 경계가 하는 일까지만
