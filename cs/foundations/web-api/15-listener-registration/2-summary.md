...
| **안 흔들린다 — 단 호출 수와 다르다** | **콘솔 경고의 줄 수** | 판마다 **두 줄**로 같다. 8번 부른 `preventDefault` 가 두 줄인 것은 **합친 것이 아니라, 기본 passive + 합성 이벤트 조합은 경고를 내지 않기 때문**이다((9) · [19번 주제](../19-passive-and-scroll/2-summary.md)의 (6)) |
...
| **콘솔에 안 남은 무시** | 기본 passive + 합성 이벤트 조합은 **무시되는데 경고를 내지 않는다**((9)의 두 줄 밖의 호출이 그것이다 · [19번 주제](../19-passive-and-scroll/2-summary.md)의 (6)) |
...
- ★ ★ **줄 수를 믿으면 안 된다.** 이 실행에서 `passive` 리스너 안의 `preventDefault()` 는 **여덟 번** 불렸는데 콘솔에는 **두 줄**만 남았다. **합친 것이 아니다** — 두 줄 다 가리키는 소스 14행은 **명시 `{ passive: true }` 리스너**이고 그것이 두 번 불린 것과 맞는다. **나머지 호출은 기본 passive + 합성 이벤트 조합이라 경고를 아예 내지 않는다**([19번 주제](../19-passive-and-scroll/2-summary.md)의 (6)이 같은 페이지를 다시 열어 확인했다 — 그 밖의 칸은 전부 호출 한 번 = 경고 한 줄이었다). **「경고가 안 보인다」를 「안 났다」로 읽으면 안 된다.**
...
실측에서 **여덟 번의 위반이 두 줄만 남았다** — 합친 것이 아니라 **기본 passive + 합성 이벤트 조합이 경고를 내지 않았다**([19번 주제](../19-passive-and-scroll/2-summary.md)의 (6)). 경고의 **건수는 근거가 아니다.**
...
| **콘솔 경고의 줄 수** | ★ **구현.** Chrome 은 기본 passive + 합성 이벤트 조합에 경고를 내지 않는다([19번 주제](../19-passive-and-scroll/2-summary.md)의 (6)) |
