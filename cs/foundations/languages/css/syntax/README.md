...
| 03 | [상속 — 상속되는 속성과 `inherit`/`initial`/`unset`/`revert`/`revert-layer`](03-inheritance-and-global-keywords/) | 계산 모델 | 어떤 속성이 자식에게 내려가고 다섯 전역 키워드가 각각 무엇으로 되돌리는지 예측할 수 있다 | 01 | — | 도움 | A |
| 04 | [값 처리 단계 — 지정값·계산값·사용값·실제값](04-value-processing-stages/) | 계산 모델 | `em`·`%`·`auto` 가 어느 단계에서 픽셀이 되는지, `getComputedStyle` 이 무엇을 돌려주는지 설명할 수 있다 | 03 | — | 도움 | B |
| 05 | [`@layer` 캐스케이드 레이어 — 선언 순서와 레이어 밖의 위치](05-cascade-layers/) | 계산 모델 | 레이어를 쓰면 명시도가 왜 덜 중요해지는지, `!important` 에서 레이어 순서가 왜 뒤집히는지 설명할 수 있다 | 01 | — | 도움 | B |
| 06 | [`@scope` — 스코프 루트·하한과 근접성(proximity)](06-scope/) | 계산 모델 | 스코프가 캐스케이드의 어느 자리에 끼어드는지, 하한(`to`)이 무엇을 잘라내는지 판단할 수 있다 | 05 | — | 도움 | C |
| 07 | [CSS 구문과 오류 복구 — 선언·at-rule·모르는 것을 버리는 규칙](07-syntax-and-error-recovery/) | 계산 모델 | 모르는 속성·값 하나가 선언 하나만 버려지는지 규칙 전체가 버려지는지 예측하고, 이것이 점진적 향상의 토대임을 설명할 수 있다 | — | [`../../../compiler-pipeline/`](../../../compiler-pipeline/) (렉서·파서 일반론) | 불필요 | C |
| 08 | [기본 선택자·조합자·속성 선택자](08-basic-selectors-and-combinators/) | 선택자 | 자손/자식/인접형제/일반형제를 구분하고 `^=`·`$=`·`*=`·`i` 플래그로 속성 매칭을 설계할 수 있다 | 02 | — | 도움 | A |
| 09 | [구조적 의사 클래스 — `:nth-child(An+B of S)` 계열](09-structural-pseudo-classes/) | 선택자 | `nth-child` 와 `nth-of-type` 이 갈리는 지점과 `of S` 가 세는 대상을 바꾸는 것을 예측할 수 있다 | 08 | — | 필수 | A |
| 10 | [상태·폼 의사 클래스 — `:hover`/`:focus-visible`/`:checked`/`:user-invalid`](10-state-and-form-pseudo-classes/) | 선택자 | `:focus` 와 `:focus-visible` 이 갈리는 입력 방식, `:invalid` 와 `:user-invalid` 가 갈리는 시점을 판단할 수 있다 | 08 | — | 필수 | A |
| 11 | [`:is()`·`:where()`·`:not()` 와 명시도](11-is-where-not/) | 선택자 | 같은 목록을 `:is()` 로 쌌을 때와 `:where()` 로 쌌을 때 명시도가 어떻게 달라지는지 계산할 수 있다 | 02, 08 | — | 도움 | A |
| 12 | [`:has()` — 관계 선택자와 그 한계](12-has-relational-selector/) | 선택자 | 「자식을 가진 부모」를 선택하는 형태를 쓰고, `:has()` 안에 또 `:has()` 를 못 쓰는 제약과 명시도 기여를 설명할 수 있다 | 11 | — | 필수 | A |
| 13 | [의사 요소와 생성 콘텐츠 — `::before`/`::after`·`content`·`::marker`·카운터](13-pseudo-elements-and-generated-content/) | 선택자 | 생성 콘텐츠가 어느 상자에 들어가고 접근성 트리에 어떻게 보이는지, `counter()` 로 번호를 만드는 형태를 설계할 수 있다 | 08 | — | 필수 | A |
| 14 | [중첩(nesting) — `&` 의 의미와 중첩 규칙의 명시도](14-css-nesting/) | 선택자 | 중첩이 펼쳐진 결과 선택자를 손으로 복원하고 `&` 가 `:is()` 로 감싸지며 명시도를 올리는 자리를 예측할 수 있다 | 11 | — | 불필요 | B |
| 15 | [박스 모델과 `box-sizing`](15-box-model-and-box-sizing/) | 박스·레이아웃 | `width` 가 어디까지를 가리키는지 두 모드로 나눠 계산하고 패딩·테두리가 더해지는 자리를 예측할 수 있다 | 04 | — | 필수 | A |
...
| `:user-valid`/`:user-invalid` | 10 | **widely** | 2023-11-02 | 2026-05-02 |
...
| 속성 선택자의 `s` 플래그(대소문자 구분) | **limited** | **Firefox 66 만**. 뺀 것이 아니라 `08` 안에서 **「쓰면 안 되는 것」으로** 다룬다 — Chrome 151 에서 `querySelectorAll` 은 `SyntaxError` 를 던지는데 **CSS 규칙은 `cssRules` 에서 조용히 사라진다**(실측). 같은 문법이 자리에 따라 소리 나게도 소리 없이도 실패하는 자리라 학습 가치가 있다 |
