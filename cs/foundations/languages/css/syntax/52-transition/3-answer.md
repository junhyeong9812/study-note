...
- 반복·무한 재생·역방향 재생도 `animation` 쪽이다. 정본은 [목록의 **53번 주제**](../53-keyframes-and-animation/).
...
- 둘 다 Baseline **newly**(2024-08-06) — 정본은 [목록의 **57번 주제**](../57-starting-style-and-entry-exit-transitions/).
...
- 정본은 [목록의 **60번 주제**](../60-prefers-reduced-motion/).
...
**방법** — `extract-demo-blocks.py` 는 이 주제의 블록을 거의 다 「확인 불가」로 찍는다(`:hover`·`transition` 이 걸려 있다). 그래서 **CDP 로 직접 몰았다**: 블록마다 `Page.navigate` 로 **새로 띄우고**, `Input.dispatchMouseEvent` 로 실제 마우스를 올린 뒤, 벽시계 기준 시각마다 `Runtime.evaluate` 로 `getComputedStyle` 을 읽었다. **`--virtual-time-budget` 은 쓰지 않았다**(시간이 걸린 실험이 조용히 무효가 된다).

| 무엇을 | 어떻게 | 결과 |
|---|---|---|
| demo (3) 버튼 색 | CDP · 0·50·150·300·450·700ms · **3판** | **문서의 6행과 세 판 모두 글자까지 일치** |
| demo (2) `.fixed` 높이 | CDP · 30·400·1300ms · 2판 | `24.7969` → `33.5938`/`34` → `48` — 400ms 칸만 **한 프레임 흔들린다**(문서는 34) |
| demo (2) `.auto` 높이 | 같은 판 | 30ms 에 이미 `48px` — 전환이 시작조차 안 했다 |
| demo (4) `linear`·`ease`·`ease-in` | CDP · 50·500·1000·1500·2300ms · 2판 | `6.50/65.00/130.00/195.00` · `3.41/106.22/208.62/249.72` · `0.30/24.30/81.99/161.68` — **문서 표와 일치** |
| ★ demo (4) `steps(4, end)` | 같은 실험을 **8판** 반복하고 경계에서 떨어진 시각도 같이 읽었다 | **경계 칸이 흔들린다** — 500ms `65px` 6판 / `0px` 2판 · 1000ms `130px` 6판 / `65px` 2판 · 1500ms `195px` 5판 / `130px` 3판. 경계 밖(50·300·700·1200·1700ms)은 **8판 전부 같았다**(`0/0/65/130/195`) |
| demo (5) 지연 | CDP · 30·500·900·1150·1400ms · 2판 | **문서의 5행과 두 판 모두 일치** |
| (6) 되돌리기 | CDP · 올린 뒤 600ms 재고, 마우스를 **상자 밖으로** 보낸 뒤 50·300·700ms · 2판 | 올린 뒤 600ms `64.20px` · 뗀 뒤 `60.97` → `36.72` → **`6px`(원위치)**. 문서(`65.8`/`62.6`/`38.3`/`6`)보다 **일관되게 1.6px 낮다 = 정확히 한 프레임** |
| 「`:hover` 안에만 쓰기」 | CDP · 올릴 때 50·500·1100ms, 뗄 때 50·500ms · 2판 | 올릴 때 `rgb(150, 156, 176)` → `rgb(166, 96, 106)` → `rgb(185, 28, 28)` · **뗄 때는 50ms 에 이미 원래 색** — 문서와 일치 |
| 2번 문항의 `grid-template-rows` | CDP · 30·500·1300ms · 2판 | `0px` → `12px` → `48px`, 자식 높이가 그대로 따라갔다(문서 `11.8px` 는 한 프레임 차이) |
| 4번 문항의 **음수 지연** | CDP · 30·300·600ms · 2판 | 30ms 에 `rgb(168, 91, 101)`(문서와 같은 값) / `rgb(167, 93, 103)`(한 프레임 차) · **600ms 도착** |
| 5번 문항의 `steps(4, start)` | CDP · 50·500·1000·1500ms · 2판 | 50ms `65px` 는 두 판 같고, **500·1000·1500ms 는 여기서도 흔들린다**(`65/195/195` 대 `130/195/260`) — `end` 판과 같은 경계 문제 |
| (8) `allow-discrete` | CDP · 클래스를 붙인 뒤 50·500·900·1300ms · 2판 | `block/0.95` → `block/0.50` → `block/0.10` → **`none/0`** — 문서와 일치 |
...
| 안 흔들린다 | 흔들린다 |
|---|---|
| 「전환이 걸렸나 안 걸렸나」 · 도착값 · 도착 시각 · `display` 가 `block` 에서 `none` 으로 넘어가는 순서 · **넷이 갈리나 한 값이냐** | **`steps()` 의 계단 경계 시각 샘플** · 소수 마지막 자리 · 한 프레임(약 16.7ms)에 해당하는 양 |

**돌리지 않은 것** — 전환 선언이 `!important` 를 이기는 것(사다리 1번 칸). 본문 (7)절과 8번 문항에 「실행 확인하지 않았다」고 적혀 있고 **재검증에서도 그대로 두었다.** 만들 수 있는 실험이므로 「못 잰 것」이 아니라 **다음 판에서 돌릴 자리**다.

**구현 의존 항목** — ① `steps()` 경계 시각의 값 ② 모든 수치의 마지막 한두 자리(프레임 간격에 달렸다) ③ `opacity` 가 `0.949953` 처럼 직렬화되는 것. **Chrome 151 에서 관찰한 것**이다.\
