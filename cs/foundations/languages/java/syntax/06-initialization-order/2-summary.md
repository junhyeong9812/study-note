...
- 생성자의 **첫 문장**만 `this(...)` 또는 `super(...)` 가 될 수 있다.\
  (JDK 25의 유연한 생성자 본문은 이 앞에 일부 문장을 허용한다 — [**07번 주제**](../07-constructors/).)
...
> 이 규칙 자체는 [**09번 주제**](../09-inheritance-overriding/)(상속과 오버라이딩)가 정본이다.
...
- [**07번 주제**](../07-constructors/)(생성자 — `this()`/`super()`·25 유연한 생성자 본문) — `super()` 앞에 무엇을 쓸 수 있는지가 정본
- [**09번 주제**](../09-inheritance-overriding/)(상속과 오버라이딩 — 동적 디스패치) — 「어디서 틀리나」 1번의 디스패치 규칙이 정본
- [**12번 주제**](../12-nested-classes/)(중첩 클래스) — 익명 클래스와 초기화 블록의 조합
