...
> **이 주제는 lvalue 참조만** 다룬다(그쪽 정본은 목록의 **09번 주제**).
...
> 「값 범주(어떤 식에 참조가 묶이나)」는 목록의 **08번 주제**, 「rvalue 참조·`move`」는 목록의 **09번 주제**,\
> 「`const` 정확성 설계」는 목록의 **10번 주제**, 「매개변수 전달 방식 고르기」는 목록의 **11번 주제**,\
> 「댕글링과 수명」은 목록의 **30번 주제**가 정본이다. 여기서는 **그 앞의 한 겹**까지만 판다.
>
> ★★ **흔들리는 칸 / 안 흔들리는 칸**
>
...
- ★★ **④의 단서가 중요하다** — ``which is not a typedef or a template type argument``.\
  **직접 적을 수는 없지만 템플릿 안에서는 생길 수 있다**는 뜻이고,\
  그때 **참조 축약**(`T& &` → `T&`)이 일어난다. 그 규칙이 **전달 참조의 밑바탕**이다(목록의 **09번 주제**).
...
7. 그 밖에는 **참조**가 기본값이다 — **널 검사를 안 써도 되고**, 호출부가 `&` 없이 깔끔하다.\
   ★ 단 **작은 값**(`int`·`double`)은 **값으로 받는 편이 낫다** — 정본은 목록의 **11번 주제**.
...
★ 판단의 정본은 목록의 **11번 주제**(매개변수 전달 방식 고르기)다.
...
템플릿 안에서는 생기고 **참조 축약**으로 접힌다. 그것이 **전달 참조의 밑바탕**이다(목록의 **09번 주제**).
...
- 목록의 **08번 주제**(값 범주) — **어떤 식에 참조가 묶이나**. `T&` 는 lvalue 에만 묶인다.
- 목록의 **09번 주제**(rvalue 참조·`move`·`forward`) — **`T&&` 와 참조 축약**.\
  ★ (2)의 ④에 붙은 단서가 거기로 이어진다.
- 목록의 **10번 주제**(`const` 정확성) — `const T&` 를 **설계 도구**로 쓰는 법.
- 목록의 **11번 주제**(매개변수 전달 방식) — ★ **값·`const&`·`&&` 중 무엇으로 받나**의 정본.\
  (3)의 「참조가 공짜는 아니다」가 거기서 결론난다.
- 목록의 **30번 주제**(댕글링 참조와 수명 연장) — ★ (5)·(7)의 정본.\
...
- **rvalue 참조 `T&&`** — C++11. 이 문서는 **lvalue 참조만** 던졌다. 정본은 목록의 **09번 주제**.
