...
- ★★ **④의 단서** — ``which is not a typedef or a template type argument`` 는\
  ...
  그때 **참조 축약**(`T& &` → `T&`)이 일어나고, 그것이 **전달 참조의 밑바탕**이다([목록의 **09번 주제**](../09-rvalue-references-move-and-forward/)).
...
- **참조가 어떤 식에 묶이는지**(lvalue·xvalue·prvalue)의 정본은 [목록의 **08번 주제**](../08-value-categories-lvalue-prvalue-xvalue/)(값 범주)다.\
