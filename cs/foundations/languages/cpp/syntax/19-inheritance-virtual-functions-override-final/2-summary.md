...
> 「가상 소멸자와 다형적 삭제의 전모」는 목록의 **20번**, 「추상 클래스·vtable 비용」은 **21번**,\
...
- ★★★ **`-O2` 어셈블리 세기를 「부적용」으로 둔 이유를 분명히 한다.**\
  ...
  **인라인 불가·분기 예측·캐시**가 전부 빠져 근거가 안 선다.\
  ★ **그 논의의 정본은 목록의 21번 주제**이고, 거기서 재야 한다. **「안 쟀다」가 아니라 「여기서는 잴 것이 아니다」다.**
...
- ★ **전모는 목록의 21번 주제**다. 여기서는 **에러 두 개까지**만 본다.
...
- ★ **전모는 목록의 20번 주제**다. 여기서는 **계수와 경고 조건까지**다.
...
- ★★★ **비용은 재지 않았다.** 이 절이 보인 것은 **구조**이지 시간이 아니다 — 정본은 목록의 **21번 주제**다.
...
- ★ **「가상 호출이 비싸니 피하라」는 이 문서의 조언이 아니다** — **비용은 재지 않았다.**\
  ★ 고르는 기준은 「**기반 포인터로 쓸 것인가**」이지 성능이 아니다. 비용 논의의 정본은 목록의 **21번 주제**다.
...
- 목록의 **20번 주제** — **가상 소멸자와 다형적 삭제의 전모.** (8)은 **계수와 경고 조건까지**다.
- 목록의 **21번 주제** — **추상 클래스·vtable 비용.** ★ **비용은 거기서 재야 한다** — 이 문서는 **구조만** 봤다.
- 목록의 **22번 주제** — 연산자 오버로딩. 멤버/비멤버 선택이 (4)의 이름 조회와 같은 축이다.
...
- **`final` 이 여는 최적화** — 정적 타입이 `final` 이면 가상 호출을 직접 호출로 바꿀 수 있다. ★ **이 문서는 그것을 재지 않았다** — 목록의 **21번 주제**다.
