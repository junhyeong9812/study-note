...
- ★ 「이동 후 상태」가 **계약으로서** 무엇인지의 정본은 [목록의 **17번 주제**](../17-move-constructor-assignment-and-moved-from-state/)다.
...
- ★★★ **`[2]` 에서 `copy(2)` 가 찍혔다.** 1번에서 본 대로 `std::move(c)` 의 타입이 **`const Noisy&&`** 이고,\
  ...
  **복사 생성자(`const Noisy&`)가 유일한 후보**가 된다.\
  ★ 오버로드 표는 [목록의 **08번 주제**](../08-value-categories-lvalue-prvalue-xvalue/)에 있다(`f(std::move(ci))` 줄).
...
- 「어떤 식이 xvalue 인가」는 [목록의 **08번 주제**](../08-value-categories-lvalue-prvalue-xvalue/)가 정본이다.\
  ★ 09 는 **그 범주를 만드는 법**을 줄 뿐이고, 1번의 타입 표가 08 의 격자와 정확히 겹친다.
- 「이동 생성자를 어떻게 구현하나」와 「유효하되 미지정」이라는 계약은 [목록의 **17번 주제**](../17-move-constructor-assignment-and-moved-from-state/)다.\
  ★ 여기서는 **부르는 쪽**만 봤다 — 4번의 표가 「계약이 무엇을 보장하나」의 실측이다.
- `noexcept` 가 **계약으로서** 무엇을 뜻하는지는 목록의 **53번 주제**다. 5번은 **`vector` 한 자리의 결과**만 봤다.
- 「그래서 매개변수를 무엇으로 받나」는 [목록의 **11번 주제**](../11-choosing-parameter-passing/)다 — 08 → 09 → 11 사슬의 끝이다.
