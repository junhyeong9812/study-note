...
- **`g(42)` 가 `g(int&)` 로 못 가는 이유** — `42` 는 이름 없는 임시(prvalue)이고\
  **비const lvalue 참조에는 묶이지 않는다.** `const int&` 는 묶인다.\
  값 범주가 참조 오버로드를 가르는 규칙의 정본은 목록의 **08번 주제**다.
...
- 그래서 이 규칙이 말하는 것은 「**해석은 인자만 보고 한다**」이다.\
  ...
  그것은 오버로드 해석이 아니라 **변환 함수 선택**이다(목록의 **24번 주제**).
...
- **이 주제의 선택은 컴파일 시간**에 끝난다. 실행 중에 고르는 것은 **가상 함수 디스패치**이고\
  정본은 목록의 **19번 주제**다. 둘은 **같은 「고르기」라는 낱말을 쓰는 다른 기계**다.
- **정수 승격의 정본**은 C 갈래의\
...
- **네임스페이스 이름 탐색과 ADL** 은 목록의 **06번 주제**다.\
...
- **안 돌려 본 것** — 템플릿이 후보에 섞였을 때의 순위(목록의 **31번 주제**) ·\
  ADL 이 후보를 늘리는 것(목록의 **06번 주제**) · `=delete` 로 후보를 죽이는 것 ·\
