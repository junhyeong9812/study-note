...
- **비트 플래그** — `|`·`&` 가 자연스러워야 하는 자리. (범위 있는 열거형으로 하려면 연산자를 직접 오버로드해야 한다 — [목록의 **22번 주제**](../22-operator-overloading/).)
...
- [목록의 **22번 주제**](../22-operator-overloading/)(연산자 오버로딩) — 범위 있는 열거형으로 **비트 플래그**를 하려면 거기가 필요하다.
...
- **비트 플래그를 범위 있는 열거형으로** — `operator|`·`operator&` 를 직접 오버로드하거나\
  `std::underlying_type_t` 로 감싸는 래퍼를 만든다. 정본은 [목록의 **22번 주제**](../22-operator-overloading/).
