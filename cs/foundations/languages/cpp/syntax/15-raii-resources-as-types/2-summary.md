...
- ★★ **컴파일은 아무 말도 안 했다** — `cc exit=0`, 경고 0건. **소멸자를 적었는데 복사를 안 막은 것**이\
  **0/3/5의 법칙**([목록의 **18번 주제**](../18-rule-of-zero-three-five-default-delete/))이 말하는 바로 그 자리다.
...
- ★ **`shared_ptr` 이 두 배인 것은 제어 블록 포인터 때문**이다 — 정본은 [목록의 **27번 주제**](../27-shared-ptr-and-reference-counting/).
...
- ★ **복사를 안 막으면 이중 해제**다((4)). 소멸자를 적는 순간 **복사·이동도 같이 결정해야 한다**([목록의 **18번 주제**](../18-rule-of-zero-three-five-default-delete/)).
...
| 상황 | 고르는 것 | 왜 |
|---|---|---|
| 메모리 하나 | **`unique_ptr`** | 직접 만들 이유가 없다 — 정본은 [목록의 **26번 주제**](../26-unique-ptr-and-ownership-transfer/) |
...
| 소유권을 나눈다 | **`shared_ptr`** | 대신 제어 블록 8바이트 — [목록의 **27번 주제**](../27-shared-ptr-and-reference-counting/) |
