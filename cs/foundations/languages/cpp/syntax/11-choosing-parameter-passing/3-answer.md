...
- ★★ **prvalue 를 값으로 받으면 임시가 곧 매개변수가 된다** — 옮길 것이 애초에 없다.\
  C++17 의 **물질화** 규칙이고, 정본은 목록의 **08번 주제**다.
...
- ★★★ **`by_cref(Probe(2))` 가 copy 1** 이다. **훔쳐도 되는 임시를 받아 놓고 복사했다** —\
  ...
  ★ 「`const` 를 `move` 해도 복사가 된다」와 **같은 자리**이고, 정본은 목록의 **09번 주제**다.
...
- ★★ **`by_pair` 네 줄과 `by_fwd` 네 줄이 한 칸도 다르지 않다.**\
  ...
  **전달 참조 하나가 오버로드 두 개와 같은 결과**를 낸다(정본은 목록의 **09번 주제**).
- ★ **`by_rref` 행이 `std::move(lv)`** 인 이유 — `by_rref(lv)` 는 **컴파일되지 않는다.**\
  `T&&` 는 lvalue 를 못 받는다(묶기 격자의 정본은 목록의 **08번 주제**).
...
- ★★★ **이 프로그램은 ill-formed 가 아니다.** 표준상 **적법한 프로그램**이고,\
  ...
  ★ 「`cc exit=0` 인데 ill-formed」인 정본 사례는 목록의 **10번 주제**에 있다.
...
- **lvalue·prvalue·xvalue** 의 정본 — 목록의 **08번 주제**.\
  1번의 `read_value(Probe(2))` 가 copy 0 인 이유(물질화)가 거기 있다.
- **`std::move` 가 아무것도 옮기지 않는다**는 것의 정본 — 목록의 **09번 주제**.\
  2번의 `by_rref` 가 move 1 인 것은 **본문에 `std::move` 가 한 번 더** 있기 때문이다.
- **`std::forward` 와 전달 참조** 의 정본 — 목록의 **09번 주제**.\
...
- **`const` 를 어디에 붙이나** — 목록의 **10번 주제**. ★ 값 매개변수의 `const` 는 **시그니처의 일부가 아니다.**
