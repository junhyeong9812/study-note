# python/syntax/36-dataclasses — 정답
...
> ★★★ **트레이스백이 한 블록도 없다.** 던진 예외가 전부 `dataclasses.py` 를 지나
> **절대 경로가 박히기** 때문에 전부 `except` 로 받아 **타입과 메시지만** 찍었다.
> ★ **`MISSING` 의 기본 `repr` 에도 주소가 박히므로 이름으로 바꿔 찍었다**(11번 답).
> 그래서 이 파일에는 **주소가 한 곳도 없다.**

## 정답

### 1. 아홉 개 · `(self, x: int, y: int = 0, tags: list = <factory>) -> None`
...
* ★ **①에서 새로 생긴 이름이 아홉**이다. 던더가 아닌 것이 **`y` 하나** 섞여 있는데,
  그것은 `@dataclass` 가 만든 것이 아니라 **내가 `y: int = 0` 으로 대입한 값이 클래스 변수로 남은 것**이다.
  ★ `x` 와 `tags` 는 기본값을 안 줬거나 `field()` 로 줬으므로 안 남는다.
  ★ `__match_args__` 는 `match` 문이 쓰는 것이고 정본은 [목록의 **39번 주제**](../39-match-statement/) 다.
* ★★★ **②의 서명이 이 문항의 과녁이다** —
  `(self, x: int, y: int = 0, tags: list = <factory>) -> None`.
  ★ **`<factory>` 가 실제 기본값이 아니다.** 「만드는 법이 등록됐다」는 **표시**이고,
  이 표기 자체는 `inspect` 와 `dataclasses` 가 합의한 CPython 쪽 것이다.
  ★ `inspect.signature(Point)` 는 `self` 를 뺀 **부르는 쪽 서명**을 준다 — 둘을 혼동하기 쉽다.
* ★ **⑤에서 `p == (1, 2)` 가 거짓**이다. 생성된 `__eq__` 는 **`other.__class__` 가 같을 때만** 비교하고
  아니면 `NotImplemented` 를 준다 — [31번](../31-comparison-protocol-and-sortability/2-summary.md)의 반사 연산 규칙을 탄다.
  ★ **`Point.__hash__` 가 `None`** 이다. [30번](../30-repr-eq-hash-contracts/2-summary.md)이 정본이고
  거기 네 옵션 표가 그 이유를 전부 적었다 — 여기서는 **결론만 확인**했다.
* ★★ **⑥에서 `Point3` 의 필드가 `['x', 'y', 'tags', 'z']`** 다. **부모 필드가 앞**이다.
  ★ 생성기가 **MRO 를 거꾸로 훑어** 모으기 때문이고, 그 결과의 함정이 7번 답이다.

### 2. 셋 다 `class` 문에서 `ValueError` — 그런데 내 가변 클래스는 통과하고 샌다
...
* ★★ **①의 셋이 전부 `ValueError` 이고 `class` 문에서 난다.** 인스턴스를 만들지도 않았다.
  ★ 문구가 **고치는 법까지 준다** — `… is not allowed: use default_factory`.
* ★ **③·④의 두 수치가 `default_factory` 의 정확한 뜻이다.**
  세 번 만들면 **3**, 인자를 주면 **0**. 「기본값」이 아니라 「**기본값을 만드는 법**」이라는 말이 이 두 수치다.
* ★★★ **⑤가 이 문항의 과녁이다.** `MyMutable` 은 **통과**한다.
  `hash(MyMutable())` 이 되기 때문이다 — **판별 기준**이 「가변인가」가 아니라 「**해시할 수 있나**」다.
  ★ 그 결과 `s1.m is s2.m` 이 참이고 `s2.m.items` 가 **`['샜다']`** 다.
  **[20번](../20-mutable-default-args/2-summary.md)의 함정이 그대로 재현된다.**
  ★★ 문서 자신이 그렇게 적는다 — *"Unhashability is used to approximate mutability."* 와
  *"This is a partial solution, but it does protect against many common errors."*
  ★ **[20번](../20-mutable-default-args/2-summary.md)이 「이 방어는 부분적」이라 적어 둔 자리를 여기서 던져 확인한 것**이다.
* ★ **⑦ — 튜플은 통과**하고 `is` 가 참이다. **불변이라 공유해도 안전**하므로 맞는 설계다.

★★★ **판정 한 줄** — **「`dataclass` 를 쓰면 이 함정이 막힌다」는 틀린 문장이다. 흔한 셋만 막는다.**

### 3. 넷이 참이 된다 — `compare=False` 때문이고, `init=False` 는 `repr` 까지 터뜨린다
...
* ★ **①의 여덟 줄 중 `==` 가 참인 것은 넷**이고 전부 `compare=False` 인 줄이다.
  `init`·`repr` 은 `==` 를 안 건드린다 — **세 스위치가 서로 독립**이다.
* ★★★ **`compare=False` 가 가장 조용하다.** `b` 가 `2` 와 `99` 로 다른데 `==` 가 참이다.
  ★ **예외도 경고도 없다.** 의도한 것이면 맞는 도구지만(캐시·타임스탬프),
  모르고 켜면 **`set`·`dict` 에서 값이 겹쳐 하나가 사라진다** — [30번](../30-repr-eq-hash-contracts/2-summary.md)의 그 집안이다.
* ★★ **③이 `init=False` 의 함정이다.** 기본값도 없으면 **속성 자체가 안 생긴다** —
  ...
  ★ **객체는 만들어졌는데 찍을 수조차 없는 상태**다 — 디버거에서 그 객체를 보는 순간 터진다.
  고치는 법은 `default` 를 같이 주거나 `__post_init__` 에서 채우는 것이다.
* ★ **④ — `Field` 가 들고 있는 칸이 열하나**다. 세 스위치 말고 `hash`·`metadata`·`kw_only` 가 있고
  `_field_type` 은 **내부 이름**이다(6번 답에서 쓴다).

### 4. `__lt__`·`__le__`·`__gt__`·`__ge__` — 그리고 모양이 같아도 클래스가 다르면 막힌다
...
* ★ **①에서 넷이 다 클래스 칸에 있다.**
  ★★ [31번](../31-comparison-protocol-and-sortability/2-summary.md)이 정본인 `functools.total_ordering` 은
  **`__lt__` 를 내가 줘야 하고 셋만 채운다.** 이쪽은 **필드 목록만 보고 넷을 다 만든다** — **적는 양이 다르다.**
  ★ 그쪽이 **`__hash__`·`__ne__`·`__eq__` 를 못 채운다**는 것도 정본이 거기라 여기서는 인용만 한다.
* ★ **`max(rows)` 가 돈다.** [31번](../31-comparison-protocol-and-sortability/2-summary.md)의 결론대로 **`max` 는 `__gt__` 를 부르는데**,
  생성기가 넷을 다 만들었으므로 그 경로가 막히지 않는다.
  ★ `total_ordering` 으로 만든 클래스였다면 채워진 `__gt__` 가 도는 것이고, **어느 쪽이든 돈다.**
* ★★★ **③이 이 문항의 과녁이다.** 필드 이름·순서·타입이 **똑같은 `Other`** 와도 `TypeError` 다.
  ★ 생성된 `__lt__` 가 **`other.__class__ is self.__class__` 를 보기** 때문이다 —
  **구조가 아니라 이름(정체)이 기준**이다.
  ★★ [35번](../35-abc-and-protocol/2-summary.md) 의 **구조적 판정과 정반대 축**이라는 것이 여기서 드러난다.
  같은 저장소 안에서 두 철학이 나란히 있는 셈이다.
* ★ **④가 그것을 코드로 확인한다.** `co_names` 에
  `('__class__', 'major', 'minor', 'tag', 'NotImplemented')` 가 들어 있다 —
  **무엇을 보는지 함수 자신이 들고 있다.**
  ★ 생성기가 **소스를 만들어 컴파일하기** 때문에 이런 창이 열린다.
* **⑤ — `compare=False` 는 그 튜플에서도 빠진다.** `==` 와 `<` 가 같은 목록을 쓴다.
* **⑥ — `eq must be true if order is true`** 는 [30번](../30-repr-eq-hash-contracts/2-summary.md)이 이미 실측한 것이고
  **같은 값이 나오는지만** 확인했다.

### 5. `__setattr__`·`__delattr__` 둘 · 선언 안 한 이름도 막힘 · 리스트 안은 안 막힘
...
* ★★ **①에서 `frozen` 쪽에만 `__setattr__`·`__delattr__` 이 있다.**
  `Locked.__setattr__.__qualname__` 이 `Locked.__setattr__` 로 나온다 — **생성기가 만든 함수**다.
  ★ **별도 불변 장치가 아니라 [33번](../33-property-descriptor-slots/2-summary.md)의 「클래스 칸에 무엇을 앉히나」 그대로**다.
* ★ **② — 선언 안 한 `lk.newone = 1` 도 막힌다.**
  `__setattr__` 은 **이름을 안 가리고 무조건** 던지기 때문이다.
  ★ 오타로 새 속성을 만드는 것도 못 한다 — [33번](../33-property-descriptor-slots/2-summary.md)의 `__slots__` 와 **같은 부수 효과인데 다른 기전**이다.
* **③ — `FrozenInstanceError` 가 `AttributeError` 의 하위**다.
* ★★★ **④가 가장 흔한 오해다.** `s.xs.append('들어갔다')` 가 **된다.**
  ★ **막는 것은 「이름을 다시 묶는 것」뿐**이고 **담긴 객체를 고치는 것**은 아니다.
  [03번](../03-mutability-and-copying/2-summary.md)의 「고치기와 새로 묶기」가 그대로 걸린다 — **`frozen` 은 얕다.**
* ★ **⑤ — 해시가 되살아난다.** `Open.__hash__` 가 `None` 인데 `Locked` 는 만들어져 있고,
  같은 값 둘이 **키 하나**가 된다. **[30번](../30-repr-eq-hash-contracts/2-summary.md)의 표를 여기서 다시 만들지 않고 결론만 확인**했다.
* ★★ **⑥이 실무 관용구다.** `frozen` 에서 파생값을 채우려면 **`object.__setattr__`** 로 갈아끼운 것을 건너뛴다.
  ★ [29번](../29-classes-and-attribute-lookup/2-summary.md)이 `__getattribute__` 재귀를 피할 때 `object.__getattribute__` 를 쓴 것과 **같은 수법**이다.

### 6. 서명에 있고 · `fields()` 에 없고 · 덤프에 있다 — 그리고 `__post_init__` 이 없어도 안 터진다
...
* ★★★ **②가 이 문항의 과녁이다.** 세 창이 **세 답**을 준다 —
  서명에는 `db_key`·`salt` 가 **있고**(`dataclasses.InitVar[str]` 으로 찍힌다),
  `fields()` 에는 **없고**(`['name', 'token']`),
  `__dataclass_fields__` 에는 **있다**(`['name', 'db_key', 'salt', 'token']`).
...
* ★★ **④가 조용한 자리다.** `InitVar` 가 있는데 `__post_init__` 이 없으면 **예외가 안 난다.**
  ...
  ★ **문법은 맞는데 내가 넘긴 값이 사라진다** — 예외도 경고도 없다.
* ★★ **⑦이 이 주제의 마지막 조용한 자리다.** `n: int` 에 **문자열**이,
  `xs: list` 에 **실수**가 그냥 들어간다.
  `fields()` 는 `int`·`list` 라고 적어 두었는데 **실제로 들어간 것은 `str`·`float`** 다.
  ★★ **`dataclasses` 는 어노테이션을 읽기는 하는데 값이 그 타입인지는 안 본다** — 두 일이 다르다.
  ★ [35번](../35-abc-and-protocol/2-summary.md) 과 같은 축이다 — **런타임이 안 보는 자리**다.

### 7. `non-default argument 'b' follows default argument` — 파이썬 함수 규칙의 문구다
...
* ★★ **①의 문구는 `dataclasses` 가 만든 것이 아니다.**
  `def f(a=0, b)` 가 애초에 **파이썬 함수 정의의 문법이 아니고**,
  생성기가 `__init__` 을 **소스로 만들어 컴파일하기** 때문에 그 규칙에 그대로 걸린다.
  ★ 필드가 **「부모 먼저」로 줄을 서서**(1번 답의 ⑥) `(a=0, b)` 가 된 것이 원인이다.
* ★★★ **③이 3.10 의 해법이다.** 서명이 `(self, *, a: int = 0, b: int)` 가 된다.
  ★ **`*` 가 하나 더 들어갔고**, 그 뒤에는 **순서 제약이 없다** — 키워드로만 받기 때문이다.
  ★ 대신 `KwSub(1)` 이 `takes 1 positional argument but 2 were given` 이다 — **위치 인자를 못 쓴다.**
...
* ★★ **⑥이 「필드」와 「속성」을 가른다.** `fields(FromPlain)` 이 `['a']` 뿐인데
  `FromPlain().z` 는 `1` 이다.
  ★ `PlainBase` 가 `dataclass` 가 아니므로 **생성기가 그 속성을 안 모았고**,
  그런데 **보통 상속으로는 보인다** — [34번](../34-inheritance-mro-super/2-summary.md)의 MRO 로 올라가 찾는다.
  **두 규칙이 겹쳐 도는 자리**다.

### 8. 거짓 — `slots=True` 는 새 클래스를 돌려준다
...
* ★★★ **①이 이 문항의 과녁이다.** `@capture` 로 **`@dataclass` 가 받기 직전의 클래스**를 붙잡아 두니
  `Slotted is BEFORE["Slotted"]` 가 **거짓**이고, 같은 방식으로 붙잡은 `Normal` 은 **참**이다.
  ★ **붙잡아 둔 쪽에는 `__slots__` 가 없고 돌려받은 쪽에만 있다** — 원본은 그대로 두고 **복제**한 것이다.
* ★★ **평소에 안 보이는 이유**는 [24번](../24-decorators/2-summary.md)의 규칙 때문이다 —
  `class` 문이 만든 이름에 **데코레이터의 결과가 다시 묶이므로** 겉보기로는 같아 보인다.
  ★ **창을 바꿔야 보이는 자리**다. `__slots__` 유무만 물으면 안 보이고 `is` 로 물어야 보인다.
* ★ **② — 함수로 부르면 더 분명하다.** `dataclass(slots=True)(raw) is raw` 가 거짓이고
  `raw` 에는 `__slots__` 가 **안 생겼다.**
  ★★ **실무의 함정** — 데코레이터 앞뒤로 같은 클래스 객체를 들고 있다고 믿으면
  등록이나 `isinstance` 검사가 **엉뚱한 클래스를 가리킨다.**
...
* ★ **④가 [33번](../33-property-descriptor-slots/2-summary.md)과 이어진다.** 클래스 칸의 `a` 가 **`member_descriptor`** 다 —
  그 편이 실측한 것과 **같은 물건**이다.
* ★★ **⑤는 [33번](../33-property-descriptor-slots/2-summary.md)과 같은 규칙을 따른다** — **크기는 재고 속도는 안 잰다.**
  근거로 쓰는 것은 **「어느 쪽이 작나」** 한 줄뿐이다.

### 9. 아니다 — 그리고 「읽는다」와 「값을 본다」는 다른 말이다

**왜 그런가**

* ★ **어노테이션이 있어야 필드다.** `x = 1` 만 쓰면 그냥 클래스 변수이고
  `__init__` 에도 `repr` 에도 `==` 에도 안 들어간다.
  ★ 거꾸로 `x: int` 만 쓰고 값을 안 줘도 **필드가 된다**(기본값 없는 필드).
* ★★ **「읽는다」와 「값이 그 타입인지 본다」는 다른 말이다**(6번 답의 ⑦).
  `dataclasses` 는 어노테이션을 **읽어서** `Field.type` 에 넣고 서명에 찍는다 —
  **거기까지만** 한다. `Typed("정수가 아니다", 3.14)` 가 그냥 만들어진다.
* ★★ **이 구분이 중요한 이유** — [목록의 **40번 주제**](../40-type-hints-at-runtime/) 가 「힌트는 실행을 안 바꾼다」의 정본인데,
  **`dataclasses` 는 그 예외처럼 보인다.** 실제로는 **절반만 예외**다 —
  **구조를 만드는 데는 쓰고 값을 검사하는 데는 안 쓴다.**
* ★ 그래서 검증은 **`__post_init__` 에 내가 쓰는 것**뿐이다(6번 답의 ⑥).

### 10. 「해시할 수 있나」이고, 문서가 스스로 "partial solution" 이라 부른다

**왜 그런가**

* ★★★ **판별 기준**은 「가변인가」가 아니라 「**해시할 수 있나**」다.
  문서의 낱말이 그렇다 — *"will raise a ValueError if it detects an **unhashable** default parameter.
  The assumption is that if a value is unhashable, it is mutable."*
  그리고 한 줄 더 — *"**Unhashability is used to approximate mutability.**"*
  ★ **`approximate`** 라는 낱말이 핵심이다. **근사이지 판정이 아니다.**
* ★ **문서가 스스로 부르는 이름** — *"This is a **partial solution**, but it does protect against many common errors."*
* ★★ **그래서 조용히 새는 것** — **해시 가능한 가변 객체**다.
  `__eq__` 를 재정의하지 않은 내 클래스는 **`object.__hash__` 를 물려받아 해시가 되므로**
  가변이어도 그냥 통과한다(2번 답의 ⑤).
  ★ 실무에서 흔한 모양이 **설정 객체·빈 컬렉션 래퍼**다.
* ★ [20번](../20-mutable-default-args/2-summary.md)이 그 판별 기준이 **3.11 에서 바뀌었다**고 적어 두었다 —
  그 전에는 `list`·`dict`·`set` 이라는 **목록**이었고 지금은 **해시 가능성**이다.
  ★★ **목록에서 근사로 바뀐 것이라 넓어졌지만 여전히 반쪽**이다.

### 11. 넷은 계약, 새 클래스는 구현 — `MISSING` 의 `repr` 에는 주소가 박힌다

**왜 그런가**

| 사실 | 층 | 근거 |
|---|---|---|
| `order=True` 가 **넷을 만든다** | **모듈 계약** | `dataclasses` 문서 |
| 비교가 **같은 클래스일 때만** 되는 것 | **모듈 계약** | 문서 |
| 가변 기본값을 `ValueError` 로 거부하는 것 | **모듈 계약** | 문서 |
| ★★ 그 방어가 **부분적**인 것 | **모듈 계약** | 문서가 *"partial solution"* 이라 적는다 |
| `frozen` 이 `__setattr__`·`__delattr__` 을 더하는 것 | **모듈 계약** | 문서 |
| `InitVar` 가 필드가 아닌 것 | **모듈 계약** | 문서 |
| `kw_only`·`slots` 가 **3.10** 신설인 것 | **모듈 계약** | 문서의 *Changed in version 3.10* |
| ★★ **`slots=True` 가 새 클래스를 돌려주는 것** | ★ **CPython 구현** | ★ **문서는 「`__slots__` 를 더한다」까지만 적는다.** `is` 로 직접 확인했다 |
| `<factory>` 라는 서명 표기 | **CPython 구현** | 실행 |
| `_FIELD`·`_FIELD_INITVAR` 라는 이름 | **CPython 구현** | `_` 로 시작한다 |
| `co_names` 로 생성된 함수를 들여다볼 수 있는 것 | **CPython 구현** | 소스를 만들어 컴파일하기 때문 |
| 예외 **문구** 전부 | **이 판의 관찰** | 실행 |

* ★★★ **`MISSING` 의 기본 `repr` 을 안 찍은 이유** — 그 객체는 `__repr__` 을 따로 안 가져서
  **`<dataclasses._MISSING_TYPE object at 0x…>` 처럼 주소가 박힌다.**
  ★ 주소는 **머리말의 「흔들리는 칸」 표에 있는 칸**이고, 실으면 재대조에서 매번 불일치가 난다.
  ★ 그래서 **이름으로 바꿔 찍었다**(`show()` 함수). **정규화 규칙을 늘리는 대신 안 흔들리게 만든 것**이다.
* ★ 같은 규칙으로 **`getsizeof` 의 바이트 수**도 근거로 안 썼다 —
  [33번](../33-property-descriptor-slots/2-summary.md)과 같이 **「어느 쪽이 작나」만** 쓴다.

### 12. 다섯이 각각 대 주는 것 · 빼는 법이 정반대 · 절반만 예외
...
| 주제 | 이 주제에 무엇을 대 주나 |
|---|---|
| [20번](../20-mutable-default-args/2-summary.md) | **가변 기본값의 구조**와 `None` 센티널·`default_factory` 가 **같은 해법**이라는 것 |
| [30번](../30-repr-eq-hash-contracts/2-summary.md) | **`__hash__` 가 옵션 넷에 따라 갈리는 표** — 여기서는 결론만 인용했다 |
| [31번](../31-comparison-protocol-and-sortability/2-summary.md) | `total_ordering` 이 **셋만 채운다**는 것 — `order=True` 의 **넷**과 대비된다 |
| [33번](../33-property-descriptor-slots/2-summary.md) | `__slots__` 가 만드는 **`member_descriptor`** 와 **크기는 재고 속도는 안 잰다**는 규칙 |
| [34번](../34-inheritance-mro-super/2-summary.md) | 필드가 **MRO 를 거꾸로 훑어** 모인다는 것 — 「부모 먼저」의 근거 |

* ★★ **코틀린과는 「빼는 법」이 정반대다.**
  Kotlin 갈래 목록([`kotlin/syntax/README.md`](../../../kotlin/syntax/README.md))의 **22번** 이 적은 대로,
  코틀린은 **본문으로 옮기면 암묵적으로 빠지고** `copy` 가 그것을 **초기값으로 되돌린다.**
  ★ 파이썬은 **`field(compare=False)` 로 명시해야** 빠진다.
  ★★ **파이썬 쪽은 그 사고가 안 난다** — 어노테이션을 붙이면 **반드시 필드**이기 때문이다.
  대신 **`compare=False` 를 켜면 같은 조용한 상태가 된다**(3번 답). **함정의 위치가 옮겨 갔을 뿐이다.**
* ★★ **[목록의 40번 주제](../40-type-hints-at-runtime/) 의 「힌트는 실행을 안 바꾼다」에 이 주제는 절반만 예외**다 —
  `dataclasses` 는 어노테이션을 **읽어서 구조를 만들지만**(그러므로 예외),
  **값이 그 타입인지는 안 본다**(그러므로 예외가 아니다).
  ★ 9번 답의 구분이 그것이다.
...
| 무엇을 | 어떻게 | 몇 번 | 결과 |
|---|---|---|---|
| 생성된 것 · 서명 · 필드 덤프 | `python3 - <e36_generated.py` | 2(캡처 + 재대조) | 새 이름 9개 · `<factory>` 표기 · 부모 필드가 앞 |
| 가변 기본값 세 타입 + 반쪽 방어 | `python3 - <e36_mutable.py` | 2 | 셋 다 `ValueError` · **`MyMutable` 은 통과해 샘** |
| `init`·`repr`·`compare` 전수 격자 | `python3 - <e36_grid.py` | 2 | 8줄 · `compare=False` 4줄이 `==` 참 · `repr` 까지 터짐 |
| `order=True` 가 만드는 넷 | `python3 - <e36_order.py` | 2 | 넷 생성 · 모양이 같은 다른 클래스와도 `TypeError` |
| `frozen` 의 정체와 한계 | `python3 - <e36_frozen.py` | 2 | `__setattr__` 교체 · 선언 안 한 이름도 막힘 · **속은 안 막힘** |
| `__post_init__`·`InitVar`·타입 미검사 | `python3 - <e36_postinit.py` | 2 | **세 창이 세 답** · 훅 없으면 값이 조용히 버려짐 |
| 상속과 `kw_only` | `python3 - <e36_inherit.py` | 2 | `TypeError` · `*` 로 제약 사라짐 · 「필드」와 「속성」이 갈림 |
| `slots=True` 가 새 클래스인 것 | `python3 - <e36_slots.py` | 2 | `is` 가 **거짓** · 원본에는 `__slots__` 없음 |
| 판 + `kw_only`/`slots` 유무 | `python3 - <e36_version.py` | 2 | 3.12.3 · 둘 다 받는다 |

**구현 의존 항목** — 판이 오르면 다시 돌려야 하는 것.

| 항목 | 왜 |
|---|---|
| ★★ **`slots=True` 가 새 클래스인 것** | 문서는 「`__slots__` 를 더한다」까지만 적는다 |
| `<factory>` 라는 서명 표기 | `inspect` 와 `dataclasses` 의 합의다 |
| `_FIELD`·`_FIELD_INITVAR` 라는 이름 | 내부 이름이다 |
| `Field.__slots__` 가 열한 칸인 것 | 판마다 늘어난다 |
| `co_names` 로 생성 함수를 읽을 수 있는 것 | 소스를 만들어 컴파일하는 구현이라서다 |
| `getsizeof` 의 **바이트 수** | ★ 판·빌드가 바꾼다. **대소 관계만** 근거다 |
| 예외 **문구** 여섯 전부 | 종류는 계약, 문구는 아니다 |
| ★ `MISSING` 의 기본 `repr` 에 **주소가 박히는 것** | 그래서 **이름으로 바꿔 찍었다** |

★ **안 흔들리는 칸** — `__dataclass_fields__` 와 `fields()` 의 **순서**(`dict` 라 보장된다) ·
어느 던더가 **클래스 칸에 생겼나 안 생겼나** · `inspect.signature` 가 만든 **서명 문자열** ·
`is` 의 참·거짓 · 예외 **종류** · `(exit N)` · 크기의 **대소 관계**.
★★ **이 주제가 한 번도 안 잰 것** — **속도.** [33번](../33-property-descriptor-slots/2-summary.md)과 같은 규칙이다.
