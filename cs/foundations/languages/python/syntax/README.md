...
|---|------|------|----------------------|------|-----------|------|
| 01 | [객체와 이름 바인딩 모델](01-object-and-name-binding/) | 문법 | 대입이 값을 복사하지 않고 이름을 객체에 묶는다는 것을 그림으로 설명하고, 두 이름이 한 객체를 가리킬 때 한쪽 변경이 다른 쪽에 보이는지 예측할 수 있다 | — | `cs/foundations/variables-and-memory/` | A |
| 02 | [`is` 대 `==` 와 인터닝](02-is-vs-eq-interning/) | 관용구 | 두 비교가 갈라지는 지점을 설명하고, 작은 정수·짧은 문자열에서 우연히 같아 보이는 이유와 `None` 비교에 무엇을 써야 하는지 판단할 수 있다 | 01 | — | A |
