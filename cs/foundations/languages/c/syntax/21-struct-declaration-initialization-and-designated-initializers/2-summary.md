...
> 「복합 리터럴」의 정본은 목록의 **27번 주제**다 — 여기서는 **저장 기간만** 본다.
...
- **초기자가 아예 없으면** 자동 저장 기간 객체는 **불확정**이다(목록의 **30번 주제**).
...
- 목록의 **25번 주제** (불완전 타입과 opaque struct) — 헤더에 `struct S;` 만 노출하는 쪽
- 목록의 **26번 주제** (유연 배열 멤버) — 마지막 멤버가 `[]` 인 구조체
- 목록의 **27번 주제** (복합 리터럴) — ★ **복합 리터럴의 정본.** 여기는 **저장 기간만** 봤다
- 목록의 **30번 주제** (초기화 규칙과 불확정 값) — ★ **초기자를 아예 안 줬을 때**가 거기다
...
- **`static` 구조체의 초기화** — 정적 저장 기간이면 초기자가 없어도 **전부 0** 이다.\
  ★ 자동 저장 기간과 **여기서 갈린다**(목록의 **28번 주제**·**30번 주제**).
