...
- ★★ **자리 ③ 문자열 리터럴로 배열을 초기화할 때** — `char s[] = "hi"` 는 리터럴이 **복사**된다.\
  ...
  ★ **`q` 가 가리키는 것을 고치면 UB** 다 — [목록의 **20번 주제**](../20-null-terminated-strings-and-string-literals/)가 정본이고 **이 문서는 던지지 않았다.**
...
★ **이 문서는 VLA 매개변수를 던지지 않았다**([목록의 **18번 주제**](../18-variable-length-arrays-vla/)).
...
- **`s` 는 고칠 수 있고**(`Hi` 가 찍혔다) **`q` 가 가리키는 것을 고치면 UB** 다([목록의 **20번 주제**](../20-null-terminated-strings-and-string-literals/)).
...
- [목록의 **17번 주제**](../17-multidimensional-arrays-and-pointer-types/) (다차원 배열과 그 포인터 타입) — `int a[3][4]` 의 메모리 배치를 **자세히**
- [목록의 **18번 주제**](../18-variable-length-arrays-vla/) (VLA) — 열 수가 실행 시간에 정해지는 매개변수
- [목록의 **20번 주제**](../20-null-terminated-strings-and-string-literals/) (널 종단 문자열과 문자열 리터럴) — ★ **`char *q = "hi"` 를 고치면 UB 인 것의 정본**
...
- **VLA 매개변수**(`void f(int n, int a[n])`·`int (*a)[n]`)는 열 수가 실행 시간에 정해질 때 쓴다.\
  ★ **이 문서에서 던져 보지 않았다.** [목록의 **18번 주제**](../18-variable-length-arrays-vla/)의 몫이다.

- ★ **같은 내용의 문자열 리터럴을 컴파일러가 공유하는지**(`"hi"` 두 개가 같은 주소인지)는 **미명시**이고\
  ★ **던져 보지 않았다.** 이 문서는 `char s[]` 와 `char *q` 의 **주소 대역이 다르다**는 것까지만 봤다.

- ★ **`char *q = "hi"; q[0] = 'H';`** 를 실제로 던지지 않았다 — [목록의 **20번 주제**](../20-null-terminated-strings-and-string-literals/)의 몫이다.\
