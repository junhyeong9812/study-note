...
- [목록의 **14번 주제**](../14-pointers-address-dereference-and-pointer-types/) (포인터) · [목록의 **19번 주제**](../19-void-pointer-null-pointer-and-null/) (`void *`·`NULL`) — 포인터 자체의 정본
- [목록의 **23번 주제**](../23-union-and-the-boundary-of-type-punning/) (`union` 과 타입 펀닝) — C 에서 `union` 을 통한 펀닝이 허용되는 범위
- [목록의 **31번 주제**](../31-const-and-pointer-const-placement/) (`const` 와 포인터 const 위치) — `const` 가 계약이라는 것의 정본
...
- **`union` 을 통한 타입 펀닝은 C 에서 허용된다**(C++ 과 다르다). 마지막에 쓴 멤버가 아닌 멤버를 읽는 것이\
  C 에서는 「값이 재해석된다」로 정의돼 있다. 이 문서에서는 **던져 보지 않았다** — 정본은 [목록의 **23번 주제**](../23-union-and-the-boundary-of-type-punning/)다.
...
- **`restrict` 는 앨리어싱의 반대 방향**이다 — 「겹치지 않는다」를 프로그래머가 약속하는 것.\
  `memcpy` 의 시그니처에 `restrict` 가 붙어 있고 `memmove` 에는 없다. 정본은 [목록의 **33번 주제**](../33-restrict-and-the-aliasing-contract/).
