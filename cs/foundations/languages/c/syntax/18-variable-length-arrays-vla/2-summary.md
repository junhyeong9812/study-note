...
> **다차원 배열의 안쪽 보폭**은 목록의 **17번 주제** — ★ 그쪽은 **컴파일 시점**, 여기는 **실행 시점**이다. 저장 기간 고르기는 목록의 **28번 주제**, `malloc` 은 목록의 **37번 주제**.
...
- ★★★ **2차원은 다르다** — `sizeof a` 는 8 인데 **`sizeof a[0]` 이 12** 다. **`cols = 3` 이라는 실행 시점 값**이 타입에 들어 있다.\
  ★ **목록의 17번 주제와 같은 12 인데 정해진 시점이 다르다.**
...
- ★★★ 한 줄로 — **VLA 의 저장 기간은 고를 수 있는 것이 아니라 자동으로 고정된다**(고르는 이야기는 목록의 **28번 주제**).
...
- 함수 밖으로 내보내야 하면 **`malloc`**(목록의 **37번 주제**), 구조체에 붙이려면 **유연 배열 멤버**(목록의 **26번 주제**)다.
...
| 전역·구조체 멤버 | 고정 크기 또는 **유연 배열 멤버**(목록의 **26번 주제**) | VLA — ★ 컴파일 에러 |
...
- ★★ **VLA 매개변수는 1차원이면 포인터**(`sizeof a` = 8)**, 2차원이면 안쪽 한 겹이 남는다**(`sizeof a[0]` = **12**).\
  ★ **목록의 17번 주제와 같은 12 인데, 거기서는 컴파일 시점 값이고 여기서는 실행 시점 값**이다.
...
- 목록의 **17번 주제** (다차원 배열) — ★ **안쪽 보폭이 컴파일 시점인 판.** 여기는 **실행 시점인 판**
- 목록의 **26번 주제** (유연 배열 멤버) · 목록의 **28번 주제** (저장 기간 4종) · 목록의 **37번 주제** (`malloc` 계열)
