...
- ★★ **바이트 배치는 아니다.** **같은 소스에서 두 번째 바이트가 달랐다**(`78` ↔ `08`).\
  ★★ 갈리는 축은 **컴파일러만이 아니다** — 여덟 벌로 던지면 `78` 은 **gcc `-O0` 한 칸뿐**이고\
  gcc `-O1`·`-O2`·`-Os` 와 clang 넷은 전부 `08` 이다. **한 컴파일러 안에서도 갈린다.**\
...
- **미사용 비트가 갈렸다**(`D3 78 00 00` ↔ `D3 08 00 00`) — `78` 은 gcc `-O0` 한 칸뿐이고 나머지 일곱 벌은 `08` 이다.
