...
- ★★ **앞 4바이트가 다르다.** **비트필드의 할당 순서·패딩·미사용 비트가 구현에 맡겨져 있다.**\
  ★★★ **다만 「gcc 와 clang 의 차이」로 읽으면 반쪽이다 — 갈리는 축은 최적화 수준까지 포함한다.**\
  같은 프로그램을 여덟 벌로 던지면 **`D3 78` 은 gcc `-O0` 한 칸뿐**이고 `-O1`·`-O2`·`-Os` 와 clang 네 벌은 전부 `D3 08` 이다.\
  **한 컴파일러 안에서도 갈리므로 「미사용 비트에 기대지 마라」가 더 강해진다.**\
...
| 비트필드 (11-i) | `sizeof`=4 · 필드 값 일치 · ★ **미사용 비트가 갈린다** — `D3 78 00 00` 은 **gcc `-O0` 한 칸뿐**이고 나머지 일곱 벌(gcc `-O1`·`-O2`·`-Os` · clang 넷)은 `D3 08 00 00` · 절단 경고 2건 | gcc·clang `-Wall -Wextra` × `-O0`\~`-Os` |
...
- `int d : 4` 가 **부호 있는 것**으로 잡힌 것 · 비트필드의 **미사용 비트**가 `78` ↔ `08` 로 갈리는 것(gcc `-O0` 만 `78`).
