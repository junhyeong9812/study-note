...
비용 — 없다. 이건 정의다. 다만 `DISTINCT` 가 붙으면 중복 제거를 위해 정렬이나 해시가 한 겹 더 들어간다(계획은 [목록의 **59번 주제**](../59-scan-join-sort-operators/)).
...
- **`COUNT(*)` 이 「빠른 연산」이라는 인상은 엔진에 따라 다르다.** PG 는 MVCC 때문에 전체 `COUNT(*)` 에도 가시성 확인이 필요하다.\
  이 주제에서는 **재지 않았다.** 계획을 읽는 법은 [목록의 **58번 주제**](../58-explain-plan-tree/), 스캔 연산자는 [**59번 주제**](../59-scan-join-sort-operators/)다.
