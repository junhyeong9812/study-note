...
팬아웃 자체와 그 처방 전체는 [목록의 **25번 주제**](../25-join-fan-out/)가 정본이다. `EXISTS`/`IN` 은 [목록의 **19번 주제**](../19-semi-anti-join/)다.\
...
대가 — 등호가 아니면 해시 조인을 못 쓴다. 큰 표에서는 중첩 루프가 되어 느려진다([목록의 **59번 주제**](../59-scan-join-sort-operators/)).\
...
FROM 왼쪽표 [INNER] JOIN 오른쪽표 USING (공통열)  -- [목록의 **18번 주제**](../18-using-and-natural-join/)
...
- **`COUNT(*)` 로 「사원 수」를 센다.**\
  조인 뒤의 `COUNT(*)` 은 사원 수가 아니라 **행 수**다. `COUNT(DISTINCT e.id)` 가 사원 수다([목록의 **21번 주제**](../21-aggregate-functions-count-forms/)).
...
```
  ...
  `USING`·`NATURAL` 은 [목록의 **18번 주제**](../18-using-and-natural-join/)가 정본이다. **여기서 인출할 것은 "조용히 0행이 나온다" 하나다.**
...
- **조인 알고리즘은 결과를 바꾸지 않는다.** 해시든 머지든 중첩 루프든 답은 같다([목록의 **59번 주제**](../59-scan-join-sort-operators/)).
...
- **안 쓴다 — 상대 표의 값이 필요 없을 때.** 「프로젝트가 있는 사원」만 알면 되면 `EXISTS` 가 낫다. **행이 안 늘어난다**([목록의 **19번 주제**](../19-semi-anti-join/)).
- **안 쓴다 — 외래키가 `NULL` 을 허용하는데 그 행도 필요할 때.** `dan` 이 바로 그 경우다.
- **주의 — 조인 뒤 집계.** `INNER JOIN` 을 쓰든 말든, 1:N 이면 행이 는다. 집계 전에 행 수를 센다([목록의 **25번 주제**](../25-join-fan-out/)).
...
- **`USING`·`NATURAL`** 은 [목록의 **18번 주제**](../18-using-and-natural-join/), **`EXISTS`/`IN`** 은 [**19번 주제**](../19-semi-anti-join/), **팬아웃 처방**은 [**25번 주제**](../25-join-fan-out/)가 정본이다.
- [SQL 주제 목록](../README.md)
...
- **`USING` 절** — 양쪽에 이름이 같은 열로 붙이는 축약형. 공통 열이 하나로 합쳐진다.\
  예: `JOIN dept USING (id)` — 여기서는 의미가 안 맞아 0행이 나온다. [목록의 **18번 주제**](../18-using-and-natural-join/).
...
- **반조인(semi join)** — 「저쪽에 짝이 있는지만」 보는 조인. 행이 안 늘어난다.\
  예: `WHERE EXISTS (SELECT 1 FROM emp_proj ep WHERE ep.emp_id = e.id)`. [목록의 **19번 주제**](../19-semi-anti-join/).
