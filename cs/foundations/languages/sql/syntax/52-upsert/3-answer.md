...
대안으로 `SELECT ... FOR UPDATE` 로 잠그는 방법도 있지만, **없는 행은 잠글 수 없다.** 신규 삽입 경쟁에는 안 통한다([목록의 **57번 주제**](../57-explicit-locking-and-deadlock/)).
...
같은 트랜잭션 안이면 내가 쓴 것은 보이므로 대부분은 괜찮다. 다만 **문 하나로 끝나지 않는다**는 점은 남는다 — 왕복이 하나 더 늘고, 격리 수준에 따라 읽히는 것이 달라진다([목록의 **56번 주제**](../56-isolation-levels-read-phenomena-mvcc/)).
...
**이 주제는 쓰기 주제다.** 그래서 모든 실험을 PG 는 `BEGIN … ROLLBACK`, MySQL 은 `START TRANSACTION … ROLLBACK` 으로 감쌌다.

| 무엇을 | 어디서 | 몇 번 | 비고 |
|---|---|---|---|
| 기본 upsert (1번·동작 방식 1) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `id=30` 이 `people` 로 · MySQL 의 영향 행 수는 **2** |
| ★ `(40,'hr')` — 같은 문, 다른 답 (2번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | **PG 는 `dept_name_key` 위반 에러 / MySQL 은 조용한 0행** |
| 충돌 대상을 `name` 으로 (2번) | PG 18.6 | 1회 | `id` 가 30 → **40** 으로 바뀐다 |
| 영향 행 수 1 / 2 / 0 (3번) | MySQL 8.4.10 | 1회(한 트랜잭션에 세 문) | **★ MySQL 에서만 결론이 서는 자리** |
| `RETURNING` + `xmax = 0` (4번) | PG 18.6 | 1회 | 갱신 `f` · 삽입 `t` |
| `RETURNING` 을 MySQL 에 (8번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | **`ERROR 1064` 가 근거다** |
| `ON CONFLICT DO NOTHING` (5번) | PG 18.6 | 1회 | `INSERT 0 0` — 2번의 그 문장이 조용히 통과 |
| 대상 없는 `DO UPDATE` (5번) | PG 18.6 | 1회 | `requires inference specification or constraint name` |
| 유니크 제약 없는 열을 대상으로 (5번) | PG 18.6 | 1회 | `emp.dept_id` — `no unique or exclusion constraint` |
| `INSERT IGNORE` + 경고 (5번) | MySQL 8.4.10 | 1회 | ★ **경고는 `SHOW WARNINGS` 로 따로 물어야 보인다**(1062·`dept.PRIMARY`) |
| ★ 한 문 안 같은 키 둘 (6번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | **PG 거부 / MySQL 은 `legal2` 채택** |
| 조건부 갱신 `DO UPDATE … WHERE` (7번) | PG 18.6 | 1회 | `INSERT 0 0` — `'hr' < 'aaa'` 가 거짓 |
| 조건부 갱신 `IF()` (7번) | MySQL 8.4.10 | 1회(두 경우 연속) | `aaa` 는 안 바뀌고 `zzz` 는 바뀐다 |
| 구식 `VALUES()` 경고 (문법 절) | MySQL 8.4.10 | 1회 | ★ **경고 1287 — 서버가 직접 낸 문구다** |
| `ON CONFLICT` 문법을 MySQL 에 (문법 절) | MySQL 8.4.10 | 1회 | `ERROR 1064` |
| 부분 유니크 인덱스 대상 (어디서 틀리나) | PG 18.6 | 2회 | 조건 없이 → 에러 / `WHERE active` 까지 적으면 → `INSERT 0 1` |
| 뒷정리 확인 | PG 18.6 · MySQL 8.4.10 | 각 1회 | `\dt` · `SHOW FULL TABLES` · **`emp` 4행 · `dept` 3행 재확인** |

**이 편이 만든 표와 그 뒷정리** — 「어디서 틀리나」의 부분 유니크 인덱스 실험만 표를 하나 만든다(`t52`·인덱스 `t52_uq`).\
**PG 는 DDL 도 트랜잭션 안이라 `BEGIN … ROLLBACK` 으로 만들고 지웠고**, 그래서 커밋된 자국이 없다.\
나머지 실험은 전부 **정본 표 `dept` 위에서 롤백**으로 돌렸다 — MySQL 의 경고 문구에 `dept.PRIMARY` 가 그대로 찍혀야 근거가 되기 때문이다.\
제출 전 두 엔진의 표 목록을 찍어 **`emp`·`dept` 둘만 남았고 4행·3행 그대로**임을 확인했다.

```text
--- PG 18.6 ---  \dt · SELECT COUNT(*)
 Schema | Name | Type  |  Owner
--------+------+-------+----------
 public | dept | table | postgres
 public | emp  | table | postgres
(2 rows)
 emp_rows = 4 · dept_rows = 3
--- MySQL 8.4.10 ---  SHOW FULL TABLES · SELECT COUNT(*)
+-----------------+------------+
...
**★ 한쪽에서만 결론이 서는 실험** — 3번(영향 행 수 1/2/0)은 **MySQL 출력만이 근거**다. PG 는 삽입·갱신을 모두 `INSERT 0 1` 로 보고하므로\
**그 현상 자체가 없다.** 반대로 4번(`RETURNING`)은 **PG 출력만이 근거**다.\
2번과 6번은 **양쪽을 다 던져야** 결론이 선다 — 한쪽만 보면 「upsert 는 에러가 난다」나 「조용히 넘어간다」 중 하나만 외우고, 엔진을 옮기는 순간 틀린다.

**구현 의존 항목** — 3번의 **1/2/0 이라는 세는 방식**(갱신을 삭제 1 + 삽입 1 로 센다), 4번의 **`xmax = 0`**(문서화된 계약이 아닌 PG 내부 열),\
제약 이름 `dept_name_key`·`dept.PRIMARY`, 에러·경고 번호(`1064`·`1062`·`1287`)다.\
**외울 것은 숫자나 문구가 아니라 「이 엔진은 무엇을 충돌로 보는가」다.**\
★ 가장 중요한 구현 의존은 **2번** — MySQL 이 **어느 유니크 키에서 충돌을 잡았는지 문장에 안 적혀 있다**는 것. 스키마를 읽어서는 알 수 없고 **던져야 드러난다.**

**언어 보장 항목** — 1·5·6번의 **의미**. 충돌이 나면 삽입이 갱신으로 전환된다는 것, PG 의 `DO UPDATE` 가 충돌 대상을 요구한다는 것,\
충돌 대상은 유니크 제약(또는 유니크 인덱스)이 있어야 한다는 것은 **두 매뉴얼의 `INSERT` 페이지가 정한 것**이다.
