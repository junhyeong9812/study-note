...
집합 연산의 나머지 규칙(열 개수·`ALL` 의 중복 제거)은 [목록의 **34번 주제**](../34-set-operations-union-intersect-except/)가 정본이다.
...
1. **먼저 스키마를 의심한다.** 숫자만 담는 열이 `varchar` 라면 진짜 처방은 **열 타입 수정**이다([목록의 **42번 주제**](../42-create-alter-drop-table/)).\
   질의를 고치는 것은 그 다음이다.
2. **값 쪽을 열의 타입에 맞춘다.** `WHERE code = '000123'`. 애플리케이션에서 문자열로 넘긴다.
3. **열을 꼭 변환해야 한다면 표현식 인덱스를 만든다.** PG 는 `CREATE INDEX ... ON t (substr(code,2))`,\
   MySQL 은 생성 열에 인덱스를 건다([목록의 **45번 주제**](../45-check-not-null-default-generated-columns/)·46번 주제).
...
`rows: 19905` 는 추정치이고 실제 행 수는 20,000 이다 — 추정과 실측의 차이는 [목록의 **60번 주제**](../60-explain-analyze-estimates-vs-actuals/)가 다룬다.\
