...
같은 표를 두 별칭으로 올려 행끼리 비교하는 형태는 [목록의 **17번 주제**](../17-self-join/)(SELF JOIN)가 정본이다.
...
> `NATURAL JOIN` 은 이 겹침을 **조용히** 이용한다 — `emp` 와 `dept` 를 `NATURAL JOIN` 하면 `id` 와 `name` **둘 다**로 붙어 0행이 나온다. [목록의 **18번 주제**](../18-using-and-natural-join/)가 정본이고, [13번](../13-inner-join/)에 실행 출력이 있다.
...
4. **`FROM` 의 형제 항목끼리는 서로를 모른다.** 바깥 행을 참조하려면 `LATERAL`([목록의 **20번 주제**](../20-lateral-join/)).
...
- **파생 테이블 안에서 바깥 행을 참조한다.**\
  "행마다 상위 N개"를 파생 테이블로 쓰려다 만나는 벽이다. `LATERAL`([목록의 **20번 주제**](../20-lateral-join/)) 또는 상관 서브쿼리([목록의 **11번 주제**](../11-subquery-scalar-correlated-any-all/))로 간다.
- **`VALUES` 문법을 옮겨 쓴다.**\
...
- 계획 노드 이름을 읽는 법은 [목록의 **58번 주제**](../58-explain-plan-tree/)·[**59번 주제**](../59-scan-join-sort-operators/)가 정본이다.
...
- **안 쓴다 — 두 번 이상 참조할 때.** 같은 파생 테이블을 두 번 적으면 두 번 계산된다. 그때는 CTE(`WITH`)가 낫다 — [목록의 **32번 주제**](../32-cte-with-clause/).
- **안 쓴다 — 세 겹을 넘어갈 때.** 읽는 사람이 안쪽부터 되짚어야 한다. CTE 로 펴면 위에서 아래로 읽힌다.
- **안 쓴다 — 바깥 행을 참조해야 할 때.** 그건 `LATERAL`([목록의 **20번 주제**](../20-lateral-join/)) 또는 상관 서브쿼리([목록의 **11번 주제**](../11-subquery-scalar-correlated-any-all/))다.
...
- **상관 서브쿼리**(스칼라·`ANY`/`ALL`)는 [목록의 **11번 주제**](../11-subquery-scalar-correlated-any-all/), **`LATERAL`** 은 [목록의 **20번 주제**](../20-lateral-join/), **CTE(`WITH`)** 는 [목록의 **32번 주제**](../32-cte-with-clause/)가 정본이다.
- [SQL 주제 목록](../README.md)
