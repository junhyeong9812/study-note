...
벽에 난 문이 `LATERAL` 이고 [목록의 **20번 주제**](../20-lateral-join/)다. 바깥 행마다 도는 서브쿼리 일반은 [목록의 **11번 주제**](../11-subquery-scalar-correlated-any-all/)다.\
...
이 규칙을 안 지킨 대가가 가장 크게 나오는 곳이 `NATURAL JOIN` 이다 — 겹치는 이름으로 **조용히** 붙는다([목록의 **18번 주제**](../18-using-and-natural-join/), 실행 출력은 [13번](../13-inner-join/)에 있다).
...
**결과는 어느 쪽이든 같다.** 계획 노드를 읽는 법은 [목록의 **58번 주제**](../58-explain-plan-tree/)·[**59번 주제**](../59-scan-join-sort-operators/)다.
...
CTE 의 범위·참조 규칙과 `MATERIALIZED` 같은 부가 문법은 [목록의 **32번 주제**](../32-cte-with-clause/)가 정본이다.\
