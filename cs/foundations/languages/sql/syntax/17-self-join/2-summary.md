...
비용 — 깊이가 **미지수**이면 재귀 CTE 로 간다(목록의 **33번 주제**). **경계: 여기는 깊이가 고정일 때까지, 거기는 깊이가 미지수일 때부터.**
...
★ **같은 일을 윈도우 함수 `LAG`/`LEAD` 로 하면 한 번만 훑는다**(목록의 **30번 주제**).\
...
6. **깊이만큼 별칭이 필요하다.** 깊이가 미지수면 재귀 CTE(목록의 **33번 주제**)다.
...
- **안 쓴다 — 깊이가 미지수일 때.** 재귀 CTE 다(목록의 **33번 주제**).
- **안 쓴다 — 이웃 행 한 개만 필요할 때.** `LAG`/`LEAD` 가 훨씬 싸다(목록의 **30번 주제**).
...
- **깊이가 미지수인 계층**은 목록의 **33번 주제**, **이웃 행 비교**는 **30번 주제**가 정본이다.
- [SQL 주제 목록](../README.md)
...
- **재귀 CTE(recursive CTE)** — 깊이를 모르는 계층을 펴는 `WITH RECURSIVE` 문법.\
  예: 조직도가 몇 단인지 모를 때. 목록의 33번 주제.
- **`LAG` / `LEAD`** — 정렬된 이웃 행의 값을 가져오는 윈도우 함수.\
  예: 급여 사다리를 한 번만 훑고 계산한다. 목록의 30번 주제.

## 더 들어가면

- **`SELF JOIN` 은 「같은 표를 두 번 스캔한다」는 뜻이 아니다.** 엔진이 한 번 읽어 양쪽에 쓸 수도 있다 — 계획을 보고 판단한다(목록의 **58번 주제**).
- **`staff` 에 외래키를 걸 수 있다** — `mgr_id` 가 같은 표의 `id` 를 참조하는 자기 참조 외래키(목록의 **44번 주제**). 그러면 없는 상사를 가리키는 행이 막힌다.
