...
MySQL 에서는 `UNION ALL` 목록이나 재귀 CTE 로 만든다([목록의 **33번 주제**](../33-recursive-cte/)).\
...
같은 표를 두 별칭으로 올리는 형태 일반은 [목록의 **17번 주제**](../17-self-join/)(SELF JOIN)가 정본이다.
...
그림 해설 — **오른쪽이 1행이면 곱해도 행 수가 안 는다.** `4 × 1 = 4`. 그래서 이 용법은 안전하다.`AVG` 값이 `300+500+400)/3 = 400` 인 것은 `cho` 의 `NULL` 을 집계가 건너뛰었기 때문이다([04번](../04-null-three-valued-logic/)).비용 — 한 행짜리 표를 붙이는 것이라 거의 공짜다. 같은 일을 윈도우 함수로도 할 수 있다([목록의 **26번 주제**](../26-window-functions-vs-aggregates/)).

> **덤으로 보이는 차이** — 같은 `AVG` 인데 PG 는 소수 16자리, MySQL 은 4자리로 찍는다.> 수치 타입과 정밀도 이야기는 이 주제가 아니라 [목록의 **36번 주제**](../36-numeric-types-and-functions/)다.
...
- ★ **그러므로 「카티션곱」은 성능 경고가 아니라 의미 모델이다.** 성능이 궁금하면 `EXPLAIN` 을 본다([목록의 **58번 주제**](../58-explain-plan-tree/)).
...
- **같은 표를 두 번 올리는 형태**는 [목록의 **17번 주제**](../17-self-join/), **`USING`·`NATURAL`** 은 [목록의 **18번 주제**](../18-using-and-natural-join/), **조인 팬아웃**은 [목록의 **25번 주제**](../25-join-fan-out/)가 정본이다.
- [SQL 주제 목록](../README.md)
