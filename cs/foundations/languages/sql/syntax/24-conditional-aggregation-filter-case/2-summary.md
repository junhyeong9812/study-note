...
- **조건 열에 함수를 씌운다.**\
  `CASE WHEN YEAR(hired) = 2026` 류는 인덱스를 못 탄다 — 다만 조건부 집계는 대개 전체 스캔이라 **여기서는 큰 문제가 아니다.** 인덱스 이야기는 목록의 **47번 주제**.
...
- **`FILTER` 는 윈도우 함수에도 붙는다**(PG). 다만 윈도우 자체는 목록의 **26~31번 주제**이고 여기서는 다루지 않는다.
