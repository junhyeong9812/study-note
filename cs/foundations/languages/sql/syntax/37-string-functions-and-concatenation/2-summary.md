...
- **안 쓴다 — `WHERE` 의 열에 문자열 함수.** `UPPER(col)`·`SUBSTRING(col,...)` 은 인덱스를 죽인다([목록의 **47번 주제**](../47-when-indexes-are-used/)).\
  꼭 필요하면 표현식 인덱스를 만든다([목록의 **46번 주제**](../46-index-definition-composite-partial-expression/)).
