...
대가 — 정렬 키가 하나 늘어 **인덱스 정렬을 못 쓰게 될 수 있다.** 큰 표에서는 계획을 확인한다(목록의 47·58번 주제).
...
- **큰 표에서는 정렬 키와 인덱스를 맞춘다.** 정렬 키를 표현식으로 감싸면 인덱스 정렬을 못 쓴다(목록의 46·47번 주제).
...
- **정렬과 인덱스**는 목록의 **46·47번 주제**, **계획 읽기**는 **58번 주제**가 정본이다.
- [SQL 주제 목록](../README.md)
...
- **PG 에서 `UPDATE` 후 순서가 바뀌는 것은 MVCC 의 부산물이다.** 갱신은 제자리 수정이 아니라 **새 판본을 쓰고 옛 판본을 죽은 것으로 표시**하는 일이라 새 위치가 생긴다. 순서가 바뀐 것은 버그가 아니라 저장 구조가 드러난 것이다. MVCC 의 정본은 목록의 56번 주제다.
