...
순위 함수의 정본은 [목록의 **29번 주제**](../29-ranking-functions/)다.
...
(MySQL 의 추정 `rows=100161` 은 **추정치**이고, 실제로 만든 것은 `actual ... rows=10` 이다. 추정과 실측이 어긋나는 자리를 읽는 법은 [목록의 **60번 주제**](../60-explain-analyze-estimates-vs-actuals/)다.)
...
MySQL 의 추정 `rows=100161` 처럼 **추정과 실측이 어긋나는 것**도 그 자체가 관찰 대상이다([목록의 **60번 주제**](../60-explain-analyze-estimates-vs-actuals/)).
