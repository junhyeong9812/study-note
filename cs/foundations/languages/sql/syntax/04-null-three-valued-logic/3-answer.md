...
**판정 결과는 양쪽이 완전히 같다.** 타입 체계 이야기는 [목록의 **35번 주제**](../35-type-system-and-casting/)이다.
...
두 엔진의 값이 완전히 같다. **소수 자릿수만 다르다** — `AVG` 의 결과 타입 차이일 뿐 값은 400 으로 같다(수치 타입은 [목록의 **36번 주제**](../36-numeric-types-and-functions/)).
...
`COUNT` 의 세 형태(`COUNT(*)`·`COUNT(열)`·`COUNT(DISTINCT 열)`)는 [목록의 **21번 주제**](../21-aggregate-functions-count-forms/)이 정본이다.
...
비교 연산자 쪽의 자세한 규칙은 [목록의 **05번 주제**](../05-null-comparison-is-distinct-from/)이 정본이다.
...
| 무엇을 | 어디서 | 몇 번 | 비고 |
|---|---|---|---|
| `NULL` 과의 비교 셋 (3번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `= NULL`·`<> NULL`·`= 0` 전부 `NULL` 표시 |
| ★ 3값 논리 다섯 칸 (2번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | **`FALSE AND NULL` 이 `FALSE`** 인 것이 핵심 |
| 조건을 뒤집어도 안 맞는다 (1번) | PG 18.6 · MySQL 8.4.10 | 각 3회 | `gt`=1 · `le`=2 · `total`=4 — **1+2=3≠4** |
| `IS NOT TRUE` 로 누르기 (1번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | 3 — 이제 `1+3=4` |
| `NOT (salary > 400)` (3번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | 2행 |
| `salary <> 300` (3번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | 2행 — `cho` 가 안 온다 |
| `IS NULL` · `IS TRUE` 계열 (동작 방식 4) | PG 18.6 · MySQL 8.4.10 | 각 2회 | 표시만 `t/f` ↔ `1/0` |
| ★ `NOT IN` 에 `NULL` (4번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | **0행** · MySQL 은 한 줄도 안 찍는다 |
| 리터럴 `IN`/`NOT IN` 전개 (4번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `NULL` 이 끼자 `UNKNOWN` |
| `IN` 은 멀쩡하다 (4번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | 2행 — **이것이 함정을 키운다** |
| `NOT EXISTS` (5번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `hr` 1행 |
| 집계 네 값 (6번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `4 · 3 · 1200 · 400` — 자릿수만 다르다 |
| 전부 `NULL` 인 그룹의 `SUM` (7번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | `dept_id=20` 이 **`NULL`** |
| `DISTINCT` 가 `NULL` 을 접는다 (8번) | PG 18.6 · MySQL 8.4.10 | 각 1회 | 4행 |
| `COALESCE(salary,0)` (문법 절) | PG 18.6 · MySQL 8.4.10 | 각 1회 | |
| ★ `NULL` 안전 등호 두 방언 (8번) | PG 18.6 · MySQL 8.4.10 | 각 2회 | **양쪽에 서로의 문법을 던졌다** — 둘 다 에러 |

(위 질의를 PG 에는 **한 세션에 묶어 한 번 더** 던졌다 — 그래서 PG 쪽은 표의 두 배, MySQL 쪽은 표 그대로다.\
두 판의 **값이 같았다**. 첫 판은 `NULL` 표시 설정만 달라 빈칸으로 찍혔다 — 서버가 다른 답을 준 것이 아니다.)

**★ 한쪽에서만 결론이 서는 실험** — 8번의 두 에러는 **서로를 대신하지 못한다.**\
MySQL 의 `ERROR 1064` 는 **`IS NOT DISTINCT FROM` 이 없다는 근거**일 뿐이고, PG 의 `operator does not exist: unknown <=> unknown` 은\
**`<=>` 가 없다는 근거**다. 「양쪽 다 `NULL` 안전 등호가 있다」로 뭉뚱그리면 **이식할 때 터진다** — 양쪽에서 도는 표현은 없다.

**구현 의존 항목** — 2·6번의 **표시 방식**이다. `t/f` 대 `1/0` 은 PG 에 진짜 `boolean` 타입이 있고 MySQL 의 `BOOLEAN` 이 `TINYINT(1)` 이기 때문이고,\
`AVG` 의 소수 자릿수(`400.0000000000000000` 대 `400.0000`)는 **결과 타입의 선택**이다. **값은 같다 — 외울 것은 표시가 아니라 판정이다.**\
에러 번호·문구도 구현 세부다.

**언어 보장 항목** — 1·2·3·4·5·6·7·8번의 **판정 자체**. `NULL` 과의 비교가 `UNKNOWN` 이라는 것, `WHERE` 가 `TRUE` 만 통과시킨다는 것,\
`UNKNOWN` 이 결과가 안 정해졌을 때만 번진다는 것, `NOT IN` 이 `NULL` 앞에서 절대 `TRUE` 가 못 된다는 것,\
집계가 `NULL` 을 건너뛰고 `COUNT(*)` 만 예외라는 것, `GROUP BY`·`DISTINCT` 가 `NULL` 을 한 덩어리로 묶는다는 것은\
**두 엔진에서 한 칸도 다르지 않았다.** 이 주제는 방언이 거의 없는 쪽이다.

**방언이 갈리는 항목 — 버전이 오르면 다시 찍을 자리** — ① `NULL` 안전 등호의 문법(8번) ② 불린 표시(`t/f` ↔ `1/0`)\
