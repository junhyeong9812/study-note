...
- ★ 파생이 거는 조건은 트레이트마다 다르다 — 그 전수는 목록의 **27번 주제**의 몫이다.
...
- 다른 점은 셋뿐이다 — **prelude 에 들어 있어 `use` 가 필요 없고**, `?` 연산자가 붙고,
  `map`·`and_then` 같은 조합 메서드가 딸려 있다(목록의 **21번 주제**·**22번 주제**).
...
- 상세한 `Box` 이야기는 여기가 정본이 아니다(목록의 **40번 주제**). 여기서는 **왜 막히나**까지다.
...
- **변형이 계속 늘고 남이 추가해야 할 때** — 그때는 트레이트가 맞다(목록의 **33번 주제**).
...
- 목록의 **21번 주제**·**22번 주제** — `Option`·`Result` 의 **메서드 표면**. 여기는 **그것이 enum 이라는 사실**까지.
- 목록의 **40번 주제** — `Box` 와 재귀 타입. 여기는 **왜 막히나**까지, 푸는 법의 정본은 거기다.
- 목록의 **27번 주제** — `derive` 매크로가 거는 조건 전수. 여기는 **열거형에서 걸리는 둘**만.
...
- **`enum` 에 제네릭을 걸 수 있다** — `MyOption<T>` 가 그 예다((6)). 경계 문법은 목록의 **31번 주제**.
