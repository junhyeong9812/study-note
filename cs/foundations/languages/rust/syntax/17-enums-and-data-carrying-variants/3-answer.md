...
- 이 숫자 역시 **이 판의 관찰**이다. `Box` 가 재귀를 어떻게 푸는지의 정본은 목록의 **40번 주제**다.
...
- 다른 점 **셋** — ① **prelude 에 있어 `use` 가 필요 없다** ② **`?` 연산자가 붙는다**
  ③ **`map`·`and_then`·`unwrap_or` 같은 조합 메서드가 딸려 있다**(목록의 **21번 주제**·**22번 주제**).
...
- **변형이 계속 늘고 남이 추가해야 하면** 열거형이 아니라 **트레이트**가 맞다(목록의 **33번 주제**).
...
- `Box` 와 재귀 타입의 정본은 목록의 **40번 주제**다. 여기서는 **왜 막히나**와 **고치면 몇 바이트인가**까지만 다뤘다.
