...
- (5)의 임시값 실험이 창으로 쓴 것은 **`Drop` 이 찍는 순서**이고, `Drop` 시점의 정본은
  [**09번 주제**](../09-copy-clone-and-drop/)와 [목록의 **44번 주제**](../44-drop-mem-drop-replace-and-take/)다.
- **넷째 길(조합 메서드)** 은 [목록의 **21번 주제**](../21-option-and-combinators/)(`Option`)와 **22번 주제**(`Result`·`?`)다.
