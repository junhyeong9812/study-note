...
- ★ 그래서 트레이트를 설계할 때 **필수 메서드를 적게 두고 기본 메서드로 넓히는** 꼴이 표준이다.
  `Iterator` 가 `next()` 하나만 요구하고 나머지 수십 개를 기본 메서드로 주는 것이 그 극단이다([목록의 **36번 주제**](../36-iterator-adapters-laziness-and-collect/)).
...
- ★ 기본 메서드도 **표에 한 칸을 차지한다** — 안 덮었으면 트레이트의 몸통을 가리키는 칸이 들어갈 뿐이다.
  그래서 `dyn` 에서도 덮어쓰기가 그대로 먹는다. 정본은 [목록의 **33번 주제**](../33-dyn-trait-objects-and-object-safety/)다.
...
**한 타입이 여러 오른쪽 피연산자를 받아야 하기 때문**이다([목록의 **30번 주제**](../30-operator-overloading-std-ops-index-and-deref/)).
...
공개 API 에서는 **호출자가 타입을 찍을 길을 막는 것**이므로 생각하고 쓴다([목록의 **32번 주제**](../32-impl-trait-argument-return-position-and-2024-capture/)).
...
- [목록의 **31번 주제**](../31-generics-trait-bounds-where-and-monomorphization/) — 제네릭과 트레이트 경계·단형화. **경계 문법과 코드 팽창**의 정본이다.
- [목록의 **32번 주제**](../32-impl-trait-argument-return-position-and-2024-capture/) — `impl Trait` 의 정본. 여기서는 (6)만큼만 닿았다.
- [목록의 **33번 주제**](../33-dyn-trait-objects-and-object-safety/) — `dyn Trait` 와 객체 안전성의 정본. (2)의 표(vtable)와 (5)의 조건이 거기서 깊어진다.
- [목록의 **36번 주제**](../36-iterator-adapters-laziness-and-collect/) — `Iterator`. **필수 메서드 하나 + 기본 메서드 수십 개**의 극단 사례다.
