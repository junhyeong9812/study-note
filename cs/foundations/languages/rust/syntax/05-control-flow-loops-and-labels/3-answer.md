...
- 갈림의 근거는 **그 타입이 `Copy` 인가**다. 판정의 정본은 목록의 **09번 주제**다.
...
- 이 규칙(불변 빌림 여럿 ↔ 가변 빌림 하나)의 정본은 목록의 **10번 주제**(빌림 `&`·`&mut`)이고,\
  고치는 관용(인덱스·`retain`·`mem::take`)은 목록의 **11번 주제**다. 여기서는 **현상까지**다.
...
- **`Copy` 판정** — 목록의 **09번 주제**(`Copy`와 `Clone`, `Drop` 시점).\
