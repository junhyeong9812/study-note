...
- `Copy` 판정의 정본은 목록의 **09번 주제**(`Copy`와 `Clone`)다.
...
- 규칙과 해결법의 정본은 목록의 **10번 주제**(빌림)와 **11번 주제**(빌림 검사기 전형)다. 여기서는 **현상까지**.
...
| 배열에서 원본이 남는 것 | **언어** | `[T; N]` 이 `Copy` 라서. 정본은 목록의 **09번 주제** |
...
- 목록의 **09번 주제**(`Copy`와 `Clone`) — 배열이 왜 남고 `Vec` 이 왜 사라지는지의 판정 기준
- 목록의 **10번 주제**(빌림) · **11번 주제**(빌림 검사기 전형) — 순회 중 변경(E0502)의 정본
