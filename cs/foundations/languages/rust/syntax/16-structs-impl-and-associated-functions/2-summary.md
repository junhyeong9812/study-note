...
- ★ **newtype 은 안쪽 타입과 크기·정렬이 같다** — `Newtype(u64)` 도 `u64` 도 `8/8`.
  **감싸는 비용이 0**이라는 것이 newtype 관용구의 근거다(목록의 **26번 주제**가 정본).
...
- `derive` 자체(무엇을 요구하나·언제 안 되나)는 목록의 **27번 주제**가 정본이다. 여기는 맛만 본다.
...
| 상속으로 공통 동작을 뽑고 싶다 | ★ **트레이트** | Rust 에 상속은 없다 — 목록의 **25번 주제** |
...
- 목록의 **17번 주제**(열거형) — 구조체가 「**전부 다 있다**」면 열거형은 「**하나만이다**」다
- 목록의 **25번 주제**(트레이트) — `impl Trait for Type` 은 거기. **여기는 고유 `impl`(inherent impl)만**
- 목록의 **26번 주제**(고아 규칙과 newtype) · **27번 주제**(`derive`) · **45번 주제**(모듈·가시성) ·\
...
- ★ **ZST 는 컬렉션에서 실제 이득이 된다** — `HashSet<K>` 가 `HashMap<K, ()>` 로 구현되는 것이 대표다.\
  `Vec<Unit>` 는 길이만 세고 **힙 할당을 하지 않는다**(목록의 **38번 주제**·**39번 주제**).
...
- **`self` 리시버에 다른 타입도 온다** — `self: Box<Self>`·`self: Rc<Self>` 같은 형태가 있다.\
  `dyn Trait` 를 소비하는 메서드에서 쓰인다(목록의 **33번 주제**·**40번 주제**).
