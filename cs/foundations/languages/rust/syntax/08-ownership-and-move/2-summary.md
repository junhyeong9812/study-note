...
- ★ **이 불편함이 빌림(`&`)의 동기다.** 정본은 [목록의 **10번 주제**](../10-borrowing-and-aliasing-rules/).
...
- ★ **`&mut T` 는 `Copy` 가 아니다** — 그랬다면 가변 별칭이 공짜로 생긴다(정본은 [목록의 **10번 주제**](../10-borrowing-and-aliasing-rules/)).\
...
- **필드 하나가 `Copy` 가 아니면 구조체도 못 된다.** 정본은 [목록의 **09번 주제**](../09-copy-clone-and-drop/)다.
...
| 상황 | 쓸 것 | 이유 |
|---|---|---|
| 값을 읽기만 한다 | `&T` ([목록의 **10번 주제**](../10-borrowing-and-aliasing-rules/)) | 이동이 안 나고 원본이 산다 |
...
- `clone()` 은 힙을 새로 할당한다. **빌릴 수 있으면 빌리는 것이 먼저**다([목록의 **10번 주제**](../10-borrowing-and-aliasing-rules/)).
