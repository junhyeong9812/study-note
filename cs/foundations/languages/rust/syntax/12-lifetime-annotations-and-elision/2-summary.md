...
참조를 필드로 담는 이야기 전체는 [목록의 **13번 주제**](../13-struct-references-and-static/)가 정본이다.
...
| 참조를 필드에 담는다 | `struct X<'a>` | 선택지가 없다. 자세한 것은 [목록의 **13번 주제**](../13-struct-references-and-static/) |
...
- [목록의 **13번 주제**](../13-struct-references-and-static/)(구조체에 참조 담기·`'static`의 두 의미) — ★ **여기는 표기와 생략 규칙까지**,\
  **참조를 필드로 담는 타입의 설계**와 `'static` 심화는 거기다
- [목록의 **14번 주제**](../14-string-vs-str/)(`String` 대 `&str`) · **32번 주제**(`impl Trait` 의 2024 수명 포착) ·\
...
- ★ **`for<'a>`(HRTB)** 는 「어떤 수명이 와도 된다」는 뜻이다. (2)에서 컴파일러가 찍어 준 `for<'a> fn(&'a _) -> &'a _` 가 그것이고,\
  클로저를 인자로 받는 API 에서 자주 나온다([목록의 **34번 주제**](../34-closures-fn-fnmut-fnonce-and-move/)·**35번 주제**).
- **반환 위치 `impl Trait`** 는 어떤 수명을 「포착」하는지가 **2024 에디션에서 바뀌었다** — [목록의 **32번 주제**](../32-impl-trait-argument-return-position-and-2024-capture/).
