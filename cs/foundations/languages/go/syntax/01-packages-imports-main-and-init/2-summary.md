...
| 맵 순회 순서 | **매번 바뀐다 — 이 주제는 아예 안 쓴다** | 목록의 **09번 주제** 참조 |
...
- 고치는 법 — 순서가 필요하면 `init` 을 쓰지 말고 **한 `init` 안에서 명시적으로 부른다**,
  또는 `sync.Once` 로 첫 사용 시점에 미룬다(목록의 **32번 주제**).
...
- 대신 사람들이 **패키지를 억지로 합치는 것**으로 푼다. 보통 옳은 해법은
  **인터페이스를 쓰는 쪽에 두는 것**이다(목록의 **20번 주제**).
...
- 이 문서에서는 **돌려 보지 않았다** — 고루틴은 목록의 **28번 주제**의 정본이다. 여기서는 명세 인용까지만 한다.
...
| 첫 사용 시점까지 미루고 싶다 | `sync.Once`(목록의 **32번 주제**) | `init` 은 **무조건** 돈다 |
| 패키지 둘이 서로를 부른다 | 인터페이스를 소비자 쪽에(목록의 **20번 주제**) | 순환은 컴파일이 거부한다 |
...
- 목록의 **28번 주제**(고루틴) — 「`main` 이 끝나면 다른 고루틴은 어떻게 되나」의 정본.
  여기서는 명세 인용까지만 했다
- 목록의 **32번 주제**(`sync`) — `init` 대신 `sync.Once` 로 미루는 방법의 정본
