...
- ★★★ `-race` 가 통과하는 이유 — **이것은 데이터 레이스가 아니다.**
  ...
  (정본은 [목록의 **35번 주제**](../35-data-races-and-the-race-detector/)다.)
...
- 그러면 무엇으로 막나 — 셋이다.
  ...
  - **테스트에 「인자가 그대로인가」를 계약으로 적는 것**([목록의 **49번 주제**](../49-testing-table-driven-t-run-cleanup-and-parallel/)).
...
- ★ 즉 **원소는 고쳐지고 길이는 안 고쳐진다** — [목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/) (3)절의
...
- ★ 키가 여럿이었다면 마지막 줄을 **정렬해서** 찍어야 한다 —
  ...
  이 프로그램은 **키가 하나뿐**이라 그 문제가 성립하지 않는다. 정본은 [목록의 **09번 주제**](../09-maps-declaration-comma-ok-delete-and-iteration-order/)다.
...
- **「`cap` 이 남으면 재사용한다」** — [목록의 **06번 주제**](../06-len-cap-and-append-reallocation/) (2)절(계약)·(3)절(예측식).
  그 앞의 「헤더가 복사된다」는 [목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/) (3)·(4)절이다.
- **막는 법(`s[:n:n]`·`copy`·`clear`)** — [목록의 **08번 주제**](../08-copy-three-index-slicing-and-memory-retention/).
- **조용한 실패의 총론** — [`../../../../../ops-patterns/failure-modes/`](../../../../../ops-patterns/failure-modes/).
  **그쪽은 분류와 관측 설계까지**, 여기는 **Go 슬라이스 별칭 한 사례**부터다.
- **`-race` 가 실제로 무엇을 잡나** — [목록의 **35번 주제**](../35-data-races-and-the-race-detector/).
- **「인자가 안 바뀌었나」를 표 기반 테스트로** — [목록의 **49번 주제**](../49-testing-table-driven-t-run-cleanup-and-parallel/).
- **맵 인덱스 식과 순회 순서** — [목록의 **09번 주제**](../09-maps-declaration-comma-ok-delete-and-iteration-order/).
...
| 고루틴 둘이 같은 배열을 만질 때 | **안 던져 봤다.** 그때는 진짜 레이스이고 정본은 [목록의 **35번 주제**](../35-data-races-and-the-race-detector/)다 |
