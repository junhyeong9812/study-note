...
- ★ 이 주제에서 맵을 쓰는 곳은 여기뿐이고 **키가 하나뿐**이라 순회 순서 문제가 없다.
  키가 여럿이면 **정렬해서 찍어야 한다** — Go 는 맵 순회를 일부러 무작위화한다([목록의 **09번 주제**](../09-maps-declaration-comma-ok-delete-and-iteration-order/)).
...
- [목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/)(배열과 슬라이스) — **그쪽은 「헤더가 복사된다」는 사실까지**,
...
- `bytes.Buffer`·`strings.Builder` 는 내부 슬라이스를 **밖에 안 내준다.**
  `Bytes()` 가 내부 배열을 주는 것은 알려진 예외라 문서가 경고한다. 정본은 [목록의 **11번 주제**](../11-strings-strconv-bytes-and-unicode-utf8/)다.
