...
- 마지막 루프에서 `v` 는 **`1`** 이다. `range` 가 배열을 **한 번 복사해 두고** 돌기 때문이다.
  `a[i] = 0` 은 원본을 미는데 `v` 는 복사본에서 나온다. 정본은 목록의 **14번 주제**다.
...
- **`cap` 의 증가 규칙** — 목록의 **06번 주제**. 이 주제는 「갈림이 있다」까지만 말하고
  「얼마로 늘어나나」는 거기서 **구현으로 표시하고 실측**한다.
- **(4)절 함정이 버그가 되는 사례** — 목록의 **07번 주제**. 부분 슬라이스에 `append` 하기,
  제자리 필터, 삭제 관용구가 거기 모여 있다.
- **공유를 끊는 법** — 목록의 **08번 주제**(`copy`·`s[a:b:c]`·메모리 유지).
  이 주제의 (7)절은 맛보기다.
- **`range` 가 배열을 복사하는 것** — 목록의 **14번 주제**.
