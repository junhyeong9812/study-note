...
- **슬라이스 헤더 세 칸** — 정본은 [목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/).
  여기서는 `Sizeof` 24 와 `nil` 여부까지만 봤다.
- 「**값은 `nil` 인데 인터페이스는 `nil` 이 아니다**」 — 정본은 [목록의 **21번 주제**](../21-nil-interface-vs-interface-holding-nil-pointer/).
...
| `Point` 의 56바이트 배치 | **필드 정렬·패딩**([목록의 **17번 주제**](../17-struct-literals-comparability-field-tags-and-sorting/)). 필드 순서를 바꾸면 달라질 수 있다 |
