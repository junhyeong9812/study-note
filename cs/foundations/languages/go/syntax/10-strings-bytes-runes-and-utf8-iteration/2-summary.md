...
비용 — 불변이라 **복사가 헤더 두 칸**이다. 이어 붙일 때마다 새 문자열이 생기는 것이 대가다(목록의 **11번 주제**).
...
- **맵 키가 된다** — 불변이고 비교 가능하기 때문이다(목록의 **09번 주제**).
...
| 많이 이어 붙인다 | `strings.Builder` | 목록의 **11번 주제** |
...
- 목록의 **14번 주제**(`for` 의 네 형태) — `range` 문 전체의 정본
