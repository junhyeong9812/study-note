...
- ★★ 이것은 **슬라이스와 정반대**다. 슬라이스는 `&s[0]` 이 되고 `s[0].N = 2` 도 된다 —
  슬라이스의 원소는 **자리가 안 움직이기** 때문이다([목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/)).
...
- ★ **슬라이스 필드를 가진 구조체도 거부된다**(`invalid map key type Bad`) —
  구조체의 비교 가능성은 **필드가 전부 비교 가능할 때만** 성립하기 때문이다([목록의 **17번 주제**](../17-struct-literals-comparability-field-tags-and-sorting/)).
...
- ★ 왜인가 — 맵의 「용량」은 **gc 의 내부 사정**이고 명세가 노출하지 않기로 한 것이다.
  슬라이스는 `cap` 이 **관찰 가능한 계약의 일부**라 정반대다([목록의 **06번 주제**](../06-len-cap-and-append-reallocation/)).
...
- ★★ 슬라이스와 대비하면 이렇다 — 슬라이스는 **(포인터, len, cap) 세 칸**이라
  **원소 수정은 보이고 `append` 는 안 보인다**([목록의 **05번 주제**](../05-arrays-vs-slices-value-and-header/)).
  맵은 **한 칸**이라 **길이 변화까지 보인다.** 안 보이는 것은 **재대입 하나**다.
- `maps.Clone`(**1.21**)은 **얕은 복사**다 — 값이 슬라이스면 그 슬라이스는 여전히 공유다([목록의 **07번 주제**](../07-slice-sharing-silent-bugs/)).
...
- ★ 그래서 **맵 필드를 가진 구조체도 `==` 가 안 된다** — 슬라이스 필드와 같은 이유다([목록의 **17번 주제**](../17-struct-literals-comparability-field-tags-and-sorting/)).
...
| `fmt` 가 **정렬해 찍는 것** | **표준 라이브러리의 계약** | 1.12 **릴리스 노트**에 적힌 변경이다 — 이 판(1.27.1)의 `fmt` 패키지 문서 본문에는 그 문장이 없고, 구현은 `internal/fmtsort` 다. `range` 와 무관하다 |
