...
### 5. `range` 는 50·5·2·50·50·100·10 — 깃발은 `number`·`date`·`time` 쪽에만 켜진다 · 속성과 스크립트가 갈린다
...
- ★★★ **`a2`(속성 `1.3`)와 `a2s`(스크립트 `1.3`)는 값이 같은데 깃발이 갈렸다** — `a6`/`a6s` 도 같다. 명세의 step mismatch 줄에는 **값이 들어온 경로를 묻는 조건이 없다** — 이 판의 Chrome 이 명세와 갈린 자리다(A9). `a3`(`step` 없음 = 기본 1, 값 `1.5`)도 깃발이 없다 — 속성으로 넣은 값이라 `a2` 와 같은 꼴로 읽힌다(스크립트 판은 던지지 않았다).
...
### 9. 갈림은 `stepMismatch` 의 「속성 대 스크립트」 · 화면 형식은 사용자 로케일을 따랐고 그것은 구현의 선택이다

- ★★★ **갈린 자리** — `number`·`time` 에서 **속성으로 넣은 값은 `stepMismatch` 가 안 켜지고 스크립트로 넣으면 켜진다**(A5). 명세의 줄 — 「허용 눈금이 있고, 값을 수로 바꾼 것이 눈금 기준에서 **정수배가 아니면** step mismatch」 — 에는 **값의 출처 조건이 없다.** `date` 는 속성으로도 켰다(`a5`).
...
- **`min`/`max`/`step` 의 눈금 규칙** — 목록의 **28번 주제**. 여기는 그 깃발과 문구가 어떻게 보이나까지.
...
| 항목 | 이 판의 값 | 왜 다시 찍나 |
|---|---|---|
| 속성으로 넣은 `number`·`time` 값의 `stepMismatch` | **꺼져 있다** | 명세와 갈린다 |
