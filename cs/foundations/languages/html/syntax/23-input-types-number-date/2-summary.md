...
- ★★★ **같은 값 `1.3` 이 `a2`(`value="1.3"`)에서는 `(없음)`, `a2s`(스크립트 `.value = "1.3"`)에서는 `stepMismatch`** 다. `time` 도 같다(`a6`/`a6s`). ★ 갈린 것은 값이 들어온 경로가 아니라 **눈금의 출발점(step base)** 이다. 명세의 step base — 「`min` 속성 → 없으면 **`value` 속성** → 없으면 타입의 기본 기준 → 0」. `a2` 는 `min` 이 없어 **`value="1.3"` 자신이 출발점**이라 `1.3` 이 눈금 위에 있고(`a6` 도 `value="13:07"` 이 출발점), `a2s`·`a6s` 는 `value` 속성이 없어 출발점이 0 이라 눈금에서 벗어난다. **`date` 의 `a5` 가 켜진 것은 `min` 이 있어서**다(출발점 = `min`). 명세의 step mismatch 줄이 값의 경로를 묻지 않는 것 그대로이고, **이 판은 명세대로 동작했다** — 경로(속성·스크립트·키)를 바꿔 가며 다시 잰 격자는 [28번](../28-validation-attributes/2-summary.md)의 (2)에 있다.
...
| **명세(HTML)** | step base = `min` → **`value` 속성** → 타입의 기본 기준 → 0 — `a2`·`a6` 은 `value` 속성이 출발점이라 통과, `a2s`·`a6s` 는 출발점 0 이라 `stepMismatch` · `a5` 는 `min` 이 출발점 ([28번](../28-validation-attributes/2-summary.md)의 (2)) | (4) |
...
7. **`step` 의 눈금은 `min`, 없으면 `value` 속성에서 출발한다 — 그래서 `value` 속성으로 준 값은 `min` 이 없으면 언제나 눈금 위에 있다(명세대로).**
...
- [28번 주제](../28-validation-attributes/2-summary.md)(`min`/`max`/`step` 의 눈금 규칙 · step base) · 목록의 **30번 주제**(`inputmode`) · **33번 주제**(`output`).
