...
- **`<div>` 시작 태그가 `<p>` 를 닫는다.** `p` 의 콘텐츠 모델이 구절(phrasing)만 받으므로, 블록이 오면 파서가 **먼저 닫는다**(콘텐츠 모델의 정본은 목록의 **05번 주제**).
...
- 목록의 **05번 주제**(콘텐츠 카테고리와 콘텐츠 모델) — **「무엇이 무엇 안에 올 수 있나」의 정본.**\
  여기는 「**어긴 결과**」까지, 거기는 「**규칙 자체**」
- 목록의 **17번 주제**(표 구조) — `tbody` 삽입이 표 설계에 미치는 영향의 정본
