...
**안 돌려 본 것** — ① **Firefox·Safari 의 접근성 트리**(엔진이 없다). ② **`role` 로 암묵 역할을 덮어쓰는 경우** — 목록의 42번 주제의 표면이라 안 던졌다. ③ **`aria-labelledby` 가 여럿을 가리킬 때의 이름 이어 붙이기** — 43번의 표면이다. ④ **`<form role="search">` 와 `<search>` 를 함께 썼을 때** — 중복 역할이 어떻게 되는지 안 던졌다. ⑤ **`<address>`·`<figure>` 등 구획이 아닌 구조 요소** — 목록의 19번이다. ⑥ **건너뛰기 링크**(목록의 49번).
