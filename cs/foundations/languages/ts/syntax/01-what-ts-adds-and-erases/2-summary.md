...
비용 — 헬퍼 6줄 + 데코레이트 호출. `emitDecoratorMetadata` 까지 켜면 더 붙는다(→ 목록의 **47번 주제**).
...
- **표준 데코레이터(TC39)는 소거 대상이 아니다.** `experimentalDecorators` 없이 쓰면 이 판이 `__esDecorate` 계열 헬퍼를 방출한다 — 레거시보다 **더 많은** 코드가 붙는다. 자세한 비교는 목록의 **47번 주제**.
