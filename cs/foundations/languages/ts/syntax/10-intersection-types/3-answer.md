...
- **조각 수에 따른 검사 시간** — 「더 들어가면」의 마지막 줄. **재지 않았고 수치를 적지 않았다.** [목록의 **45번 주제**](../45-type-level-performance/)에서 잰다.
- **브랜드 타입의 전면 서술** — 3절의 `A & string` 이 줄지 않는다는 성질만 확인했다. 관용구 자체는 [**05번 주제**](../05-structural-typing/)가 정본이다.
- **`Omit` 으로 충돌 키를 빼는 관용구** — 형태만 적었고 던지지 않았다. [목록의 **28번 주제**](../28-utility-types/)에서 던진다.
