...
★ `as unknown as T` 처럼 **두 번 단언해 겹침 검사까지 우회하는 법**과 `!` 는 목록의 **30번 주제**(타입 단언과 non-null `!`)가 정본이다.
...
- 목록의 **30번 주제**(타입 단언과 non-null `!`) — ★★ `as`·`as unknown as`·`!` 의 정본. **여기는 `satisfies` 와 나란히 놓았을 때 `as` 가 무엇을 안 보나**까지.
...
- **`as` 의 겹침 검사를 우회하는 `as unknown as T`** — 목록의 **30번 주제**가 정본이다. **여기서는 안 던졌다.**
