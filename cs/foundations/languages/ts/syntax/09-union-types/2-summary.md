...
- 목록의 **10번 주제**(인터섹션 타입) — `&` 는 그쪽. 여기서는 **함수 매개변수가 교집합이 되는 자리**에서만 스친다.
- 목록의 **11번 주제**(리터럴 타입과 `as const`) — 리터럴 유니온이 넓어지는 자리는 그쪽. 여기서는 **흡수 규칙**까지.
- 목록의 **12번 주제**(좁히기) — `typeof`·`in`·`instanceof` 의 전면 서술은 그쪽. 여기서는 **판별 필드**까지.
- 목록의 **24번 주제**(조건부 타입과 분배) — 유니온이 조건부 타입에서 **분배**되는 규칙은 그쪽.
...
- **판별 칸이 없으면** — `in` 연산자로 가르는 길이 있다(`"fly" in pet`). 목록의 **12번 주제**에서 던진다.
- **리터럴 흡수를 피하는 법** — `"asc" | "desc" | (string & {})` 처럼 적으면 편집기 자동완성은 살리고 흡수는 막는 관용구가 있다. 목록의 **11번 주제**와 엮인다.
- **유니온이 커지면 검사가 느려진다** — 멤버 수에 따라 할당 가능성 판정이 곱으로 늘어난다. 이 배치에서는 **재지 않았다** — 목록의 **45번 주제**에서 잰다.
