...
★★ 템플릿 리터럴 타입 자체의 정본은 목록의 **27번 주제**이고,
`Awaited`·`ReturnType` 같은 표준 도구의 정본은 목록의 **28번 주제**다.
...
검사 비용이 궁금하면 목록의 **45번 주제**(타입 수준 성능)가 그 자리다.
...
| 표준 도구(`ReturnType`·`Awaited`)로 **모자랄** 때 | 표준 도구가 이미 있을 때 — 목록의 **28번 주제** |
...
- 목록의 **27번 주제**(템플릿 리터럴 타입) — 3·4절의 `` `${…}` `` 서식은 그쪽이 정본. 여기는 **`infer` 와 만나는 자리**까지.
- 목록의 **28번 주제**(유틸리티 타입) — `Awaited`·`ReturnType`·`Parameters` 의 정본. 여기는 **그것들이 재귀 조건부라는 사실**까지.
