...
- ★ 11행 — 템플릿 리터럴로 **문자열 키만** 뽑으면 `number` 와 `unique symbol` 이 빠진다.\
  **메서드 이름만 남는다.** 필요한 쪽만 걸러 쓰는 수다([목록의 **27번 주제**](../27-template-literal-types/)).
...
- ★★ 22행 — `#hidden` 은 **JS 의 진짜 private** 이고, 역시 `keyof` 에 **안 나온다**(`"shown"` 뿐).\
  ★ TS 의 `private` 와 JS 의 `#` 는 **소거되느냐 런타임에 강제되느냐**가 다른데([목록의 **32번 주제**](../32-class-type-aspects/)),\
...
| **안 잰 것** | `keyof` 가 **매핑 타입**·**템플릿 리터럴**과 만나는 자리 | [목록의 **26번 주제**](../26-mapped-types/)·**27번 주제** |
...
- [목록의 **26번 주제**](../26-mapped-types/)(매핑 타입) — `in keyof` 로 **키를 돌며 새 타입을 만드는** 자리. 여기는 **읽기**까지만.
- [목록의 **27번 주제**](../27-template-literal-types/)(템플릿 리터럴 타입) — 2절 11행의 `` `${keyof T & string}` `` 가 그쪽 정본.
- [목록의 **28번 주제**](../28-utility-types/)(유틸리티 타입) — `Pick`·`Omit`·`Record` 가 전부 `keyof` 로 만들어진다.
