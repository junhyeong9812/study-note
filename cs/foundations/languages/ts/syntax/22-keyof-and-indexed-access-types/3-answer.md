...
- ★★ 22행 — JS 의 진짜 private 인 `#hidden` 도 **안 보인다.**\
  TS 의 `private` 는 소거되고 `#` 는 런타임에 강제되는데([목록의 **32번 주제**](../32-class-type-aspects/)), **`keyof` 에서는 똑같이 빠진다.**
...
- **매핑 타입**(`in keyof`)·**템플릿 리터럴 키 리매핑** — **안 던졌다.** [목록의 **26번 주제**](../26-mapped-types/)·**27번 주제**.
- **`noUncheckedIndexedAccess`** 를 켠 판 — **안 던졌다.** [목록의 **41번 주제**](../41-index-and-optional-property-strict-flags/).
