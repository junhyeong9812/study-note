...
- ★★★ `declare module "foo" { … }` 는 「**"foo" 라는 모듈이 있다**」는 선언이다 — 대체할 다른 표기가 없으므로 막을 까닭이 없다. 세 판 다 `OK`. 목록의 **37번 주제**가 이 꼴의 본체다.
...
- ★★★ 목록의 **36번 주제** — `import type X = require(…)` 는 **방출 0줄인데 `TS1294`** 다(거기서 던졌다). 「꼴이 `import =` 이면 막는다」가 더 맞는 설명이다.
