...
- [목록의 **29번 주제**](../29-satisfies/)(`satisfies`) — `satisfies` 의 전면 서술은 그쪽. 여기서는 **이 검사를 안 끈다**는 한 칸만.
- [목록의 **30번 주제**](../30-type-assertions-and-non-null/)(타입 단언과 non-null `!`) — `as` 가 무엇을 끄는지는 그쪽.
...
- **이 검사를 진짜 강제로 만들고 싶으면** — 타입만으로는 안 된다. [목록의 **26번 주제**](../26-mapped-types/)(매핑 타입)에서 「선언되지 않은 키는 `never`」를 만드는 수법이 있지만, 그것도 **리터럴 자리에서만** 돈다.
