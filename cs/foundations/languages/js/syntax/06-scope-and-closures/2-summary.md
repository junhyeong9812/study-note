...
> **이어지는 곳** — [목록의 **07번 주제**](../07-this-binding-four-rules/) 「`this` 바인딩 네 규칙」 · [목록의 **08번 주제**](../08-function-forms-and-parameters/) 「함수 정의 형태와 매개변수」
>
...
- ★ **`[5]` 는 같은 일을 하는 다른 도구다.** 클래스의 `#` 필드도 `Object.keys` 에 안 보이고,
  밖에서 읽으면 **`SyntaxError`** 다 — 클로저 쪽은 `undefined` 였는데 이쪽은 파싱 단계에서 죽는다. 정본은 [목록의 **16번 주제**](../16-class-syntax/)다.
