...
> 예: `get` 트랩은 프로퍼티를 읽을 때마다 불린다. 정본은 [목록의 **45번 주제**](../45-proxy/) 「`Proxy`」이고, 여기서는 **로그를 심는 도구**로만 쓴다.
...
- [목록의 **16번 주제**](../16-class-syntax/) 「`class` 문법」 — **그쪽이 필드·정적 멤버·프라이빗 `#` 이 어디에 붙나의 정본**이다.
  여기는 **`extends` 가 체인을 두 줄 만든다**는 사실까지.
- [목록의 **17번 주제**](../17-inheritance-and-super/) 「상속과 `super`」 — **그쪽이 `super`·`new.target`·내장 상속 제약의 정본**이다.
- [목록의 **18번 주제**](../18-for-in-and-enumeration/) 「`for...in` 과 열거」 — **그쪽이 체인 순회 열거의 정본**이다.
- [목록의 **34번 주제**](../34-type-checking-idioms/) 「타입 검사 관용구」 — **그쪽이 어느 검사가 언제 깨지나의 정본**이다.
