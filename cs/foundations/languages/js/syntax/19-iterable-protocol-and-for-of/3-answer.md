...
- **「세 자리의 문」**(`apply`·스프레드·`Array.from`) → [11번](../11-spread-and-rest/2-summary.md) ·
  **제너레이터의 `yield`/`next(값)` 흐름** → 목록의 **20번 주제** ·
  **이터레이터 헬퍼** → 목록의 **21번 주제** ·
  **`for await...of`** → 목록의 **40번 주제** ·
  **`Map` 의 키 비교(SameValueZero)** → 목록의 **23번 주제**.
...
- ★★ **이터레이터 헬퍼의 유무**(`js16b-19f-close-and-helpers.js` 의 `[4]`) — ES2025 를 들인 판에서는 답이 바뀐다. 그때는 목록의 **21번 주제**로 넘어간다.
