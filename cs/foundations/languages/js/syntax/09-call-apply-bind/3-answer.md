...
- ★ **`Reflect.apply` 는 값이 같다.** 다른 점은 **`f.apply` 가 `f` 의 `apply` 프로퍼티를 조회한다**는 것 —
  누가 그것을 바꿔 놨으면 다른 함수가 불린다. `Reflect.apply` 는 그 조회를 건너뛴다. 정본은 [목록의 **46번 주제**](../46-reflect/)다.
...
- ★★ **모듈(ESM)에서 돌리면 엄격 쪽 답이 나온다.** 모듈 코드는 **항상 엄격**이기 때문이다.
  ★ 그래서 `f.call(null)` 의 뜻이 **파일을 모듈로 바꾸는 것만으로 달라진다.** 정본은 [목록의 **42번 주제**](../42-esm-modules/)다.
...
| `Reflect` 전체 | [목록의 **46번 주제**](../46-reflect/) |
| `Object.prototype.toString` 을 타입 검사로 쓰기 | [목록의 **34번 주제**](../34-type-checking-idioms/) |
