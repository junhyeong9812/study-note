...
> **이어지는 곳** — 목록의 **08번 주제** 「함수 정의 형태와 매개변수」 · 목록의 **09번 주제** 「`call`·`apply`·`bind`」
>
...
- ★★ **`[5]` 가 「없는 것이 `this` 만은 아님」을 보인다.** 화살표는 `new` 를 못 받고 `prototype` 프로퍼티도 없다.
  ...
  같은 자리의 **클래스 메서드는 떼면 `undefined`** 다. 정본은 목록의 **16번 주제**다.
...
- 이어지는 곳: 목록의 **08번 주제** 「함수 정의 형태와 매개변수」 — 화살표에 **`arguments` 도 없다**는 것, 그리고 형태별 차이 표의 정본.
- 이어지는 곳: 목록의 **09번 주제** 「`call`·`apply`·`bind`」 — **세 메서드의 API 세부**가 거기다. 여기는 「`this` 를 정한다」까지다.
- 이어지는 곳: 목록의 **16번 주제** 「`class` 문법」 — 클래스 필드 화살표가 어디에 붙나.
