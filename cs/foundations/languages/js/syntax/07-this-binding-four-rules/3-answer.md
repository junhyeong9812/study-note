...
- **정본은 각각 이렇다.**
  **`call`/`apply`/`bind` 의 API 세부** → [목록의 **09번 주제**](../09-call-apply-bind/) · **엄격 모드** → [목록의 **35번 주제**](../35-strict-mode/) ·
  **클래스 필드** → [목록의 **16번 주제**](../16-class-syntax/) · **이벤트 루프** → [목록의 **36번 주제**](../36-event-loop-and-microtasks/) ·
  **브랜드 태그로 타입을 가리는 것** → [목록의 **34번 주제**](../34-type-checking-idioms/).
