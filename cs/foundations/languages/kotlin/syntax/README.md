...
> 실행 검증: **현재 불가**. 이 머신에 `kotlinc` 가 없다(sdkman 후보는 `java`·`maven` 뿐). Gradle(snap)은 있으므로 Kotlin JVM 플러그인을 받아오는 최소 프로젝트로 돌릴 수는 있으나, **도구 설치는 사용자 환경 변경이라 임의로 하지 않는다**. 3파일 단계에서 `sdk install kotlin` 을 제안하고, 그전까지 작성하는 예제는 **「실행 검증 안 됨 — 공식 문서 규칙으로만 접지」** 를 명시한다.
...
|---|------|------|----------------------|------|-----------|------|
| 01 | `val`/`var` 와 기본 타입 — 암묵 수치 변환이 없다 | 문법 | `Int` 를 `Long` 에 그냥 못 넣는 이유와 `val` 이 막는 것·못 막는 것을 설명할 수 있다 | — | [`../언어-특성/README.md`](../언어-특성/README.md) §5 | A |
| 02 | 문자열 템플릿·raw string·멀티달러 보간 (2.2+) | 문법 | `$` 를 문자로 넣어야 할 때 어느 형태를 쓸지 판단할 수 있다 | 01 | — | A |
| 03 | null 안전 타입 — `?`·`?.`·`?:`·`!!` | 문법 | 타입 시스템이 막아 주는 경계와 `!!` 가 그 경계를 버리는 지점을 설명할 수 있다 | 01 | [`../언어-특성/README.md`](../언어-특성/README.md) §2 | A |
| 04 | 스마트 캐스트와 그것이 깨지는 자리 | 문법 | `var` 프로퍼티·다른 모듈 프로퍼티에서 스마트 캐스트가 안 되는 이유를 예측할 수 있다 | 03 | — | A |
| 05 | 플랫폼 타입 — Java 경계에서 null 보증이 끝나는 곳 | 문법 | Java 라이브러리에서 온 값이 왜 런타임에야 터지는지, 무엇으로 막는지 판단할 수 있다 | 03 | [`../언어-특성/README.md`](../언어-특성/README.md) §2·§9 | A |
| 06 | `when` 식 — 주체 있는/없는 형태·guard 조건 (2.2+) | 문법 | `when` 이 식일 때 완결성이 요구되는 조건을 설명할 수 있다 | 01 | — | A |
| 07 | 반복문·`range`·progression·비지역 `break`/`continue` (2.2+) | 문법 | `until`/`..<`/`downTo`/`step` 이 만드는 범위를 예측하고 인라인 람다 안에서 루프를 빠져나올 수 있다 | 01 | — | A |
| 08 | 함수 선언 — 기본 인자·이름 붙인 인자·단일식 함수 | 문법 | 기본 인자가 오버로딩을 어떻게 대체하고 Java 에서 볼 때 무엇이 사라지는지 판단할 수 있다 | 01 | — | A |
| 09 | 가변 인자·spread 연산자·로컬 함수 | 문법 | `vararg` 에 배열을 넘길 때 `*` 가 필요한 이유를 설명할 수 있다 | 08 | — | B |
...
실행 검증이 붙기 전까지 3파일에는 **「Kotlin 2.4.20 문서 기준 · 이 머신에서 실행 검증 안 됨」** 을 명시한다.
