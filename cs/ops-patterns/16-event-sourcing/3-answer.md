...
> 예: 스레드 8개가 50번씩 붙이면 성공 400건·충돌로 거절 146건이 나오고, 거절당한 쪽은 다시 읽고 다시 붙인다.
...
> **접기/재생(fold/replay)** — 빈 상태에서 사건을 순서대로 반영해 지금 상태를 만드는 것.\
> 예: Opened → Deposited(1000) → Withdrawn(5000) 을 차례로 접으면 잔액 -4000 이 된다.
...
> **보상(compensation)** — 08번 사가의 장치 — 롤백 대신 반대되는 일을 새로 하는 것.\
> 예: 실수로 10000을 두 번 입금했으면 지우지 않고 Withdrawn(10000)을 새로 붙인다.
...
> **스냅샷(snapshot)** — 어느 버전까지 접어 둔 중간 상태.\
> 예: 500번째까지 접어 둔 상태를 저장해 놓고 그 뒤 사건만 이어 접는다.
...
> 예: 사건 개수가 줄지 않고 단조 증가해서 개수가 곧 버전이 된다.
...
> 예: 잔액 1000 인 계좌에 "5000 출금해라"가 오면 RejectedException 으로 거절된다.

> **사건(event)** — "출금되었다" — 이미 일어난 일. 과거형이 계약이고 거절할 수 없다.\
> 예: 옛 규칙에서 저장된 Withdrawn(5000) 은 오늘 규칙에 걸려도 재생 때 그대로 접는다.
...
> **sealed interface** — 하위 타입을 코드에 열거해 두는 자바 문법.\
> 예: Opened·Deposited·Withdrawn·Closed 넷 중 switch 가 빠뜨린 종류를 컴파일러가 잡는다.
...
> **펜싱 토큰(fencing token)** — 11번의 장치 — 낡은 소유자의 쓰기를 번호로 막는다.\
> 예: 낡은 버전으로 붙이려 하면 번호가 안 맞아 거절되는 여기의 버전 검사와 같은 일을 한다.
