...
> 예: 블록 5의 내용이 바뀌면 5의 해시가 달라져 6이 담고 있는 앞 해시와 안 맞는다.
...
> 예: `[#3] 앞해시=h2, 해시=h3` 처럼 앞 칸의 해시를 물고 이어진다.
...
> 예: 저장된 hash 자체는 payload 에 안 들어간다(자기 지문을 자기 내용에 넣을 수 없다).
...
> **SHA-256** — 검증된 암호학적 해시 함수.\
> 예: "같은 해시가 나오는 다른 입력을 일부러 만들 수 없다"는 성질이 필요해 가져다 쓴다.
...
> 예: 3번 내용만 고치면 `#3 부터 깨짐`, 해시까지 고치면 `#4 부터 깨짐`이 나온다.
...
> **TamperReport** — 검사 결과 — 정상이거나 "#k 부터 깨짐: 이유".\
> 예: `#3 부터 깨짐: 내용과 해시가 안 맞는다` 처럼 처음 깨진 자리를 알려준다.
...
> 예: 1000칸 사슬의 500번부터 위조하면 해시 501회로 검사를 통과한다.
...
> 예: 난이도 2면 해시가 "00…"으로 시작하는 nonce 를 찾을 때까지 돌린다.
...
> 예: 10칸 만드는 데 난이도 0 은 12회, 난이도 3 은 25,807회가 든다.
...
> **nonce** — 해시 결과를 바꾸려고 돌리는 숫자.\
> 예: 0, 1, 2 … 로 올려가며 조건에 맞는 해시가 나오는 값을 찾는다.
...
> 예: 난이도 0으로 만들어진 제네시스를 생성자 끝에서 다시 만든다.
