...
> 예: A가 임기 1, 뺏어간 B가 임기 2 — 번호가 늘 1이면 둘을 구별할 근거가 없다.
...
> 예: 이 장치가 없으면 리더가 죽어도 "리더가 죽었다"는 이벤트 자체가 안 생긴다.
...
> 예: 밀려난 옛 리더는 steppingDown 목록에 들어가고, 정리를 끝내면 steppedDown 으로 FOLLOWER 가 된다.

- 안 세면: 사고가 **"가끔 배치가 두 번 돈다"로만 나타나고 원인을 못 찾는다.**\
  leaderChanges 는 임기 설정이 맞는지 판단하는 자다.

> **leaderChanges(교체 횟수)** — 리더가 바뀐 횟수. 임기 길이 선택을 조정하는 자다.\
> 예: 박동 1000ms 기준 임기 500ms 면 19회, 1500ms 면 0회다.

- 11번의 **펜싱 토큰**과 정확히 같은 일 — 커지기만 하는 번호로 낡은 놈을 가려낸다.

> **펜싱 토큰(fencing token)** — 11번 락에서 점유마다 커지던 번호. 저장소가 이 번호로 낡은 쓰기를 가려냈다.\
> 예: 여기서는 임기 번호가 같은 일을 한다 — 커지기만 하는 번호로 낡은 놈을 가려낸다.
...
> **resign(사임)** — 스스로 물러나기. 번호는 그대로 두고 자리만 비운다.\
> 예: 되돌리면 옛 리더의 번호와 새 리더의 번호가 겹쳐 구별할 수 없다.
...
> 예: 정확히 leaseMillis 만큼 지난 순간을 `>=` 는 이미 "끊긴" 것으로 본다.
...
> 예: 그래서 응답 없음이 "죽음"인지 "느림"인지 원리적으로 구별되지 않는다.

- 생길 수 있는 것: 느린 리더를 죽었다고 치고 새로 뽑으면 **리더가 둘**이 된다.\
  유일한 대책은 둘이 됐을 때 **피해를 줄이는 것** — 그것이 임기 번호다.

> **split brain(리더 둘)** — 옛 리더와 새 리더가 동시에 자기가 리더라고 믿는 상태. 완전 예방은 불가하고 임기 번호로 피해를 줄인다.\
