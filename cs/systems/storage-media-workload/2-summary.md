...
> **이 문서는 Claude 초안이다(2026-09-14).** 따라 친 원고 없이 포트폴리오 사례(랜덤 액세스 워크로드를 HDD에 두었다가 계층을 나눈 경험)를 일반화해 달라는 요청으로 작성했다. README 규칙상 서머리는 본인 문장이 원칙이므로, 읽고 틀린 곳을 고치거나 본인 문장으로 교체한 뒤 이 줄을 지운다. 수치는 대략의 자릿수이며 세대, 제품에 따라 다르다.
...
[SSD]  기계 부품이 없고 수십 개 die가 병렬로 페이지를 읽는다 -> 위치와 무관하게 수십~수백 us
       -> 랜덤 읽기에서 HDD와의 격차가 가장 크게 벌어진다 (지연 약 100배, IOPS 수백~수천 배)
...
```text
순차 읽기:  헤드 한 번 이동 -> 트랙을 따라 연속 섹터를 그대로 읽음      -> 전송 시간만 든다 (150~280MB/s)
랜덤 읽기:  읽을 때마다 [탐색 수 ms] + [회전 대기 평균 반 바퀴 ~4ms]    -> 4KB 하나에 ~10ms
            7,200rpm 기준 초당 100~200번 정도가 한계 = 100~200 IOPS
```

- 회전 대기는 물리 법칙이다. 7,200rpm이면 한 바퀴 8.3ms, 평균 반 바퀴 약 4.2ms. 어떤 컨트롤러도 이 아래로 못 내린다.
...
```text
랜덤 4KB 읽기:   SATA ~100K IOPS, NVMe 수십만~100만 IOPS   지연 ~50~100us
순차 읽기:       SATA ~550MB/s, NVMe PCIe 4.0 ~7GB/s, 5.0 ~14GB/s
```
...
|---|---|---|---|
| 랜덤 읽기 지연 | ~10ms | ~0.1ms | **약 100배** |
| 랜덤 4KB 읽기 IOPS | 100~200 | 수십만~100만 | 수백~수천 배 |
| 순차 읽기 대역폭 | 150~280MB/s | 3~14GB/s | 10~50배 |
| 용량당 가격 | 1 | 대략 3~6 (시점에 따라 변동, 확인 필요) | |
| 랜덤 쓰기 | 느림 (탐색) | 빠르나 WAF, 수명 비용 | |

읽어야 할 것은 두 가지다.

1. **순차 대역폭 격차(수십 배)보다 랜덤 격차(수백~수천 배)가 훨씬 크다.** 순차 워크로드라면 HDD도 "충분히 빠른" 경우가 많고, 랜덤 워크로드라면 HDD는 자릿수가 다르게 느리다.
2. **HDD의 랜덤 지연 ~10ms는 사람이 체감하는 단위다.** 한 화면을 그리는 데 흩어진 키를 100번 읽으면 1초다. 같은 일을 SSD는 10ms에 끝낸다.
...
- **HAMR / MAMR**: 레이저 열 또는 마이크로파로 기록 지점만 잠시 약하게 만들어 더 작은 비트를 쓴다. 30TB급 이상 대용량이 이걸로 나온다.
- **듀얼 액추에이터**: 암을 두 벌로 나눠 독립 동작시켜 IOPS를 약 2배로 올린다. 그래도 수백 IOPS 수준이다.
- **달라지지 않은 것**: 회전수. 용량은 수십 TB로 갔는데 IOPS는 그대로라 **용량당 IOPS(TB당 초당 몇 번 읽을 수 있나)는 세대마다 나빠진다.** 큰 HDD 하나에 랜덤 워크로드를 몰아넣는 것은 갈수록 더 나쁜 선택이 된다.
...
2. 그 랜덤 읽기를 사람이 기다리는가?
   예  -> SSD. HDD의 ~10ms x 조회 횟수가 그대로 응답 시간이 된다.
   아니오(배치) -> 3으로
...
- 격차는 순차(수십 배)가 아니라 랜덤 읽기(지연 약 100배, IOPS 수백~수천 배)에서 벌어진다.
