...
3. **브라우저가 쿠키를 조용히 버린다.** `Secure` 쿠키는 보안 채널(HTTPS)에서만 저장·전송된다. HTTP 응답이 `Secure` 쿠키를 설정하면 브라우저는 저장하지 않는다.\
   서버 로그에는 로그인 성공·쿠키 발급이 찍히지만, 다음 요청에 쿠키가 없어 "로그인이 유지되지 않음"으로만 보인다. 에러는 없다.
...
5. **쿠키 jar는 브라우저의 것이다.** 서버에서 도는 fetch는 브라우저가 아니므로 사용자의 쿠키 저장소가 없다. BFF가 받은 요청의 쿠키를 **명시적으로 꺼내 헤더로 넘겨야** 백엔드가 사용자를 안다.\
   또 서버 측 fetch가 응답을 캐시하면 사용자별 인증 응답이 다른 사용자의 요청에 재사용될 수 있다 → 인증이 얽힌 호출은 `cache: "no-store"`를 기본으로 한다.
