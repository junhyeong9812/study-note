...
[교정 — 검사와 사용의 링크 의미를 같게]
  ...
  생성      : open(p, "x") = O_CREAT|O_EXCL — 검사+생성이 한 syscall (마지막 성분 TOCTOU 없음)
...
- 존재 검사 후 생성은 경쟁 창이 남는다 → **O_EXCL로 검사와 생성을 한 syscall**에 묶는다. (O_EXCL의 링크 거절은 마지막 성분뿐 — 중간 성분은 포함 검사가 맡는다)
