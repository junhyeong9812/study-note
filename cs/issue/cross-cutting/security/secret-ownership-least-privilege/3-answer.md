...
8. (추가) **명령행은 공개 게시판이다.** 프로세스의 argv는 `/proc/<pid>/cmdline`·`ps`로 같은 호스트의 다른 사용자에게 보인다.\
...
9. (추가) **작업 트리에서 지워도 히스토리에는 남는다.** git 객체는 불변이라, 코드에서 비밀·개인정보를 지운 커밋을 만들어도 과거 커밋에서 그대로 읽힌다.\
   ...
   재작성은 로컬 참조만 바꾸므로, 원격의 **모든 브랜치 참조를 각각** 덮어써야 원격에서도 사라진다 — 한 브랜치만 밀면 다른 원격 브랜치에 남는다.
...
git tag backup/pre-rewrite <old-head>                   # 백업 태그 보존
for b in $(git for-each-ref --format='%(refname:short)' refs/heads); do git push --force origin "$b"; done
# 곁가지: `. ./.env` 는 export 가 아니라 자식 프로세스에 안 감 → set -a; . ./.env; set +a
...
// 고친: 링크는 #token=... (fragment 는 서버로 전송되지 않음)
...
| 4. 히스토리 재작성 | 공개·공유 전, 재작성 합의 가능 | 해시 변경·원격 참조 전수 정리 | 원격 브랜치 하나라도 남기면 잔존 | 이미 커밋된 비밀·개인정보 |
