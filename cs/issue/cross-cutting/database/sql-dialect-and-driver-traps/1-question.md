...
## 질문
1. `UNIQUE (parent_id, name)` 제약을 걸었는데 `parent_id` 가 NULL 인 루트 행은 같은 이름으로 여러 개 들어간다. SQL 표준에서 NULL 끼리의 비교는 어떤 결과이며, 그래서 UNIQUE 가 무엇을 못 막는가. 대안은?
...
6. 경계: 다음 세 가지는 "SQL 이 쓴 대로 동작하지 않는" 엔진 의미다 — 지연(DEFERRABLE) 제약 위반이 언제 터지는가 / 마이그레이션 `UPDATE` 가 `updated_at` 트리거를 발화시키는가 / REPLACE 계열 upsert 가 FK `ON DELETE CASCADE` 자식에게 무엇을 하는가. `SET FOREIGN_KEY_CHECKS=0` 은 트랜잭션 롤백으로 원복되는가?
