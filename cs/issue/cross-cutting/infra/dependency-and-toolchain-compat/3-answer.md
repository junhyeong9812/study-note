...
2. **API 버전이 협상되고, 로거 바인딩이 없어 진단 로그가 버려졌다.**\
   ...
   > **API 버전 협상** — 클라이언트와 서버가 서로 지원하는 프로토콜 버전을 맞추는 과정. 서버의 최소 요구가 오르면 구 클라이언트는 연결 자체를 거부당한다.

3. **shade된 사본은 다른 패키지 이름으로 재배치된 복사본이라, 외부 의존성·설정이 그 클래스를 가리키지 않는다.**\
   외부에서 같은 라이브러리 새 버전을 선언해도 그건 원래 패키지명의 별개 클래스다.\
   환경변수·시스템 프로퍼티·설정 파일 여섯 가지 시도가 모두 무효였다 — 내장 사본의 하드코딩 기본값에 닿지 않았다.\
   남는 대응: 그 라이브러리 **자체**를 업그레이드한다(부모 BOM이 버전을 묶으면 명시 버전으로 override).\
   함정: 메이저 업그레이드에서 artifactId가 바뀌어 옛 이름은 "artifact 없음"이 됐다.
   > **shade** — 의존성을 자기 jar 안에 복사하고 패키지명을 바꿔(relocate) 충돌을 피하는 빌드 기법.
...
5. **서로에게 위임하면 무한 루프가 되기 때문이다.**\
   ...
   둘이 함께 있으면 로그 한 줄이 두 브리지 사이를 끝없이 돈다 — 그래서 프레임워크가 기동 시점에 명시적으로 거부한다.\
   교정: 쓸 백엔드를 하나 정하고, 전이로 들어오는 반대 방향 브리지를 모든 공급 경로에서 `exclusion`한다.

6. **peer는 "내가 설치하지 않고, 호스트 프로젝트가 이 범위로 갖고 있어야 한다"는 요구다.**\
   npm 7부터 peer를 자동 설치·엄격 검증해, 선언된 peer 범위를 벗어난 패키지가 하나라도 있으면 트리 전체 조작(uninstall 포함)이 거부된다.\
...
mvn dependency:tree | grep -icE 'orm-core'   # = 0 이어야 완료
```
...
```
깨진 것: 변조된 wrapper면 빌드가 곧 임의 코드 실행이다.\
...
RestClient client = RestClient.builder()
    .requestFactory(new JdkClientHttpRequestFactory())   // 기본: HTTP/2(h2c 업그레이드) 시도
    .build();
// 상대 서버는 HTTP/1.1 전용 → 400 "Invalid HTTP request" → 게이트웨이 502
```
② 고친 코드
```java
...
```
깨진 것: 클라이언트 기본 협상을 서버가 이해하지 못해 요청 자체가 malformed로 거부됐다.\
...
| 2 프로토콜 고정 | 클라이언트 기본값이 상대 지원 범위 밖 | HTTP/2 이점 포기 | 400/502, 원인 오진 | 상대가 HTTP/1.1 전용 서버 |
