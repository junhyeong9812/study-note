...
1. **컨테이너 프로세스는 (따로 덮어쓰지 않으면) 이미지가 정한 uid로 돌기 때문이다.**\
   `sudo`는 compose(와 데몬에 요청하는 클라이언트)를 root로 실행할 뿐, 컨테이너 안 프로세스의 uid는 이미지의 `USER`(여기선 1000)다 — compose `user:`로 덮거나 엔트리포인트가 사용자를 바꾸면 달라지고, userns-remap·rootless 환경이면 호스트 쪽 uid 매핑도 달라진다.\
   ...
   함정: NFS `root_squash`면 클라이언트 root의 chown이 막히고, CIFS는 (Unix 확장이 없으면) chown이 안 돼 마운트 옵션(`uid=`·`gid=`)으로 소유자를 지정한다.
   > **effective uid** — 커널이 권한 검사에 쓰는 프로세스의 사용자 ID. real uid(프로세스를 시작한 사용자)와 구분되며 setuid 등으로 달라질 수 있다(리눅스 파일 접근은 보통 euid와 같은 fsuid로 판정).
...
3. **권한 거부도 그만큼 빠르기 때문이다.**\
   ...
   빈 본문(`Content-Length: 0`) 404는 프레임워크의 기본 "핸들러 없음" 응답과 모양이 달라, 핸들러가 존재하고 그 안의 판정(파일 제공 가능?)이 실패했다는 신호로 읽을 수 있었다 — 기본 404의 모양은 스택마다 다르니 실제 기본 응답과 비교해 확인한다.\
...
6. **일부 파일만 새 버전인, 반쯤 갱신된 상태로 남는다.**\
   git은 호출 사용자로 파일을 쓰는데, 파일 교체(unlink·생성)는 파일 소유가 아니라 **그 파일이 든 디렉터리의 쓰기 권한**으로 판정된다 — 다른 계정이 만든 디렉터리 안의 파일을 교체하지 못하면 pull이 중간에 멈춘다.\
