...
4. (왜) 수정 `-A "wiki-sync/1.0"`이 통과하는 이유는 — `~*curl/` 정규식이 무엇만 잡길래 이 UA는 안 걸리는가. 함께 붙인 `--http1.1`은 무엇을 고쳤는가.
5. (일반화) rate limit·봇 UA 차단 같은 **광역(broadband) 규칙**이 자기 자동화를 오사하기 쉬운 구조적 이유는? 규칙 설계 단계에서 "아군 식별"을 어떻게 넣는가.
6. (경계) UA 기반 아군 식별의 약점은 — 스캐너가 `wiki-sync/1.0`을 흉내 낼 수 있는데도 왜 실용적인가. 더 강한 식별(HMAC webhook 등)이 남는 이유는.
