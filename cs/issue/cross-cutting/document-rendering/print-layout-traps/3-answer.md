...
3. **fixed 요소는 쪽의 여백 박스 밖에 그려져 클립된다.**\
   인쇄 조각화(fragmentation)에서 `position: fixed`는 매 쪽 반복되지만 `@page` 여백 영역 밖 좌표에 놓여 잘리고, 텍스트 추출도 0건이 됐다.\
   쪽 번호는 `@page { margin: ...; @bottom-right { content: counter(page) } }` 같은 **margin box**로 넣는다.
...
4. **같지 않다 — 다단 컨테이너가 생긴다. 큰 블록의 avoid는 큰 공백을 만든다.**\
   ...
   큰 블록에 `break-inside: avoid`를 걸면 블록이 한 쪽에 안 들어갈 때 통째로 다음 쪽으로 밀려 앞 쪽 하단 절반이 비었다.\
...
5. **height가 원본 비율로 상한 없이 계산되고, 셀에는 기본 여백이 있기 때문이다.**\
   ...
   워드 표 셀에는 기본 좌우 여백(약 108 dxa)이 있어 셀 폭 = 이미지가 쓸 수 있는 폭이 아니다.\
...
6. **offset 0(셀 좌상단 고정), 그리고 워드가 열 폭을 자체 계산한다.**\
   ...
   표에 열 격자(`tblGrid`)가 없으면 워드가 폭을 다시 계산해 지정과 다르게 나온다 — 격자를 명시하고, 셀 폭은 기존 값을 지운 뒤 다시 설정한다(중복 방지).
...
```css
.page   { height: 297mm; padding-bottom: 14mm; position: relative; }
.footer { position: absolute; bottom: 6mm; border-top: 1px solid; }  /* 구분선 < 본문 하한 */
