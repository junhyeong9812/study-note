...
5. **pid 재사용.** 시그널 직전 `(pid, 시작 시각)`을 기록값과 대조해 같은 프로세스인지 재확인한다(시작 시각은 살아 있을 때 미리 기록).\
   ...
   검사~시그널 사이 재사용 창은 pidfd 없이 user space에서 완전히 닫을 수 없어 남은 창으로 명시한다.
...
6. **txId 리셋.** WAL 복구는 로그의 txId로 "이 트랜잭션이 커밋됐나"를 판정한다.\
   ...
   카운터는 기동 시 로그를 스캔해 최대 txId를 구하고 `max+1`부터 시작한다(후속 과제로 체크포인트에 최대 txId를 기록해 전체 스캔 의존을 없앤다).
   > **WAL(Write-Ahead Log)** — 데이터 변경 전에 로그를 먼저 기록해, 장애 후 로그 재생으로 커밋된 변경을 복구하는 방식.
