...
1. **thread-affine 객체.** 특정 스레드(또는 이벤트 루프) 안에서만 올바르게 동작하도록 만들어진 객체다.\
   자기 I/O 구동 태스크를 `spawn_local`로 현재 스레드의 로컬 실행기에 올리면, 그 태스크와 공유하는 상태가 스레드 간 이동을 전제하지 않는다 — 그래서 타입이 `!Send`가 되어 다른 스레드로 옮길 수 없다.
   > **`Send` / `!Send`** — Rust에서 값을 다른 스레드로 옮겨도 안전하면 `Send`, 아니면 `!Send`. 컴파일러가 스레드 경계에서 검사한다.

2. **예측.** 요청 핸들러가 임의 스레드에서 실행되면 핸들러가 붙잡는 상태는 `Send`여야 하므로 **컴파일이 Send 위반으로 막힌다**.\
   억지로 멀티스레드 런타임 위에서 `spawn_local`을 호출하면 로컬 실행기(`LocalSet`) 밖이라 **런타임 패닉**이 난다.
...
4. **asyncio 교차 루프.** asyncio 객체(`Queue`·Future 등)는 자기 루프 스레드에서만 조작된다고 가정하고 락 없이 구현돼 있다.\
   ...
   합법 진입점은 대상 루프의 `loop.call_soon_threadsafe(fn, ...)` — 그 루프가 자기 차례에 `fn`을 실행하게 예약한다. 그래서 구독 시점에 큐와 함께 **그 큐의 루프**를 저장해 둔다.
