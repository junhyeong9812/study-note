...
- 로컬 태스크로 I/O를 돌리는 연결은 `!Send` — 임의 스레드에서 직접 쓰면 컴파일이 막히고, 멀티스레드 런타임의 `spawn_local`은 패닉한다.
- asyncio 객체는 자기 루프 스레드에서만 안전하다 — 다른 루프에서는 `loop.call_soon_threadsafe`가 유일한 합법 진입점.
