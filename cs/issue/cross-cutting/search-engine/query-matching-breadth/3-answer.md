...
"○○주식회사"가 `○○`·`주식`·`회사`로 쪼개지면, `주식`이나 `회사` 하나만 있어도 매칭되어 거의 전체(약 300만 건)가 나왔다(병음 두 음절 질의도 235만 건).\
...
`match_phrase`는 모든 토큰이 **같은 순서로 붙어서** 나타나야 매칭한다 — 병음 두 음절 질의가 235만 → 3,166건이 됐다.\
단일 토큰 질의에서는 순서·인접 조건이 의미가 없어 `match`와 결과가 같다(실측으로 동일 건수 확인) — 그래서 다단어에서만 동작이 바뀌고 단일어 사용자는 영향이 없는 **안전한 변경**이다.\
...
반대로 **여러 토큰짜리 질의를 `fuzzy`에 통째로** 넣으면 질의 전체가 한 term이 되어 색인 토큰과 편집거리가 너무 멀어 매치 불가가 되고, 1음절 term 단독 fuzzy는 거리 1 안의 후보가 폭증해 광역(수천 건)이 된다 — 이 경우는 `match`(분석 후 토큰별 fuzzy)가 맞다.\
어느 쪽이든 "대체만 허용하는 fuzzy"는 엔진에 없어, 본질적 해결은 색인 쪽 변형 보강이라는 결론이었다.
...
교정: 해당 분기에서 ngram 호출을 빼고 **자모 시퀀스 1토큰 필드의 정확 match**(퍼지 없음)로 대체 → 음운이 정확히 맞는 후보 249건; 이후 매핑도 "ngram은 정규화된 필드에만"으로 정리.

5. **위치 없는 자질 합집합 = 부분집합 조건이 거의 항상 참.**\
문서 배열이 여러 발음 후보의 자질을 합친 **집합**이면, 음절 위치마다 거의 모든 자질이 어딘가에 들어 있다.\
그래서 "검색어 자질이 모두 문서에 있다"(100% AND)는 조건이 거의 항상 참이 되어, 음절 수와 무관하게 **prefix 매칭처럼** 동작했다(두 음절 질의에 7,720건).\
자질이 "몇 번째 음절의 자질인지"라는 위치 정보를 잃었기 때문이다 — 자질 매칭이 의미를 가지려면 **단일 발음만** 색인하거나 **음절 위치를 별도 차원**으로 분리해야 한다.\
...
| 단어 경계를 지키는 부분일치 | letter ngram(음절 조각) | 정규화 필드 ngram / 자모 1토큰 정확 match |
| 위치가 의미인 자질 매칭 | 후보 자질 합집합 배열 | 단일 발음 or 위치 차원 분리 |
...
```python
q = bool_(should=[
    match("owner_name", "OO주식회사"),                 # OO OR 주식 OR 회사 → ~300만 건
    wildcard("owner_name.keyword", "*OO주식회사*"),     # 0건
], minimum_should_match=1)                             # 넓은 쪽이 그대로 결과
```
② 고친 코드
```python
q = bool_(should=[
    match_phrase("owner_name", "OO주식회사", boost=10.0),   # 순서·인접 유지 (단일어는 결과 동일)
    wildcard("owner_name.keyword", "*OO주식회사*"),
...
q.should(match("name_kor", kor_query))                      # 복합어 두 토큰 → 흔한 뒤 토큰 OR 노이즈
...
q.should(match("name_kor", kor_query, operator="and", boost=3.0))
...
① 문제 코드
```python
doc["features"] = sorted(set(f for cand in pronunciation_candidates for f in encode(cand)))   # 합집합
q.must(*[term("features", f) for f in encode_flat(query)])       # 검색어 자질 전부 AND
# → 부분집합 조건이 거의 항상 참 → prefix처럼 수천 건
```
② 고친 코드
```python
# 변경 롤백 + 자질 필드 제거, 역할 분리 필드
doc["name_variants_exact"] = cartesian_variants(tokens)     # 다단어 정확매치용 keyword
doc["name_variants_flat"]  = flatten_variants(tokens)       # 토큰별 후보의 순서 보존 dedup 합집합 (곱 아닌 합)
```
