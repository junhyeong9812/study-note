...
이것을 다중값 필드에 넣고 여러 n-gram 서브필드로 분석하면 문서당 term 수가 입력 길이에 지수적으로 커져, 인덱스가 수배(한 사례 약 3배, 다른 사례 100GB → cap 적용 후 15GB)로 불고 워커 메모리가 수십 MB에서 GB 단위로 튄다.\
...
분석기 서브필드를 늘리자 같은 배치의 색인 메모리가 커져 상한(3.3GB)을 넘었다.\
...
breaker가 없거나 넉넉하면 대신 힙 자체가 넘쳐 노드가 죽고(샤드 일시 RED, bulk 대량 취소), 색인은 수십만 건 부분 실패로 끝난다.
...
힙을 30GB로 잡은 것은 JVM의 **compressed oops 임계(32GB) 미만**을 유지하기 위해서다 — 넘으면 객체 포인터가 커져 같은 힙에서 쓸 수 있는 공간이 오히려 줄어든다.\
...
wildcard(특히 앞쪽 `*`)는 용어 사전을 순회해 사실상 전체 term 스캔이 되어 평균 10ms → 297ms가 됐고, **색인 시점**에 앞·뒤 edge n-gram을 만들어 쿼리를 단순 term 조회로 바꿨다 — 비용을 **쿼리 시점에서 색인 시점(저장 공간)으로** 옮긴 것이다.
...
doc["name_variants"] = variants          # 다중값 × n-gram 서브필드 여럿
```
② 고친 코드
```python
...
doc["name_variants"] = variants                            # keyword만
doc["name_variants_top5"] = variants[:5]                   # 분석기 서브필드는 상위 5개에만
```
...
같은 구조: 다국어 발음 필드를 전 언어로 만들던 것을 문서의 원 언어 + 보조 언어로 제한.
...
```yaml
environment:
  - ES_JAVA_OPTS=-Xms30g -Xmx30g        # 32GB 미만 (compressed oops)
# 재생성 순서: 엔진 → green → 앱 (앱 커넥션 풀의 죽은 연결 방지)
```
무엇이 깨졌나: 클라이언트가 서버의 메모리·큐·처리 시간 한도를 모른 채 부피와 동시성을 정했다.\
같은 구조: 대량 색인 중 힙 부족으로 약 77만 건 중 53만 건 실패 → 인덱스 삭제 후 힙 상향해 재실행.\
