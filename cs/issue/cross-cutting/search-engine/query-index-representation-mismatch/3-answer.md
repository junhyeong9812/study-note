...
7. **dead 필드와 로컬 등가 복제.**\
질의측은 `"foo bar"`를 `foobar` 한 덩어리로 만들어 term 질의하는데, 인덱스는 standard로 `foo`·`bar`를 나눈 뒤 **단어별** n-gram만 저장했으므로 `foobar` 토큰은 존재하지 않는다 — **공백을 포함한 검색어 경로**에서 그 필드는 원래 문서(`foo bar`)의 단어 경계를 가로지르는 토큰이 없어 그 문서에 매칭되지 않고 저장 비용만 남는다(같은 정규화로 만든 `_norm` 필드의 n-gram만 매칭). 역으로 `foobar`를 한 단어로 가진 **다른** 문서의 n-gram에는 맞을 수 있어, 의도와 다른 문서가 걸리는 오매칭도 생긴다.\
