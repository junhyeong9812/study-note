...
`asciifolding`이 먼저 `ü → u`로 바꾸면 뒤의 독일어 발음 변환 필터는 `Munchen`을 받아 `문헨`으로 색인하고, 한글 `뮌헨` 검색이 맞지 않는다.\
...
그래서 char_filter의 매핑은 **원문 대소문자 그대로**를 본다 — `rn => m`은 `RN`에 걸리지 않는다.\
패턴 치환으로 소문자화하는 방법은 복잡해서, **매핑에 대소문자 조합을 모두 나열**하는 쪽을 택했다(최대 수십 개라 부담이 없다).\
참고로 mapping char_filter는 긴 패턴을 우선 매칭한다(`rn`이 `r`보다 먼저).

3. **원문 통과 vs 기본값 덮어쓰기.**\
(a) 모르는 문자를 **그대로 출력에 붙이면**(pass-through) 한글은 한글로 남아, 색인된 한글과 여전히 매칭될 수 있다.\
(b) 모르는 문자를 **기본 자음으로 처리**하면(default 분기 → `ㅇ`) 숫자·한글·기호가 모두 `ㅇ + ㅡ = 으`로 바뀐다 — 숫자 `004`도 한글 세 글자도 `으으으`가 된다.\
(b)가 에러 없이 토큰을 파괴한다: 숫자 문서끼리 같은 쓰레기 토큰을 공유해 유사도 상위를 오염시키고, 한글 쿼리는 색인된 한글과 매칭되지 않는다.\
여기에 매핑에 `search_analyzer`가 없어 색인용 변환 분석기가 **쿼리에도** 적용되자, 이미 한글인 쿼리가 한 번 더 변환돼 망가졌다.
...
상속 템플릿의 진입 메서드에 가드를 넣어도, 서브클래스가 그 메서드를 **오버라이드**하면 템플릿을 우회해 가드가 적용되지 않는다 — 오버라이드한 서브클래스 네 곳에 가드를 명시로 추가했다.\
...
쉼표로 결합된 다중 이름을 **공백으로만** split해 "태국문자+쉼표+영어"가 한 토큰으로 남았고, 태국 문자만 변환하는 발음 변환기가 라틴을 남겨 "한글+쉼표+영어"가 커스텀 필터에 들어갔다 — 이 조합이 필터의 인덱스 계산을 깨뜨렸다.\
최소 재현을 조합으로 좁혔다: 한글+쉼표 OK, 한글+영어 OK, **한글+쉼표+영어만 THROW**.\
...
방법 1: 사전 기반 분절기(`icu_tokenizer`) 서브필드 + `match_phrase`(연속 위치) — 단어 경계를 지키는 부분일치(과매칭 제거 수치: 약 9,200 → 8,800 태깅).\
...
7. **각 단계가 전제하는 입력 형태.**\
동의어 필터는 **단일 토큰 경로**를 전제하는데, `decompound_mode: mixed`는 복합어를 원형 + 분해형의 여러 경로(그래프)로 내보내 다중 단어 동의어 빌드가 실패했다 → `discard`로 단일 경로화.\
규칙 파일 파서는 **UTF-8·LF 텍스트**를 전제하는데, BOM과 CRLF가 규칙 문자열에 섞여 해석이 실패했다 → 이미지 빌드 시 `dos2unix`·`iconv` + 로케일 지정.\
편집거리 fuzziness는 **문자(코드포인트) 단위**를 전제하는데, 한글은 자모가 결합된 음절이 한 문자라 "블랙 → 블렉" 같은 자모 한 개 차이가 음절 치환으로 잡혀 교정이 약하다 → edge n-gram(2~4)으로 보완하고 fuzziness는 영문 필드에만 적용하거나 1로 제한했다.\
...
① 문제 코드
```json
"analyzer": { "pron_de": { "tokenizer": "standard",
  "filter": ["lowercase", "asciifolding", "pron_g2p_de"] } }        // ü → u 후 변환 → 틀린 발음
"char_filter": { "visual": { "type": "mapping", "mappings": ["rn => m", "d => b"] } }   // 대문자 미적용
```
② 고친 코드
```json
"filter": { "folding_keep": { "type": "asciifolding", "preserve_original": true } },
"analyzer": { "pron_de": { "tokenizer": "standard",
  "filter": ["lowercase", "folding_keep", "pron_g2p_de"] } }        // 원문 + 폴딩 두 토큰, 같은 position
"char_filter": { "visual": { "type": "mapping",
  "mappings": ["rn => m", "RN => m", "Rn => m", "rN => m", "d => b", "D => b"] } }
```
...
String transliterate(String in) {
    for (char c : in.toCharArray()) {
        int cho = consonantOf(c);              // 모르는 문자 → default: 기본 자음 ㅇ
        out.append(compose(cho, VOWEL_EU));    // '0' → "으"
    }
}
```
```json
"name_pron": { "type": "text", "analyzer": "pron_x" }     // search_analyzer 없음 → 쿼리도 변환
```
② 고친 코드
```java
String transliterate(String in) {
...
for (String name : raw.split("\\s+")) {          // "ชื่อ,Brand" 가 한 토큰으로 남음
    index(partialTransliterate(name));            // 대상 문자만 변환 → "한글,Brand" → 필터에서 THROW
}
```
② 고친 코드
```java
...
```
```java
String field = containsHan(q) ? "desc_native" : "desc_en";      // 스크립트 판정 라우팅
