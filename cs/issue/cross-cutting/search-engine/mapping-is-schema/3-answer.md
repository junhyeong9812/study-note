...
분석기를 바꾸면 이미 저장된 토큰과 새 분석 규칙이 섞여 같은 필드 안에서 의미가 달라지므로, 엔진은 기존 필드의 (색인) 분석기·타입 변경을 `PUT _mapping`에서 거부한다.\
단 "고정"은 기존 필드의 색인 표현에 대한 것이다 — 새 필드·서브필드 추가, `search_analyzer`·`ignore_above` 같은 일부 파라미터 변경은 제자리에서 된다(새 서브필드는 이후 색인되는 문서부터만 채워진다).\
...
새 인덱스는 신 매핑을 처음부터 적용받고, 재색인은 모든 문서를 신 분석기로 다시 토큰화하며, 누락 보강(빠진 문서 탐색 → 추가 이관)과 표본 `_source` 검증 뒤 alias를 새 인덱스로 옮겨 **읽는 쪽 코드 변경 없이** 원자적으로 전환한다(`_aliases`의 remove/add는 한 요청 안에서 원자적).\
재색인은 시작 시점의 스냅샷을 복사하므로, 그동안 옛 인덱스에 들어온 쓰기는 이중 쓰기나 스왑 전 재동기화로 따로 따라잡아야 한다 — "누락 보강" 단계가 이 역할이다.\
...
4. **포맷 밖 값은 문서를 깨고, 동적 매핑은 첫 값으로 정한다.**\
엄격 타입 매핑은 파싱할 수 없는 값을 문서 단위로 거부한다(`ignore_malformed`를 켜면 그 필드만 버리고 문서는 받는다) — 구분자 없는 `20240115`가 선언 포맷과 달라 `failed to parse field ... of type [date]` 400이 났다.\
bulk 요청 자체는 항목별로 성공·실패를 돌려주므로 "한 건이 bulk 전체를 깬다"는 아니다 — 이 사례에선 그 형식이 원천 전반에 있어 첫 bulk부터 사실상 전량 거부되어 처리 0건으로 작업이 실패했다(클라이언트가 `errors: true`를 전체 실패로 처리하는 경우도 같은 결과가 된다).\
...
코드가 필드 구조를 바꿔(예: 한 필드를 두 언어 필드로 분리) 새 필드를 쓰기 시작했는데 인덱스가 옛 매핑이면 **새 필드를 쓰는 문서가 모두 거부**된다(모든 문서가 새 필드를 쓰면 전량) — 매핑은 코드와 같은 버전이어야 한다.
...
# 무중단이 필요하면 DELETE 대신 변형 A(새 인덱스 + alias 스왑)
```
...
// 원천에 "20240115" → 그 문서 거부 (원천 전반이 이 형식 → bulk 항목 전량 실패)
