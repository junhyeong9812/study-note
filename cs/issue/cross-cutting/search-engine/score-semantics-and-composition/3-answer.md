...
벡터 근사 인덱스(다른 방안)는 더 극단적이었다: PQ 양자화는 벡터를 압축해 **근사 점수가 실제보다 낮아지므로**, 0.85라는 절대 임계를 어떤 쌍도 넘지 못해 결과(edges)가 **에러 없이 0**이 됐다.\
여기서는 임계를 옮기지 않고 **점수를 왜곡하지 않는 인덱스로 교체**했다(아래 방안 비교).
...
index = IndexIVFPQ(d, nlist, m, nbits)       # PQ 양자화 → 근사 내적이 실제보다 낮음 → edges = 0 (에러 없음)
...
index.nprobe = 128                            # 정확 검색 대비 edge 0.02% 차이, 5배 빠름
