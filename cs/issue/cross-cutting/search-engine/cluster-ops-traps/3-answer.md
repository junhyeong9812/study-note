...
그러나 공간을 조금 비웠다고 블록이 풀리지는 않는다 — 최근 버전(ES 7.4+ 등)은 사용률이 flood_stage가 아니라 **high 워터마크 아래로** 내려가야 자동 해제하고, 구버전은 자동 해제가 없어 인덱스 설정으로 블록을 **명시적으로 해제**해야 한다. 이 사례에서도 명시적 해제가 필요했다.\
보호는 "위험 감지 즉시" 걸어야 안전하지만, 해제는 경계에서 걸림·풀림이 반복되지 않도록 더 여유 있는 조건(또는 사람의 확인)을 요구하므로 비대칭이다.\
...
병합이 못 따라가 세그먼트가 100개를 훌쩍 넘고 크기도 불균형해지자, 검색이 세그먼트를 하나씩 순회하는 비용과 병합 작업의 자원 경합이 커져 다중조건 검색이 수 초를 넘었다(두 비용 경로는 기록상 가설로 제시된 메커니즘 — 관측은 세그먼트 수·지연뿐).\
그런데 테스트 복제본에서 force merge로 단일 초대형 세그먼트를 만들자 **역효과**가 났다 — force merge는 병합 정책의 `max_merged_segment`(세그먼트 크기 상한)를 따르지 않는다.\
일반론으로 force merge는 더 이상 쓰지 않는 읽기 전용 인덱스에는 검색을 빠르게 하는 경우가 많고, 쓰기가 계속되는 인덱스에는 권장되지 않는다(상한을 넘은 초대형 세그먼트는 이후 정상 병합 대상에서 빠져 삭제 문서가 쌓이고, 세그먼트 단위 병렬 검색을 쓰는 버전에선 병렬성도 줄어든다) — 이 사례의 역효과는 그런 조건에서의 관측이다.\
교정은 세그먼트 크기 상한을 정책 기본값(5GB)으로 재지정해 재색인하고, 남은 병목인 정렬을 기본 검색 경로에서 분리한 것이다(정렬은 명시 요청·점수가 의미 있을 때만, 대량 결과는 정렬 차단) → 지연이 수 분의 1로 줄었다.
...
그 버전에서는 샤드 안의 세그먼트가 직렬로 처리되므로, 단일 쿼리의 검색 병렬성을 올릴 주된 레버는 **샤드 수**였다(데이터 규모별로 샤드 수를 달리 정함 — 샤드를 늘리면 샤드별 오버헤드도 늘어 무한정 올릴 수는 없다).\
...
기록된 부수 함정도 같은 계열이다: 인덱스 split은 (파일시스템이 지원하면) 하드링크라 빠르지만, 이후 삭제 문서 정리(expunge)는 삭제 문서가 있는 세그먼트를 다시 쓰는데 split 직후엔 사실상 모든 세그먼트가 대상이라 디스크를 크게 쓴다. 색인이 도는 중에 측정 쿼리를 섞으면 수치가 오염된다.
...
PUT _all/_settings { "index.blocks.read_only_allow_delete": null }                          # 블록 명시 해제 (자동 해제 안 되는 버전·조건일 때)
PUT /my-index/_settings { "number_of_replicas": 0 }
```
무엇이 깨졌나: 보호 블록은 자동으로 걸렸는데 해제 조건은 충족되지 않았고, replica 배치 규칙상 놓을 노드가 없었다.\
같은 구조: 디스크 100%로 샤드 복구가 `No space left on device` 실패 → primary 미할당 RED → 데이터 경로를 여유 있는 디스크로 되돌려 해소.

### 변형 B — 세그먼트 파편화 vs 과도 병합
① 문제 코드
```http
# 세그먼트 100개 초과·크기 불균형 → 검색 지연
