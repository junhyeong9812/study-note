...
2. **rename이 원자적인 이유와 전제.** POSIX 파일시스템에서 `rename(2)`는 디렉터리 엔트리 하나가 가리키는 inode를 한 번에 바꾼다 — 관찰자는 옛 파일 전체 아니면 새 파일 전체만 본다.\
...
4. **디렉터리 교체의 파괴 창.** 비어 있지 않은 디렉터리 위로의 `rename`은 `ENOTEMPTY`/`EEXIST`로 실패하므로 한 번에 덮어쓸 수 없다(리눅스는 `renameat2(RENAME_EXCHANGE)`로 두 경로를 원자 교환하는 길도 있다 — 커널·파일시스템 지원 필요).\
"옛 폴더 삭제 → 새 폴더 rename"은 두 동작 사이에 **대상이 아예 없는 창**을 만든다 — 그 사이 rename이 실패하거나 크래시하면 정상본만 사라진다.\
...
6. **ETXTBSY.** 리눅스는 (전통적으로 — 커널 버전에 따라 차이가 있을 수 있음) `exec`로 실행 중인 실행 파일을 **쓰기 모드로 여는 것**을 `ETXTBSY`로 거부한다 — `cp`로 덮어쓰기는 기존 inode에 쓰는 것이라 막힌다.\
...
파일 `fsync`는 그 파일(데이터·inode 메타)만 내구화하고, **새 디렉터리 엔트리(파일 생성·rename)**는 부모 디렉터리를 `fsync`해야 크래시 후에도 남는다(보장 수준은 파일시스템·마운트 옵션마다 다르다 — 이식 가능하게 쓰려면 둘 다 한다).\
교체 쓰기라면 순서도 중요하다: temp 내용 `fsync` → `rename` → 부모 디렉터리 `fsync` — temp를 fsync하지 않고 rename하면 크래시 후 이름은 새 파일인데 내용은 비었거나 잘린 파일이 남을 수 있다.\
append-only 저널은 크래시 직후 마지막 레코드가 부분 기록(torn tail)일 수 있다 — 레코드마다 개행으로 끝나게 쓰는 형식이라면 정상 저널은 개행으로 끝나므로 **개행 없는 마지막 줄만 버리고**, 중간 줄이 깨졌으면 복구 불가로 보고 기동을 거부한다.\
...
② 고친 코드
```python
tmp = path + '.tmp'                 # 단일 writer 전제 — 동시 writer가 있으면 변형 B의 유니크 이름
with open(tmp, 'w') as f:
    write_rows(f, rows)
    # 크래시 내구성까지 필요하면: f.flush(); os.fsync(f.fileno())  (+ replace 후 부모 디렉터리 fsync — 방안 6)
...
남은 창: 재확인~rename 사이 TOCTOU는 남는다(`rename`은 기존 대상이 파일이거나 빈 디렉터리면 조용히 교체한다 — no-clobber는 `RENAME_NOREPLACE` 류가 필요, 단일 사용자 위협모델이라 주석으로 수용).\
...
# 또는: rm -f "$INSTALLED" && cp new-app "$INSTALLED"             # 다음 재시작부터 적용 (rm~cp 사이엔 파일 부재 창)
```
