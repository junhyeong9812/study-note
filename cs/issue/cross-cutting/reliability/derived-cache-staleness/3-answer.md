...
이것은 read-your-writes 보장이 없는 읽기와 같은 결이다 — 방금 일어난 사실을 같은 흐름의 다음 판정이 못 본다(엄밀히는 쓰는 주체와 읽는 주체가 달라 "쓰기 가시성 지연"이 더 정확한 이름이다).\
...
func (ks *KeySet) Get(kid string) (*Key, error) {
    ...
    k, ok := ks.keys[kid]
    if !ok { return nil, ErrUnknownKid }                          // 없는 kid를 (nil, nil)로 돌려주지 않음
    return k, nil
}
```
