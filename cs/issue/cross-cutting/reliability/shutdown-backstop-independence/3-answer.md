...
창 강제 파괴(destroy)나 프로세스 종료는 그 절차 없이 웹뷰·프로세스를 끝낼 수 있어 cleanup 실행이 보장되지 않는다 — 그 결과 보조 창을 닫아도 세션 프로세스·폴링 스레드가 남고, 디바운스 대기 중이던 자동 저장이 유실됐다.\
...
`process::exit`는 이벤트 루프를 거치지 않고 OS 수준에서 프로세스를 끝낸다 — 다른 스레드와 어느 스택의 소멸자도 실행하지 않는다(C 런타임의 종료 처리 정도만 거친다). 그래서 백스톱을 **독립 스레드**에서 걸고, 최종 수단을 OS 종료로 했다(소멸자가 안 도는 것은 의도적으로 수용, 그 전에 자식 프로세스는 회수).
...
win.onCloseRequested(async (e) => {
  ...
  try {
    ...
    await Promise.allSettled(ids.map(closeSession));   // best-effort — 하나가 실패해도 나머지 정리를 끝까지 기다린다
  } finally {
    win.destroy();                                      // 예외여도 파괴
  }
});
