...
2. 방문 순서가 readdir 순서를 따르는 깊이 우선 탐색이기 때문이다. 예산이 어디서 떨어지는지는 트리 모양과 파일시스템이 돌려주는 순서에 달려 있고, 그 순서는 시스템 상수가 아니다 — readdir 순서는 POSIX가 보장하지 않고 파일시스템마다 다르며, 디렉토리 내용이 바뀌면(홈 디렉토리의 캐시처럼 스캔 사이에도 생성·삭제되는 항목) 순서와 방문 수가 함께 달라진다.\
...
6. 상한을 올리면 지금 보이는 누락은 사라지지만, 버킷이 100개를 넘는 날 같은 일이 다시 조용히 일어난다. 문제를 뒤로 미룬 것이다.\
...
앞 단계의 최대 출력(청크 ≤ 8KB — UTF-8에서 문자는 1바이트 이상이므로 최대 8192자)보다 뒤 단계 입력 상한(9000자)을 크게 잡으면, 정상 입력에서는 절단이 **구조적으로 불가능**해진다. 단 이 부등식은 두 상한을 **같은 단위로 환산했을 때만** 성립한다 — 바이트·문자·토큰이 섞이면(예: 모델의 실제 한도가 토큰 수) 문자 상한을 맞춰도 절단이 남을 수 있다. 경고는 부등식이 깨졌을 때를 위한 안전벨트로 남긴다.
...
① 문제 코드
```rust
fn scan_roots(start: &Path) -> (Vec<Root>, bool /* at_cap */) {
    let mut roots = Vec::new();
    let (mut visited, mut at_cap) = (0, false);
    ...
    (roots, at_cap)                                  // 예산·깊이 소진의 "포기"와 "완주"는 같은 (Vec, false)
}
...
return union(chunks);   // 키워드가 OR 결합일 때만 원 질의와 동치 — AND·점수 결합이면 합집합으로 대체 불가
```
...
for rec in records {
    if remaining == 0 { budget_exhausted = true; break; }   // 총량 소진도 결과에 표식으로 남긴다
...
if not_found { eprintln!("... not detected within budget (skipped={skipped}, exhausted={budget_exhausted})"); }   // 미검출 로그
```
...
| 3. 상한 부등식 정렬 | 앞 단계 최대 출력이 알려져 있고 같은 단위로 환산 가능하다 | 뒤 단계 상한 상향(자원) | 앞 단계 상한이 바뀌면 부등식이 깨짐 | 여러 단계가 각자 상한을 가진 파이프라인 |
...
단계 간 상한이 서로 알려져 있고 같은 단위(바이트·문자·토큰)로 비교할 수 있으면 부등식을 맞춰 절단이 아예 일어나지 않게 하는 것이 가장 확실하다(3).
