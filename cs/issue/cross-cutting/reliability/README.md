...
- [resource-bounding-last-defense](resource-bounding-last-defense/) — 클라이언트 타임아웃·취소는 서버측 작업을 멈추지 못하므로, 외부 입력에 비례해 커지는 자원(메모리·큐·연결·시간)은 서버 쪽 상한이 최후 방어선이다.
...
- [sibling-path-invariant-drift](sibling-path-invariant-drift/) — 같은 불변식을 지켜야 하는 형제 경로(분기·setter·오버로드·모듈 N벌 복사본·포팅 원본) 중 하나만 가드가 빠지는 비대칭이 결함이 된다 — 형제 전수를 대조한다.
