...
3. 공개 환경변수는 빌드 도구가 **빌드 시점에 번들 안의 문자열로 치환**한다. 실행 중에는 그 변수를 참조하는 코드가 이미 사라지고 리터럴만 남아 있어서, 런타임 env를 바꾸는 것만으로는 되살릴 수 없다(재빌드하거나, 처음부터 서버 쪽에서 런타임 값을 읽어 페이지에 주입하도록 설계해야 한다).\
로컬은 `.env.local`이 있는 곳에서 빌드했고, 도커 빌드는 `.dockerignore`가 그 파일을 빼서 빌드 컨텍스트에 값이 없었다. 테스트도 값이 있는 환경에서 돌아 그린이었다(그린 위장).

4. (a) 모듈이 import될 때 한 번 평가 (b) 컨테이너 생성 시점의 inode·소스 경로 (c) 부모 프로세스가 기동할 때 적재한 환경 사본 (d) 이미지 빌드 시점의 레이어 (e) 작업(스레드·세션)이 만들어질 때 (f) 첫 produce 때 브로커 기본값으로(자동 생성이 켜져 있을 때) — 이후 앱의 선언적 설정은 이미 있는 토픽을 대개 바꾸지 않는다(관리 클라이언트에 따라 파티션 **증가**만 시도하는 경우도 있지만, 증가는 키→파티션 매핑을 바꾸고 감소는 불가하다).\
...
tr '\0' '\n' < /proc/$(pidof -s deploy-agent)/environ | grep MODE    # 실제 적재값 확인(exec 시점 env — 이후 setenv는 반영 안 될 수 있음)
redeploy app
...
```
```yaml
# 첫 produce 때 브로커가 기본 파티션 수(num.partitions, 기본 1)로 자동 생성 → 앱의 파티션 3 선언은 무시됨
app:
  depends_on:
    broker: { condition: service_healthy }   # 기동 순서 보장
# 단 컨테이너 기동 순서는 "선언적 토픽 생성이 첫 produce보다 먼저"를 보장하지 않는다(다른 생산자·앱 내부 순서)
# → 확실한 쪽은 토픽을 --partitions 3 으로 사전 생성하거나 브로커 자동 생성(auto.create.topics.enable)을 끄는 것
```
