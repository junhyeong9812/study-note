...
3. **인스턴스마다 다른 세계.** 레이트리미터 카운트는 인스턴스별이라 요청이 분산되면 실효 한도가 **최대 N배**(인스턴스 수)까지 늘어난다. "동시 1작업" 락도 프로세스 로컬이라 다른 인스턴스에서 **동시에 실행**된다. 한 인스턴스가 발급해 등록한 토큰은 다른 인스턴스 레지스트리에 없어 **401**이 난다.
...
5. **힙 복제 vs page cache.** 워커가 각자 dict로 사전을 로드하면 사본이 워커 수만큼 생긴다(워커 4개면 4배로 추정했다). 파일 기반 저장소는 OS가 파일 페이지를 **page cache에 한 번만** 올리고 모든 프로세스가 그 물리 페이지를 공유하므로, 워커는 connection만 따로 가진다(connection마다 SQLite 자체 페이지 캐시가 별도로 있지만 작고 상한을 둘 수 있다).\
...
String issue(User u) {                                         // 매 발송 새 토큰(재사용 안 함)
    String raw = randomToken();
    repo.save(new TokenRow(sha256(raw), u, now().plus(ttl)));  // 저장은 해시만
    return raw;                                                // 원문은 발송에만 쓰고 버림
}
@Scheduled(1h) void purgeExpired() { repo.deleteByExpiryBefore(now()); }   // 1시간 주기 만료 정리
// 통합 테스트: em.flush(); em.clear(); 후 검증 = 영속성 컨텍스트 캐시를 비워 "DB만으로" 검증 (재기동의 부분 흉내 — 다른 인메모리 빈 상태까지 지우지는 않는다)
```
