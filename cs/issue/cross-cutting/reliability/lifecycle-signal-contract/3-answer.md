...
# depends_on 은 같은 compose 파일 안 서비스끼리만 → 한 파일로 묶음
```
...
command: --log-bin-trust-function-creators=1   # 부수: 비결정 함수 생성 거부로 import 가 중간 중단되던 것
```
...
**결론**: 신호를 내는 쪽을 소유하면 **명시 계약**이 유일하게 정확한 방법이다(기본).\
