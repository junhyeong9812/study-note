...
1. 클라이언트 타임아웃은 **연결(connection)** 을 끊을 뿐, 서버가 이미 시작한 **작업(work)** 을 멈추지 않는다. 서버는 요청을 받으면 그 처리를 자기 자원(CPU·GPU·메모리) 위에서 진행하는데, 클라이언트가 사라져도 서버가 연결 끊김을 감지해 취소를 전파하도록 구현돼 있지 않으면 그 작업은 별개의 실행 흐름이라 계속 돈다(일부 서버·런타임은 끊김 시 취소하지만, 그것을 방어선으로 가정할 수는 없다). 실제로 wrapper가 5초에 연결을 끊어도 모델 서빙 런타임은 서버측 생성을 이어가 3만 토큰 넘게 뽑았다. 즉 "요청을 취소했다"는 클라이언트의 관점일 뿐, 서버 자원은 여전히 물려 있다.
...
3. **큐잉**은 초과 요청을 대기줄에 쌓아 나중에 처리하고, **fast-fail**은 즉시 거절한다. 자원이 1개(GPU 1장)인데 큐를 두면 두 문제가 생긴다: (a) 하류(모델 서빙 런타임)가 이미 요청을 하나씩 직렬화하는데 래퍼가 또 큐를 두면 같은 일을 두 겹으로 하고, (b) 줄이 길어지면 뒤에 온 사용자 요청이 앞선 무거운 작업의 대기줄에 **인질로** 잡힌다. "넘치면 즉시 503/409"는 자원을 지키고(대기 자체를 안 만듦) 실패를 시끄럽게 만들어(호출자가 바로 재시도·백오프 판단) 이 조건(짧은 대화형 요청, 호출자가 재시도 가능)에서는 더 안전하다. 요청을 잃으면 안 되거나 호출자가 재시도할 수 없는 작업이라면 상한 있는(bounded) 큐가 맞을 수 있다(방안 비교 2).
   > **fast-fail(빠른 실패)** — 수용 못 할 요청을 대기시키지 않고 즉시 거절 응답을 돌려주는 것. 대기줄이 자원을 잠식하는 것을 막는다.

4. 인증 안 된 아무나 **거대한 본문**을 보내 서버 메모리를 압박(자원 고갈 시도)할 수 있다 — 비밀 검증이 본문을 다 읽은 *뒤에* 일어나므로, 검증에 실패할 요청조차 메모리를 이미 물어버린다. 막으려면 상한 검사를 **본문을 읽기 전**에 놓는다: 헤더에 선언된 `content-length`를 먼저 보고 상한(예: 4KB) 초과면 읽지도 않고 413으로 거절, 다만 content-length가 없는(chunked) 요청은 선언값 검사를 그냥 통과하므로, 실제 읽기도 바이트를 세며 상한에서 중단하거나 런타임·프록시의 본문 상한을 함께 둬야 한다(읽은 뒤 재확인만으로는 이미 적재된 뒤다). 검사를 인증·적재보다 앞으로 당기는 것이 핵심이다.
...
6. `complete()`를 빠뜨리면 **열린 연결(비동기 요청 컨텍스트)과, 전송 작업이 붙들고 있다면 그 스레드**가 반납되지 않는다 — 응답이 끝났다고 서버에 알리지 못해 비동기 타임아웃이 올 때까지(타임아웃을 무한으로 두었다면 무기한) 남는다. 스트리밍은 "응답을 여러 조각으로 나눠 흘리는" 열린 자원이므로, 정상 종료(`complete()`)든 오류 종료(`completeWithError`)든 **반드시 닫아야** 그 자원이 회수된다. "열었으면 닫는다"는 파일·소켓·락과 같은 계열의 자원 경계다.
...
export async function handler(req) {
  ...
  const body = await req.text();          // 한계: content-length 없는(chunked) 요청은 declared=0 으로 통과해 여기서 전량 적재된다
  if (new TextEncoder().encode(body).length > MAX_BODY_BYTES) return new Response(null, { status: 413 }); // 사후 재확인 (문자 수가 아니라 바이트 수)
  // 완전한 방어: 본문 스트림을 바이트 단위로 세며 읽다 상한에서 중단, 또는 런타임·프록시 본문 상한 병행
  if (!verify(req.headers, body)) return new Response(null, { status: 401 });
  // ...
}
```
...
Files.move(part, target, ATOMIC_MOVE, REPLACE_EXISTING);     // 같은 디렉토리에서 원자 교체 (ATOMIC_MOVE 시 다른 옵션은 무시 — 기존 대상 교체 여부는 구현 의존, POSIX rename 은 교체)
...
① 문제 코드
```java
RestClient client = builder.build();                         // 명시 설정이 없으면 연결·읽기 타임아웃이 무한인 경우가 많다 (요청 팩토리·버전에 따라 다름)
...
Process p = new ProcessBuilder(cmd).redirectErrorStream(true).start();   // stderr 도 합쳐서 (따로 두면 stderr 도 소비해야 함)
...
f.seek(SeekFrom::Start(len.saturating_sub(STORE_CAP)))?;    // 꼬리만 읽는다 (파일이 작으면 처음부터 — End(-N) 은 파일보다 길면 음수 위치 오류)
...
async with asyncio.timeout(5):                               # Python 3.11+ (이전 버전은 asyncio.wait_for 로 감싼다)
    raw = await chat(...)
...
진단: dmesg -T | grep -i "out of memory"              ← 호스트 커널이 죽였다면 흔적은 주로 여기에 남는다
...
def worker(batch):
    global _cache                                             # (dict 내용 변경만 하면 global 선언은 불필요 — 재할당할 때만 필요)
