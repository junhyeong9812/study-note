...
교정: 재구축 때만 증가하는 **세대 카운터**를 키에 더하거나, 원천 파일 서명 `(len, mtime_ns)`를 키로 쓴다(이것도 근사다 — 같은 길이·같은 타임스탬프 재작성은 놓칠 수 있어, 확실해야 하면 내용 해시).\
...
4. **mtime 보존 복원과 해상도.** mtime 비교형 증분 빌드(make, cargo 기본 동작)는 소스 mtime이 산출물·직전 빌드보다 새로울 때만 재컴파일한다(내용 해시 기반 빌드 도구는 이 함정이 없다).\
...
단 ns 필드여도 실제 해상도는 파일시스템·커널 타임스탬프 갱신 주기에 묶여 더 거칠 수 있고, mtime 기반 키는 위의 `cp -p` 같은 mtime 보존 변경을 원리상 못 잡는다 — 확실성이 필요하면 길이·내용 해시를 더한다.
...
head는 접두 비교라 정상 append를 오탐하지 않는다. 잔여 한계: truncate 후 재성장 + 선두 256B 동일은 못 잡는다(명시). 삭제된 파일의 inode 번호가 새 파일에 재사용되면 dev/ino 비교도 교체를 놓칠 수 있다(그때는 길이·head 검사가 남은 방어선).
...
let model_fp: u64 = t.model().map(|m| fnv(m.as_bytes())).unwrap_or(0);   // 바이트 합은 순서만 다른 이름끼리 충돌 — 해시로
let fp = (items.len(), rev_sum, turns.len(), answer_len_sum, dates.len(), sub_count,
...
② 고친 코드
```rust
for f in walk(root).filter(|p| p.extension() == Some(OsStr::new("jsonl"))) {   // Path::ends_with는 경로 성분 비교라 확장자 검사가 아님
