...
실제로 와일드카드 감지를 한 국가 모듈에만 넣어 다른 4개국은 와일드카드가 문법 파서를 우회했고, 다른 개선은 반대로 사본 3곳에만 들어가고 원본에는 빠졌다.\
...
7. **형제 전수 대조.** 결함 하나를 찾으면 "같은 불변식을 지켜야 하는 다른 경로"를 전부 나열하고(다른 setter, 다른 분기, 다른 서브클래스, 다른 국가 사본, 포팅 원본, 반대편 직렬화) 하나씩 같은 가드가 있는지 확인한다 — 비교표로 남긴다.\
...
def contains_grammar(value):
    ...
    # ...
# region_b/parser.py, region_c/parser.py, ...
def contains_grammar(value):
...
class BaseQueryParser(ABC):                             # 공통 베이스로 승격 (장기 계획)
    def contains_grammar(self, value):
        ...
        # ...
class RegionAParser(BaseQueryParser): config = A_CONFIG   # 사본마다 다른 것은 설정 1줄뿐
# 즉시 조치: 누락 사본에 같은 패치 + 사본 전수 비교표로 확인, 가드는 공통 validator 로 이동
```
무엇이 깨졌나: 같은 로직의 N벌 사본 중 일부만 고쳐져, 나머지가 조용히 옛 동작을 유지했다.\
같은 구조: OR 처리 개선이 한 사본에만 적용·미전파, 발음 변환 개선 룰이 사본 3곳에만 있고 원본에 누락, 위험 패턴 가드가 국가·경로별로 비대칭, 파서 파사드가 설정 1줄 차이로 5벌 복붙.
...
같은 구조:\
- 적재 경로를 새로 옮기면서 옛 경로에만 있던 전처리(지역별 구분자 분할)가 빠짐 → 구분자를 파라미터로 주입하는 공통 함수로 통합.\
