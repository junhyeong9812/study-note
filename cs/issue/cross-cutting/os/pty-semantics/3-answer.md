...
사람이 Enter를 누르면 터미널은 **CR(0x0D)**을 보낸다(cooked 모드라면 라인 디시플린이 ICRNL로 LF로 바꿔 주지만 raw 모드는 바꾸지 않는다) — 그래서 TUI의 "제출" 키 바인딩은 대개 CR에 걸려 있다.\
...
교정은 본문을 paste로 보내고, **별도 write로 지연(실측 하한의 약 2.5배인 300ms) 후 CR**을 보내는 것이다 — 다만 지연은 합쳐질 확률을 낮출 뿐 보장이 아니다(수신 쪽이 바쁘거나 늦게 read하면 두 write가 여전히 한 read로 합쳐질 수 있다).\
...
3. **준비 전 쓰기.** 이 사례에서는 TUI가 입력 핸들러를 설치하기 전에 도착한 바이트가 **소실됐다**. 일반적으로 쓴 바이트는 커널 입력 큐에 남아 나중에 읽힐 수도 있지만, raw 모드로 전환하며 대기 입력을 비우거나(`TCSAFLUSH` 류) 초기화 중 입력을 무시하는 프로그램에서는 사라진다 — 어느 쪽인지는 수신 프로그램에 달렸다.\
...
5. **canonical 모드의 무음 절단.** 리눅스 canonical(line) 모드 라인 디시플린은 한 줄 입력 버퍼를 **4096바이트**(N_TTY 버퍼 크기 — POSIX 상수 `MAX_CANON`의 리눅스 헤더 값 255와는 다르다)까지만 유지한다.\
개행 없이 그보다 긴 바이트가 오면 커널이 **받아들인 뒤 버린다** — 쓰는 쪽의 `write`는 `Ok`(200KiB, 9.7ms)를 돌려주지만 상대 수신은 0이거나 4095바이트로 잘린다.\
이 설계에서 쓰는 쪽(큐에 넣는 호출자)은 상대 터미널 모드를 볼 수 없어 이것을 막을 수 없다(리눅스에서는 master fd로 `tcgetattr`해 ICANON 여부를 조회하는 방법이 있지만, 모드는 쓰는 도중에도 바뀔 수 있다).\
그래서 계약을 "전달했다"에서 **"큐에 넣었다 — 이 크기는 아직 먹힐 수 있다"**로 낮추고, 위험 크기(≥ 라인 버퍼 상한, 개행 없음)면 연결당 1회 경고로 **명명**했다.

6. **EIO = EOF.** Linux PTY는 slave 쪽이 모두 닫히면(자식 종료) master `read`가 빈 결과(EOF) 대신 **EIO 오류**를 돌려준다(다른 OS는 EOF를 돌려주기도 하므로 둘 다 처리한다).\
EOF만 기다리는 코드는 종료를 감지하지 못하고 세션을 살아 있는 것으로 둔다(무음 실패).\
EIO만 EOF로 **정규화**하되, 논블로킹의 "데이터 없음"(EAGAIN)·신호 중단(EINTR)은 따로 분기해 종료로 오인하지 않고, 그 밖의 오류(EBADF 등)는 종료로 삼키지 말고 오류로 올린다.\
정규화 후 종료 통지를 내보내고 세션을 dead로 표시한다.

7. **제어 터미널.** 커널은 터미널 신호(SIGWINCH·SIGINT 등)를 **제어 터미널을 가진 세션의 포그라운드 프로세스 그룹**에 보낸다.\
`setsid`(또는 새 세션 옵션)는 새 세션을 만들 뿐 제어 터미널을 붙이지 않는다 — 이미 열어 둔 slave fd를 물려받는 것만으로는 획득되지 않는다(리눅스에서는 제어 터미널 없는 세션 리더가 `O_NOCTTY` 없이 tty를 새로 open하면 자동 획득되기도 하지만, 명시적 `TIOCSCTTY`가 확실하다).\
...
fn send(&self, data: &[u8]) -> io::Result<bool> {   // 계약: "큐에 넣음"
    ...
    Ok(data.len() >= LINE_BUF_LIMIT && !data.contains(&b'\n'))  // 4096(리눅스 N_TTY) — 위험 크기 → 호출자가 1회 경고
}
```
...
try:
    ...
    if e.errno != errno.EIO: raise   # EIO만 종료 — 다른 오류는 삼키지 않음
    data = b""            # slave 측 닫힘
if data == b"":
