...
GUI 런처로 실행한 프로그램은 데스크톱 세션의 **최소 환경**을 받아 rc가 적용되지 않고 `TERM`이 없다.\
서비스 매니저 유닛은 **유닛에 적은 것만** 받는다(사용자 매니저 PATH ≈ `/usr/local/bin:/usr/bin:/bin`).\
...
7. **플랫폼 주입과 암묵 상속.** 오케스트레이터는 같은 네임스페이스의 서비스마다 `<NAME>_PORT=tcp://…` 형식 변수를 파드에 **자동 주입**한다 — 서비스 이름이 앱 설정 변수 접두사와 같으면 설정값이 URL 문자열로 덮여 숫자 파싱이 실패했다(서비스 이름 변경 + 설정 명시로 해결).\
어떤 언어의 프로세스 API는 env 목록이 **nil이면 부모 환경 전체를 상속**하므로, 주입값이 하나도 없는 순간 격리가 무음으로 풀린다. 컴포즈 도구는 작업 디렉토리의 `.env`를 자동으로 읽는다.\
...
같은 구조: 비대화 셸에서만 홈 CLI 실패(원인 규명만, 조치 미기록) · 유닛 템플릿 경로와 실제 설치 경로 불일치 → 치환 후 검증 · 검증 명령을 설치 전에 돌려 항상 실패 → 설치 후로 이동 · 원격 대상에 빌드 도구 없음 → 로컬 빌드 산출물 전송 모드 · 사용자 서비스는 로그아웃은 기본 설정으로 버티나 재부팅 생존엔 linger 필요.
...
cmd.Env = injected                    // injected가 비면 nil → 부모 env 전체 상속, .env 자동 로드
```
② 고친 코드
```yaml
