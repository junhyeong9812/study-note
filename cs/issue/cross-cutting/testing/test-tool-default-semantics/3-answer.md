...
1. **비선점 타임아웃.** JUnit 5 `@Timeout`의 기본 스레드 모드(SAME_THREAD)는 테스트를 같은 스레드에서 실행하고 **끝난 뒤** 걸린 시간을 비교하는 방식이다.\
그래서 끝나지 않는 코드는 실패가 아니라 **무한 대기**가 된다 — 20초 제한인 테스트가 10분 넘게 돌았다.\
어노테이션이 없는 테스트엔 제한이 아예 없다.\
`junit.jupiter.execution.timeout.thread.mode.default=SEPARATE_THREAD`로 선점형(별도 스레드에서 실행하고 시간이 되면 실패)으로 바꾸고, `timeout.testable.method.default`로 전역 기본 제한을 둔다(개별 `@Timeout`이 우선).\
...
호출을 `_run()` 같은 헬퍼로 감싸면 실제 조회는 헬퍼 모듈에서 일어나므로, 원래 함수(`create_subprocess_exec`)에 건 mock은 빗나간다 → mock 대상을 헬퍼로 옮긴다(빗나간 메커니즘 설명은 일반론 — 원 기록은 "헬퍼 도입 후 mock이 안 맞음 → 대상을 헬퍼로 변경"까지).\
...
그런데 팩토리가 참조하는 `const push = vi.fn()`은 원래 자리에 남아 있어, 팩토리가 실행될 때는 아직 초기화 전(TDZ)이다.\
...
6. **Mockito 스텁 상태 머신.** 같은 호출을 두 번 스텁하면 **마지막 것만** 유효하고 경고는 없다 — 복붙한 중복 스텁이 의도와 다른 값을 돌려도 테스트는 통과했다.\
...
`body(Object)`와 `body(T)`처럼 오버로드된 제네릭 메서드는 매처의 인자 타입만으로 어느 오버로드인지 특정할 수 없어 deep stub·`any()`가 매칭에 실패한다 → `any(byte[].class)`처럼 타입을 명시하거나, fluent HTTP 클라이언트는 mock 대신 **서버 목**(요청 기대·응답 지정·verify)을 쓴다.

7. **기본값은 부분 재현이다.** `dictConfig`의 `disable_existing_loggers: True`는 이미 만들어진 로거를 비활성화하고 핸들러를 교체해, pytest가 붙인 캡처 핸들러까지 무력화한다 → 로거 호출 자체를 patch해 검증하거나 설정 테스트는 핸들러 존재·레코드 속성으로 본다.\
...
```ts
const push = vi.fn();
vi.mock("router-lib", () => ({ useRouter: () => ({ push }) }));   // 최상단으로 끌어올려짐 → TDZ
```
② 고친 코드
```ts
const { push } = vi.hoisted(() => ({ push: vi.fn() }));          // 변수 생성도 함께 끌어올림
vi.mock("router-lib", () => ({ useRouter: () => ({ push }) }));
```
...
given(client.post().body(any()).retrieve()...);  // body(Object) vs body(T) 오버로드 구분 불가
```
② 고친 코드
```java
...
assert "access" in caplog.text                  # 캡처 핸들러가 무력화돼 항상 실패
```
```java
