...
1. **동작을 구현하지 않는 더블.** jsdom은 Tab 키의 **기본 포커스 이동을 구현하지 않는다**. 그래서 Tab/Shift+Tab 이벤트를 쏴도 포커스는 원래 자리에 그대로 있고, (시작 포커스가 모달 안에 있었다면) "포커스가 모달 안에 남았나"를 단언하면 트랩 코드가 맞든 틀리든 **항상 참**이 된다 — 결과가 결함 유무와 무관하다.\
...
3. **빌드 스테이지 ≠ 런타임 스테이지.** 빌드 스테이지에는 소스 전체가 있으므로 설정 파일의 `import`가 풀리고 빌드는 통과한다.\
   ...
   진단 요령: 404 응답에 앱 프레임워크 헤더가 있으면 요청이 앱까지 도달했다는 뜻이므로, 프록시 라우팅보다 앱 쪽(설정 로드·빌드 산출물)을 먼저 의심한다(헤더를 덧붙이는 프록시 구성이면 예외). 교정은 설정 파일을 **자체완결**(앱 소스 import 없이 리터럴·환경변수)로 만들고 이미지를 재빌드하는 것.
...
4. **실행 모드 일반화 금지.** 안 된다. 실행 모드가 바뀌면 **출력 포맷·능력 보고·타이밍**이 달라질 수 있다.\
   ...
   이 사례의 헤드리스 브라우저는 포인터 hover 능력을 false로 보고했으므로(브라우저·버전·에뮬레이션 설정에 따라 다름) `@media (hover:hover)`로 감싸 생성된 hover 스타일이 적용되지 않는다 — hover UI를 헤드리스로 검증할 수 없다(같은 규칙쌍의 `focus-within`으로 대체 검증하고, 실브라우저 확인은 사람에게). 또 `visibility:hidden` 요소는 포커스를 받을 수 없다.

5. **도구 차이.** 서버 버그가 아니다. 브라우저(`encodeURIComponent`·`EventSource`)는 비ASCII를 퍼센트 인코딩해 보내지만, curl은 넣은 바이트를 그대로 보내고, 요청 대상에 허용되지 않은 문자를 엄격히 거부하는 서버(이 사례의 서버 포함)는 그런 URL을 400으로 거부한다(관대하게 받는 서버도 있다).\
...
6. **운영 컨텍스트의 일부만 올리는 도구들.**
   ...
   - `new ObjectMapper()`: 프레임워크가 자동 구성하며 등록하는 모듈(JSR310 등)이 없다 → (Jackson 2.x 기준) `Instant` 직렬화 실패. 테스트는 `findAndRegisterModules()` 또는 컨텍스트의 매퍼를 쓴다. 이 사례의 mock 응답 객체는 원시 `Set-Cookie` 헤더를 쿠키 모델로 해석하지 않았으므로(mock 구현·버전에 따라 해석하는 것도 있다) 헤더 문자열로 검증한다.
   - `@DataJpaTest` 슬라이스: `@Configuration`을 읽지 않아 필요한 설정을 `@Import`해야 하고, 전역 감사 설정이 없으면 `@CreatedDate`가 null, self-proxy 캐시는 mock으로 검증 불가, 이 사례처럼 테스트에서 마이그레이션을 끄면 시드는 실행되지 않는다(슬라이스 자체는 마이그레이션 자동구성을 포함할 수 있다).\
   공통점: **횡단 동작(프록시·감사·마이그레이션·advice)은 통합 테스트에서만 검증된다.**

7. **컴파일 OK ≠ 기동 OK.** Spring Data JPA는 **리포지토리 빈을 만들 때 `@Query` JPQL을 파싱·검증**한다. 호출 여부와 무관하게 빈 생성 단계에서 실패하므로(기본 bootstrap 모드 기준 — 지연 초기화 모드면 첫 사용 시점으로 밀린다) 오타 하나가 애플리케이션 기동 자체를 막는다. 컴파일러는 문자열 안의 JPQL을 보지 않는다.\
...
ObjectMapper om = new ObjectMapper();            // JSR310 미등록 → Instant 직렬화 예외 (Jackson 2.x)
response.getCookie("token");                     // 이 사례의 mock은 Set-Cookie 헤더를 파싱 안 함 → null
```
② 고친 코드
```java
...
```
```sh
curl "http://host/ask?q=한글"              # 비ASCII 원바이트 그대로 → (엄격한 서버에서) 400
```
② 고친 코드
```python
