...
1. **바인딩 주소.** 서버가 소켓을 `listen`할 때 어느 로컬 주소로 접속을 받을지 정한다. `0.0.0.0`은 그 호스트의 **모든 인터페이스**(루프백 + LAN + 공인 등), LAN IP는 **그 IP로 온 접속만**, `127.0.0.1`은 **자기 자신(루프백)만**. LAN IP에만 바인딩한 서버에 `127.0.0.1`로 접속하면, 커널에 그 주소로 리스닝 중인 소켓이 없으므로 **`connection refused`**가 난다 — 포트 번호는 같아도 "그 주소:포트"에 귀를 열지 않았기 때문이다. 패킷을 버리는(DROP) 방화벽의 무응답/timeout과 달리 refused는 "그 주소엔 아무도 안 듣는다"는 즉답이다(단 REJECT 규칙의 방화벽도 refused류 즉답을 낼 수 있어, refused만으로 원인을 100% 가르진 못한다).\
   (도커 포트 게시 `<IP>:8090:8090`도 같은 효과다 — 호스트의 그 IP로 온 트래픽만 컨테이너로 전달되고, 호스트 루프백에는 리스너가 없다.)
...
4. **localhost vs 127.0.0.1.** `localhost`는 이름이라 리졸버가 IP로 푼다. 컨테이너 안 `/etc/hosts` 설정에 따라 `localhost`가 **`::1`(IPv6 루프백)**로 먼저 풀릴 수 있는데, 서버가 IPv4(`127.0.0.1`)로만 바인딩했다면 `::1`로 온 접속은 받지 못한다 — 이때 **첫 주소만 시도하는 클라이언트**(예: busybox `wget`, 일부 런타임의 HTTP 클라이언트)는 IPv4로 재시도하지 않고 실패 → unhealthy. (curl처럼 해석된 주소를 차례로 시도하는 클라이언트는 `127.0.0.1`로 넘어가 우연히 통과할 수 있어, 도구에 따라 재현 여부가 갈린다.) `127.0.0.1`을 명시하면 이름 해석 단계를 건너뛰어 IPv4 루프백으로 확정되므로 이 불일치가 사라진다. 교훈: **헬스 프로브 주소는 이름이 아니라 서버가 실제 바인딩한 주소 계열로 못 박는다**.
...
6. **`compose up` 종료 0 ≠ 정상.** `docker compose up -d`의 종료코드 0은 "컨테이너를 **띄웠음**"만 뜻한다(헬스 상태까지 기다리게 하려면 `--wait` 같은 옵션을 명시해야 하고, 그것도 healthcheck가 정의된 서비스에만 의미가 있다) — 그 안 프로세스가 뜨자마자 크래시 루프거나, 떴어도 요청에 200을 못 주는 상태일 수 있다. 그래서 배포는 **헬스 URL을 실제로 폴링(200이 와야 ok, 최대 90초)**해 "떴다"와 "응답한다"를 갈라야 한다. 더 나아가 헬스체크 자체도 얕으면 안 된다 — 얕은 `/health`는 "프로세스 살아있나"만 보지만, 재부팅 후 컨테이너가 GPU를 잃고도 `loaded:true`를 계속 반환해 **6일 내내 healthy로 위장한** 실측 사고가 있었다. 그래서 **깊은 `/health/deep`(GPU로 실제 추론 1건)**로 "약속한 일을 실제로 하나"를 검사한다. "살아있음"과 "일함"은 다르다.
...
```yaml
healthcheck:
  test: ["CMD-SHELL", "wget -q -O /dev/null http://localhost:8090/health || exit 1"]   # busybox wget: localhost → ::1만 시도
```
② 고친 코드
```yaml
healthcheck:
  # localhost 금지 — 컨테이너 안에서 ::1로 풀려 IPv4 바인딩 서버가 unhealthy
  test: ["CMD-SHELL", "wget -q -T 5 -O /dev/null http://127.0.0.1:8090/health || exit 1"]
```
무엇이 깨졌나: 이름 해석 결과의 주소 계열(IPv6)이 서버의 바인딩 계열(IPv4)과 달랐고, 클라이언트가 다른 계열로 재시도하지 않았다.
...
```sh
docker compose up -d
for i in $(seq 1 90); do                             # 약 90회 폴링 (요청마다 -m으로 상한을 둬야 총 시간이 유한)
  [ "$(curl -s -m 2 -o /dev/null -w '%{http_code}' "$HEALTH_URL")" = 200 ] && { report "deploy ok"; exit 0; }
  sleep 1
done
report "deploy unhealthy"; exit 1                    # 실패도 종료코드로 전달
...
# 호스트 네이티브 DB: bind-address = 0.0.0.0 (또는 컨테이너가 도달하는 브리지/LAN 주소)
#   0.0.0.0이면 모든 인터페이스에 노출되므로 방화벽·계정 호스트 제한으로 범위를 다시 좁힌다
```
...
# 고친 (이미지별 내장 수단)
test: ["CMD-SHELL", "bash -c 'exec 3<>/dev/tcp/localhost/8090' || exit 1"]              # JRE(bash 있는 이미지 한정): TCP 연결성만
