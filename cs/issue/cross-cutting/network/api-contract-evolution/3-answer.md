...
2. **버전 에코의 위험.**\
   ...
   교정은 **지원 화이트리스트**(batch를 요구하는 버전은 제외)이고, 목록에 없는 요청엔 서버가 지원하는 버전으로 답한다 — 그러면 클라이언트가 초기화 단계에서 "버전 불일치"로 **명시적으로 실패**한다.\
...
4. **응답 형태 변경의 배포 순서.**\
   ...
   새 필드 추가는 하위호환 확장이다 — 구 소비자는 모르는 필드를 무시할 뿐 파싱이 깨지지 않는다(다만 의미는 어긋날 수 있다 — 이 사례에서 구 화면은 새 필드를 몰라 기존 필드의 특수값을 평범한 값으로 표시하게 되므로, 기록은 여기서도 동시 배포를 권장했다).
...
6. **외부 입력 필드 드리프트.**\
   `jq -r '.prompt'`처럼 한 필드명에 의존하면, 필드가 사라져도 에러가 나지 않고 **조용히 빈 값**(null)을 읽는다 — 이후 로직은 "빈 프롬프트"로 정상 진행한다.\
   소비자 방어: 폴백 후보를 나열(`.prompt // .user_message // empty`)하고, 그 필드를 **감시 항목**으로 등록해 런타임 버전이 바뀔 때마다 실측한다. 결정 출력 형식도 문서의 현행 형식과 대조해 둔다.\
   한계: 폴백은 이미 아는 이름만 막는다 — 전혀 새로운 이름으로 바뀌면 여전히 빈 값이므로, 빈 값일 때 경고하는 관측이 함께 있어야 한다.
...
```
```python
# 이 사건의 우회: 이미 받은 덤프를 스트리밍 파싱 (O(1) 메모리)
...
```sh
prompt=$(jq -r '.prompt' <<<"$input")                             # 문제: 필드 바뀌면 조용히 null
