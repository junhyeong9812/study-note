...
1. **스택 오버플로 → 전체 언마운트.**\
   ...
   이 예외가 렌더 도중 발생하면, UI 프레임워크(React 16+ 기준)는 에러 경계(error boundary)가 없는 한 상위 트리 전체를 언마운트한다 — 그래서 "한 목록이 깨짐"이 아니라 "창 전체가 빈 화면"이 된다.\
...
2. **visited vs 깊이 제한.**\
   ...
   visited 집합은 "같은 노드를 두 번 방문하지 않는다"는 사실만 쓰므로 정상 트리 데이터는 그대로 그리고 사이클만 유한하게 끝낸다(중복 렌더 key도 함께 막는다 — 대가로 여러 부모가 공유하는 노드는 한 번만 그려진다).\
...
5. **심링크와 종료 보장.**\
   ...
   링크 미추적만으로는 부족하다 — 순환·중복은 심링크 외 경로(bind mount 등, OS·파일시스템에 따라)로도 생길 수 있고, 순환이 없어도 규모를 모르는 트리는 실용 시간 안에 끝난다는 보장이 없다. 깊이·루트 개수 상한만 있으면 넓고 얕은 트리에서 깊이 한도까지 전수 방문해 지연된다. **방문 디렉터리 수 예산**까지 더하고, 예산 소진 시 재귀 복귀에서 조기 종료한다.\
   함정: 링크를 따라가는 조회(`fs::metadata(path)`·`Path::is_dir()`)가 아니라 **링크 자체의 타입**(`DirEntry::file_type()`·`symlink_metadata`)으로 판정해야 링크를 링크로 본다(Rust의 `DirEntry::metadata()`는 링크를 따라가지 않는다).
...
7. **자원 상한과의 관계.**\
   ...
   한 문장: **순회의 종료는 입력이 아니라 순회자가 보장한다 — 재방문 차단을 기본으로, 규모를 모르는 입력에는 폭·깊이 예산을 함께 둔다.**
...
fn scan(dir: &Path, depth: usize, roots: &mut Set, count: &mut usize, visited: &mut usize) {
    ...
    for entry in read_dir(dir).ok().into_iter().flatten().flatten() {   // 읽기 실패는 skip
        let Ok(ft) = entry.file_type() else { continue };   // 링크 자체의 타입 (fs::metadata(path)는 링크를 따라감)
...
탐색기를 우리가 소유하지 않고 설정으로도 링크 추적을 제어할 수 없다면, 입력 쪽에서 순환을 없애는 것이 현실적인 선택지다.
