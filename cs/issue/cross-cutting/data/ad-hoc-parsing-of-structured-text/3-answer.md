...
1. **정규식 주석 마스킹.** 정규식은 "지금 라인 주석 안인지, 문자열 안인지"를 모른다.\
   ...
   주석·문자열 경계는 현재 문맥이라는 **상태**에 의존한다. `/*`만 찾아 치환하는 단일 패턴은 그 앞의 문맥을 보지 않으므로, 패턴을 더 정교하게 해도 새 반례가 나온다 — 실제로 첫 수정(정규식 개선)이 post-fix 재점검에서 다시 회귀했다.\
   (정확히는: 중첩 없는 주석·문자열 자체는 정규 언어라, 입력을 왼쪽부터 "문자열 | 라인 주석 | 블록 주석 | 그 외" 토큰으로 **빠짐없이 순차 소비**하는 lexer라면 정규식으로도 만들 수 있다 — 그건 사실상 상태 기계 스캐너다. 반면 중첩 괄호처럼 깊이가 제한 없는 구조는 정규 언어가 아니어서 (재귀 확장 없는) 정규식으로는 원리적으로 불가능하다.)
...
for (let i = 0; i < src.length; ) {
  ...
  // ... line: EOL 에서 code, block: "*/" 에서 code(내용은 공백), str/chr: 백슬래시 이스케이프는 건너뛰고 닫는 따옴표 또는 EOL 에서 code
}
```
...
```python
# 문제
while ts.peek() != ")": ts.next()      # peek()가 EOF에서 EOF 토큰을 계속 돌려주는 토크나이저면 닫는 괄호 없을 때 무한 루프, 중첩 미지원
