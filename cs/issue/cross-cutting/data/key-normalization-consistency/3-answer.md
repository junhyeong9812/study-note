...
6. **유니코드의 두 경우.**\
   **위치 형태**(어말 `ς` U+03C2 vs 중간 `σ` U+03C3)는 같은 문자의 표기 변형이므로 **양쪽을 같은 규칙으로 정규화**한다 — 입력은 (이 구현의) 발음 구별 기호 제거 과정에서 `σ`로 바뀌고 사전 키는 `ς` 그대로라 룩업이 실패했다(표준 NFD 분해는 `ς`를 바꾸지 않는다 — 대소문자 폴딩은 `ς→σ`로 맞춘다) → `ς→σ` 한 줄로 누락 해소.\
...
7. **정규화의 경계.**\
   **하면 안 되는 곳**: URL 경로는 대소문자를 구분하므로(RFC 3986 기준 — 스킴·호스트는 비구분, 경로의 실제 처리는 서버마다 다를 수 있다) 수집용 인터셉터가 URI를 소문자화하면 camelCase 라우트가 다른 키가 된다 → context-path와 후행 슬래시만 제거하고 대소문자는 보존.\
...
ANNOT_PAREN_RE = re.compile(r"\s*\([A-Za-z ]+\)")
for k, v in rows:
    m[k] = v
    alt = ANNOT_PAREN_RE.sub("", k).strip()          # 괄호 앞 공백이 남아 새 불일치가 되지 않게
...
grep -rnP --include='*.md' '[\p{Cyrillic}\p{Greek}\p{Armenian}\p{Hebrew}\p{Arabic}]' docs/   # UTF-8 로캘 필요
```
