...
5. **중간 드롭 vs 앞 드롭.** 터미널 바이트 스트림은 이스케이프 시퀀스로 상태가 바뀌는 **상태 기계**다. 중간을 지우면 `ESC [ 3` 뒤가 사라지는 식으로 시퀀스가 잘려 **이후 전체 해석이 오염**된다(무음 손상).\
   앞에서부터 드롭하면 잘리는 것은 가장 오래된 부분이고, 선두의 잘린 시퀀스는 수신기가 흡수한다 — "스크롤백 보존"의 의미를 "최근 raw replay"로 명시적으로 좁혀 이를 계약으로 삼았다.\
...
if (dropped) {
  ...
  pending.length = 0;             // 매 회 pending 완전 비움 + 스냅샷 완전 대체
}
