...
[같은 코드, 같은 테스트 — 그런데 실행 방식만 다름]
  ...
  $ python -m pytest              $ pytest
   ...
   현재 폴더(CWD) 삽입               테스트 파일의 basedir 삽입
   (파이썬이 -m 실행 시 넣음)        (기본 import mode prepend)
        │                                │
...
**임시방편이던 빈 conftest.py.** 빈 `conftest.py`를 두면 pytest가 그것을 import하려고 그 폴더(basedir)를 경로에 넣어주는 부수효과(기본 prepend 모드)로 import가 우연히 풀렸다. 하지만 그건 "왜 통하는지"가 우연한 부수효과라 의도가 안 드러난다 → `pyproject`의 `pythonpath`로 **명시적으로** 못 박는 게 정식 해법. 정식화 후 빈 conftest는 삭제.
...
- `python -m pytest`는 CWD를, 그냥 `pytest`는 (기본 prepend 모드에서) 테스트 파일의 basedir를 경로에 넣는다 → 코드·테스트가 다른 폴더면 갈린다.
