...
1. Spring의 `StringHttpMessageConverter`는 문자열 본문을 바이트로 바꿀 때 문자셋을 정해야 하는데, Content-Type에 charset이 없고 JSON 계열(`application/json`·`+json`, 이 경우 UTF-8)도 아니면 그 기본값이 (역사적 HTTP 관례상) **ISO-8859-1**(라틴-1)이다(`application/x-ndjson`은 여기에 해당). 한글 "정"은 메모리에서 유니코드 문자지만 라틴-1에는 한글이 없으므로, JDK 인코더는 이런 매핑 불가 문자를 보통 `?`로 치환한다 — 에러 없는 **조용한 손실**이다. 반면 라틴-1에 있는 비ASCII 문자(예: 가운뎃점 `·` U+00B7)는 1바이트 `0xB7`로 인코딩되는데, 이 바이트는 UTF-8에서 연속 바이트 범위(0x80~0xBF)라 첫 바이트가 될 수 없으므로 수신 측 ES가 UTF-8로 읽으려다 `Invalid UTF-8 start byte 0xb7`로 거부한다. 즉 요청이 거부되든 조용히 `?`가 저장되든 원인은 같은 라틴-1 인코딩이다. "문자열 = UTF-8"이 착각인 이유: 문자열은 **추상적 문자의 열**이고, 그것을 어떤 바이트로 만들지는 인코딩이 결정한다. 인코딩을 지정하지 않으면 "정하지 않음"이 아니라 "프레임워크 기본값(여기선 라틴-1)"이 적용된다.
   > **문자 인코딩(character encoding)** — 추상적 문자를 구체적 바이트로 대응시키는 규칙(UTF-8, ISO-8859-1 등). 같은 문자열도 인코딩이 다르면 다른 바이트가 된다.

2. 근본 해법인 이유: 문자열을 넘기면 "문자열 → 바이트" 변환을 **프레임워크의 기본 문자셋**이 하게 되어 제어권을 잃는다. `body.toByteArray()`(코틀린 기본 UTF-8)로 **직접 인코딩한 바이트**를 넘기면, 변환이 이미 우리가 원하는 UTF-8로 끝난 상태라 프레임워크는 그 바이트를 그대로 전송한다 — 기본값이 개입할 여지가 없다. `charset=utf-8` 헤더만 붙이는 것과의 차이: 일반론으로 헤더는 **수신 측에게 "이 바이트를 UTF-8로 읽어라"** 라고 알리는 선언이고, 송신 인코딩과 일치한다는 보장은 송신 코드가 져야 한다. 송신이 라틴-1로 깨뜨렸으면 헤더가 UTF-8이어도 이미 깨진 바이트다. (단 Spring `StringHttpMessageConverter`는 요청의 Content-Type에 charset이 있으면 그 charset으로 인코딩하므로, 이 경로에선 헤더 명시만으로도 고쳐질 수 있다 — 다만 그건 컨버터 구현에 암묵적으로 기대는 것이고, 헤더를 빠뜨린 다른 호출 경로에선 재발한다.) 그래서 둘 다 두되 핵심은 **송신 인코딩을 바이트 수준에서 직접 고정**하는 것이다.
...
① 문제 코드
```java
try (Writer w = new FileWriter(file)) {           // charset 미지정 → JVM 기본(JDK 17 이하는 플랫폼·로케일 의존, JDK 18+는 UTF-8 기본)
...
if (end < text.length && Character.isHighSurrogate(text[end - 1])) end -= 1   // 반쪽이면 한 칸 당김 (size ≥ 2 전제 — size=1이면 end==start로 전진 불가)
...
run("git", "diff", "--name-status", "$prev..$head")    // core.quotepath=true(기본) → 비ASCII 경로를 "\352\267\270…"로
    .lines().map { File(repoDir, it.substringAfter('\t')) }   // 상태 열 뒤 경로 → 이스케이프된 경로라 No such file
```
② 고친 코드
```kotlin
run("git", "-c", "core.quotepath=off", "diff", "--name-status", "$prev..$head")
// ls-files · show 등 경로를 출력하는 모든 호출에 동일 옵션
// 더 견고하게는 -z(NUL 구분, 경로를 인용·이스케이프 없이 그대로 출력) — 탭·개행이 든 경로와 rename(경로 2개) 행도 안전
```
...
```sh
# 문제: DB CLI 클라이언트가 기본 문자셋(클라이언트 버전·OS 로케일에 따라 latin1 등)으로 세션을 열어 utf8mb4 데이터가 변환 손실 → 한글이 '?'
...
| 2. 클라이언트 접속 옵션 명시 | 변환을 클라이언트 세션이 수행한다 | 매 접속 옵션 | 옵션 없는 접속에서 재발(조회만이면 데이터는 멀쩡·보기만 깨짐, 그 세션으로 쓰면 데이터 손상) | 운영 조회·CLI 점검 |
