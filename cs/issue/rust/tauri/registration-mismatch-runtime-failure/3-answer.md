...
1. **무음 사망의 두 원인.** 정적 capability는 빌드 시 바이너리에 박히는 ACL이라, 선언되지 않은 plugin 커맨드는 사용자에게 묻는 절차 없이 **그냥 거부**된다(JS 쪽에서는 invoke Promise가 reject되어 await 지점에서 throw).\
...
예외가 사용자에게 표시되는 경로가 없었으므로 결과는 "에러"가 아니라 "기능이 없는 것처럼 보임"이었다.
   > **capability(정적 ACL)** — 어떤 창이 어떤 커맨드를 부를 수 있는지 빌드 타임에 고정한 허용 목록. 사용자에게 권한을 요청·승인받는 단계가 없다. (Tauri v2 기준 — 앱 코드가 런타임에 capability를 추가하는 API는 있으나, 이 사건처럼 파일로만 선언했다면 빌드된 목록이 전부다.)

2. **창 단위 권한.** capability 파일은 창 라벨 패턴(메인 창 / 팝아웃 창)별 화이트리스트다.\
메인 창 파일에만 알림 권한을 넣으면 메인 창의 호출만 허용되고, 팝아웃 창의 같은 호출은 거부된다(호출부가 reject를 삼키면 조용히).\
권한은 "앱"이 아니라 "이 capability가 적용되는 창(라벨 패턴)"에 붙는다 — `"windows": ["*"]`처럼 모든 창에 적용되게 선언할 수는 있지만, 창별 파일로 나눴다면 새 권한은 창 라벨별 파일 **둘 다**에 선언해야 한다.

3. **타입 키 불일치 예측.** 컴파일은 된다 — `State<T>`는 제네릭이라 어떤 `T`든 받는다.\
(`T: Send + Sync + 'static` 조건만 맞으면.) 단위 테스트도 통과한다(4번).\
실제 앱에서는 컨테이너가 `TypeId(Arc<T>)`를 찾는데 등록된 것은 `TypeId(T)`뿐이라 매 호출 `state not managed` 오류로 커맨드 본문이 **한 번도 실행되지 않는다**(Tauri v2 기준 — invoke가 오류로 reject된다).
...
7. **권한 요청 1회성.** 권한 요청 API는 플랫폼·브라우저에 따라 사용자 제스처 문맥 밖이나 중복 요청에서 실패·무시되거나 조용한 UI로 강등된다(일반론 — 환경마다 다름).\
여러 곳에서 동시에 요청하면 요청이 겹치고, 알림을 보낼 때마다(발화 경로에서) 재요청하면 제스처 문맥이 아니라서 실패를 되풀이하기 쉽다(한 번 거부되면 재요청해도 프롬프트가 다시 뜨지 않는 환경도 흔하다).\
...
{ "windows": ["main"], "permissions": ["core:event:default"] } // 창 열거 권한 없음
// 주의: 현행 Tauri v2의 core:default는 core:window:default(창 열거·가시성 조회 포함)를
//       포함한다 — 재현하려면 core:default 없이 개별 권한만 나열한 경우를 가정한다.
```
```ts
...
"permissions": ["core:event:default", "core:window:allow-get-all-windows",
