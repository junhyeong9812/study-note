...
1. **Error ≠ Exception.** `Throwable` 아래에 `Exception`(복구 가능한 예외)과 `Error`(JVM 수준 심각 오류)가 형제로 있다. `OutOfMemoryError`는 `Error`라 `catch (Exception)`을 그대로 빠져나가고, 러너를 직접 실행한 스레드(또는 `execute`로 제출한 풀 작업)라면 스레드는 잡히지 않은 채 종료된다(`submit`으로 제출했다면 스레드는 살아 있고 예외가 `Future`에 담겨, 아무도 `get()`하지 않으면 역시 조용히 묻힌다) — `markDone`도 `setFailed`도 실행되지 않아 상태가 `RUNNING`에 고착되고, 스레드 덤프에서는 그 스레드가 사라져 있다.\
   ...
   권고는 러너를 `catch (Throwable)`로 감싸 실패 상태로 전이 + heartbeat/오래된 잡 스윕이다. 다만 OOM 직후에는 catch 안의 상태 기록(할당·DB 호출) 자체가 다시 실패할 수 있고, 프로세스 kill·JVM 크래시는 어떤 catch로도 막을 수 없으므로 **외부 스윕**이 필요하다.
   > **Error** — `Throwable`의 하위로, 애플리케이션이 보통 잡지 않는 JVM 수준 오류(OOM·StackOverflow 등).

2. **static 초기화 순서.** `Base` 생성자가 넘겨받은 목록을 역참조한다면 `ExceptionInInitializerError`(원인 `NullPointerException`)가 난다(역참조하지 않고 보관만 하면 예외 없이 null을 품은 인스턴스가 남아 나중에 터진다). 이후 같은 클래스를 다시 쓰면 `NoClassDefFoundError`. 클래스 초기화 시 static 필드는 **소스에 선언된 순서대로** 초기화된다. `INSTANCE = new X()`가 먼저 실행되고, 그 생성자의 `super(RULES)` 시점에 `RULES`는 아직 초기화 전이라 null이다. 교정은 `RULES`를 `INSTANCE` 위로 옮기고 "규칙 먼저, 인스턴스 나중"을 관례로 고정하는 것.
...
4. **불변성과 소거.** 제네릭은 **불변**이다 — `List<Sub>`가 `List<Super>`라면 `List<Super>`로 다른 하위 타입을 넣어 원래 리스트를 오염시킬 수 있으므로 금지된다. 스트림 `map`의 결과 타입을 `.<Super>map(...)` 타입 위트니스로 지정하거나, 상위 타입 필드를 가진 wrapper로 자동 확장한다.\
   제네릭 타입 인자는 **런타임에 소거**된다 — `Paged.class`는 원시 타입 `Class<Paged>`일 뿐 원소 타입을 담을 수 없다. **타입 토큰**은 `new TypeRef<Paged<Item>>(){}`처럼 익명 서브클래스를 만들어, 그 클래스의 "상위 타입 선언"(클래스 메타데이터에 남음)에서 `Paged<Item>`을 리플렉션으로 읽어낸다.
   > **타입 소거(type erasure)** — 컴파일 후 제네릭 타입 인자를 지우고 원시 타입으로 바꾸는 Java의 구현 방식.

5. **타입 체커가 못 보는 자리.** `equals`의 파라미터는 `Object`라 무엇이든 받으므로 컴파일된다. `Optional`은 다른 `Optional`과만 같다고 판정하므로 `Optional<Node>.equals(node)`는 **항상 false**다. 교정은 id 기반 비교(`opt.map(p -> p.getId().equals(this.parent.getId())).orElse(false)` — 빈 `Optional`에 `get()`을 하지 않도록) — equals 미오버라이드 엔티티는 참조 비교라 ORM 프록시와도 틀린다.\
   위치 인자 언어에서 **타입이 같은** 인자들은 이름이 아니라 순서로만 구분된다. 순서가 바뀌어도 타입 검사는 통과한다 — 로직을 옮길 때는 옮긴 전/후를 필드 단위로 대조해야 하고, 컴파일·테스트 green은 필요조건일 뿐이다.

6. **오버로드 해석.** 가변 인자 0개 호출 `f(String.class)`는 두 varargs 오버로드 모두에 적용 가능하다. 가변 인자 원소 타입 `Class<?>`와 `TypeDesc`는 서로 하위 타입이 아니므로 어느 쪽도 다른 쪽보다 **더 구체적이지 않아** 컴파일러가 선택하지 못한다 → 배열 타입을 명시해 `f(String.class, new TypeDesc[0])`.\
   (대조: 둘째 오버로드가 `Type...`이었다면 `Class<?>`가 `Type`의 하위 타입이므로 `Class<?>...` 쪽이 더 구체적이라 모호하지 않다 — JDK 21 javac 실측.)\
...
7. **사람의 의미 ≠ 언어 정의.**
   ...
   - `\W`는 (기본 플래그에서) `[^A-Za-z0-9_]` — 밑줄은 "특수문자"에서 빠지고 공백·한글은 들어간다. 설명 문구와 실제 허용 집합이 두 방향으로 어긋난다.\
...
boolean same = parentOpt.map(p -> p.getId().equals(this.parent.getId())).orElse(false);   // id 비교 (빈 Optional 안전)
```
...
① 문제 코드
```java
f(String.class);                                   // f(Class<?>, Class<?>...) vs f(Class<?>, TypeDesc...) → 모호 (TypeDesc는 Class와 무관한 타입)
...
② 고친 코드
```java
f(String.class, new TypeDesc[0]);                  // 배열 타입 명시
