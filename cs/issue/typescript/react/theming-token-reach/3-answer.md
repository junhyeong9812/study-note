...
`stroke="var(--bg)"` 같은 **프레젠테이션 속성**은 `var()`를 받지 않을 수 있어(원 기록도 "미지원 경향"으로만 판단) 선 색이 테마를 따라간다고 기대할 수 없다.\
...
페이지 CSS 변수가 그 팝업까지 전달되지 않으므로, 스타일 손질로는 목록 팝업을 맞추는 데 한계가 있다.\
...
이 식은 `rgba(C, 0.15)`와 같은 픽셀이라 다크 모드 모습은 보존되고, 라이트에선 토큰을 따라간다(`in srgb`를 빼면 이 동치가 깨진다).
