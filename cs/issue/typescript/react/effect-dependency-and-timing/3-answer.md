...
3. effect로 동기화하면, 첫 커밋에는 **옛 값**이 쓰인다 — 보통은 그 상태가 페인트되어 같은 대상을 두 번 그리는 UI가 한 프레임 겹쳐 보인다(effect가 페인트 전에 flush되는 경우엔 안 보일 수도 있지만, 옛 값으로 커밋되는 것 자체는 같다).\
그 옛 값이 "마운트할지"를 결정한다면, 한 프레임 동안 마운트되면서 **부작용(프로세스 생성)** 이 이미 실행된다 — 화면 깜빡임이 아니라 되돌릴 수 없는 동작이다.\
"렌더 중 조정"은 이전 입력을 **state로** 기억해(`const [prev, setPrev] = useState(active)`) 입력 전환을 감지하고 **렌더 도중** 조건부 `setState`를 부른다. React는 그 렌더 결과(JSX)를 버리고 자식을 렌더하기 전에 즉시 다시 렌더하므로 옛 값이 화면·자식 마운트에 도달하지 않는다. 이전 값을 ref에 두고 렌더 중에 ref를 쓰는 방식은 StrictMode 이중 렌더·중단되는 동시성 렌더에서 어긋날 수 있어 React 문서는 state를 권한다. 더 좋은 것은 파생값을 아예 저장하지 않고 매 렌더 계산하는 것이다.
   > **렌더 중 조정(adjust state during render)** — 이전 입력을 state로 기억해, 입력이 바뀐 렌더에서 곧바로 파생 state를 고치는 패턴. 옛 값이 커밋되지 않는다(effect 방식보다 한 커밋 빠르다).
...
루프가 없는 조건: 콜백이 `scrollTop`만 바꾸고 관찰 대상의 **관찰 박스 크기(기본 content-box, 옵션으로 border-box)는 바꾸지 않는다**. 크기를 바꾸는 쪽으로 반응하면 관찰→변경→관찰의 루프가 된다. 관찰 대상 밖의 요소가 커지는 경우는 여전히 못 본다(범위 한계).

7. effect 본문에서 동기로 state를 바꾸면 **커밋 직후 추가 렌더**가 곧바로 일어나 렌더가 연쇄된다.\
동기 `setLoading(true)`는 사용자 액션의 **이벤트 핸들러**에 두고, 마운트 시의 로딩 표시는 초기 state(`useState(true)`)로 흡수한다. 마운트 effect에서는 await 이후에만 state를 바꾼다. `Promise.resolve().then(() => load())`로 미루는 것은 동기 연쇄(와 린트 경고)만 피할 뿐 `setLoading(true)`에 의한 렌더 한 번은 여전히 일어난다 — 렌더 자체를 없애는 해법은 아니다.
...
if (prevActive !== active) {                        // 렌더 중 조정(이전 값은 ref 아닌 state)
...
```
무엇이 깨졌나: effect는 커밋 뒤에 도는데, 그 커밋의 옛 값이 마운트 여부를 결정했다.\
...
② 고친 코드
```ts
const [loading, setLoading] = useState(true);            // 마운트 로딩은 초기값으로 — effect 에서 setLoading(true) 불필요
useEffect(() => { void Promise.resolve().then(() => load(1, "")); }, [load]);   // 동기 연쇄만 회피(렌더 1회는 남음)
