...
5. `click`은 `mousedown`과 `mouseup` 대상의 **가장 가까운 공통 조상**에서 발생한다(같은 버튼에서 눌렀다 떼야 그 버튼의 click이 된다 — 세부는 브라우저 구현 차이가 있다). mousedown 처리 중에 전체를 재생성하면 누른 요소는 이미 문서에서 분리되고, mouseup은 포인터 아래의 새 노드로 가므로 down·up 대상이 달라져 **그 버튼의 click이 발생하지 않는다**(공통 조상이 없으면 click 자체가 없고, 있더라도 버튼이 아닌 조상에서 발생한다).\
