...
2. 셀렉터 없는 `subscribe`는 **변경 범위와 무관하게 모든 set**에 호출된다. 150ms마다 갱신이 오고 그때마다 N개 세션을 전수 비교하므로, 갱신 하나가 세션 하나를 바꿔도 비용은 N이고 전체적으로 O(N²)에 가까운 스캔이 된다.\
...
3. 스토어의 `set`은 반환 객체를 현재 state에 **얕게 merge해 새 루트 객체**를 만든다. `{}`를 반환해도 결과는 내용이 같은 **새 참조**이고, 구독자는 `Object.is`로 비교하므로 "변경됨"으로 판단해 전 리스너가 깨어난다.\
...
```ts
setInterval(() => {
  for (const a of agents) a.parent ??= parentCache.get(a.id) ?? cacheParent(a, calls);   // 한 번만
