...
[이벤트 계층]
  ...
  포커스      소유자 1명 원칙: 자식 자동 포커스 금지 · 닫을 때 명시 이양 · 새 DOM은 커밋 뒤(rAF·effect) focus
...
- 조합 중에 value를 프로그램으로 덮으면(자르기·이전 값으로 되돌리기) 조합 버퍼가 깨진다 — 절삭은 **compositionend**에서.
...
- DnD는 drop 없이 끝날 수 있다 — 로컬 이벤트가 아니라 **창 단위 백스톱**으로 정리한다(dragend는 페이지 안에서 시작한 드래그에만 온다).
