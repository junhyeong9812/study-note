...
3. "같은 폴더 안이라도 절대경로만" 정책은 **임포트하는 쪽 파일의 이동이 그 파일의 임포트를 깨뜨리지 못하게** 만든다(임포트되는 쪽 파일이 이동하면 절대 경로도 바뀌어야 하지만, 그때는 모든 참조가 같은 고정 문자열이라 기계적 일괄 치환·grep 검증이 가능하다). 방어하는 미래 사고: 리팩토링으로 파일을 폴더 간 이동할 때(응집 재편, 도메인 분리 등) 상대 임포트가 조용히 깨져 빌드가 연쇄로 터지는 것. 절대경로만 쓰면 임포트하는 파일의 위치가 그 임포트의 정확성과 **독립**이 되어, 옮기는 작업이 그 파일 안의 임포트 수정을 유발하지 않는다.

4. `@` alias가 `tsconfig.json`의 `paths`에 있어도 vitest만 못 찾은 이유는, **vitest가 (기본 설정에서) tsconfig의 paths를 읽지 않고 자기 리졸버(vite)를 쓰기 때문**이다. `paths`는 Next(번들러)와 tsc(타입체커)가 읽는 설정이다. vitest는 vite 위에서 돌고, vite는 모듈을 자기 방식으로 해석하므로 `tsconfig.paths`에 뭐가 있든 모른다 — 같은 alias를 `vitest.config.ts`의 `resolve.alias`에 별도로 심어야 비로소 해석한다(또는 tsconfig paths를 읽어 주는 플러그인·옵션을 켠다 — 어느 쪽이든 "그 도구에게 따로 알려준다"는 점은 같다).
...
```ts
// 문제: 비리터럴 경로의 import() 는 TS가 대상 모듈을 특정할 수 없어 구체 타입이 없음(any/unknown) → 사용처와 타입 불일치
...
```js
// 문제: 변환된 ESM 에서 없는 이름을 named import → 빌드 성공(경고뿐), 값은 undefined
//       (번들러·변환 설정에 따라 다름 — 오류로 막는 번들러도 있고, 네이티브 ESM이면 링크 시점 SyntaxError)
