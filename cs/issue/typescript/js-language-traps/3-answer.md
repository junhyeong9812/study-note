...
3. **중복 키는 무음 덮어쓰기.** JS 런타임 에러는 나지 않는다 — ES2015 이후 객체 리터럴은 strict mode에서도 같은 키를 허용하고 **마지막 값**으로 덮는다(TS 타입 검사는 리터럴 키 중복을 오류로 잡지만, JS 파일·검사 제외 경로·계산된 키에서는 통과한다).\
...
5. **단일 스레드.** 페이지의 JS는 메인 스레드 하나에서 이벤트 루프로 돈다(Worker는 별도 스레드지만 DOM·렌더링은 메인 스레드 소관).\
`while (Date.now() < end) {}`는 그 스레드를 점유해 그 페이지의 렌더링·입력·타이머 콜백이 전부 멈춘다(브라우저 프로세스 전체가 아니라 해당 페이지·웹뷰).\
...
7. **정적 도구의 범위.** 중복 키는 `no-dupe-keys`(와 TS의 리터럴 중복 속성 오류)로, TDZ는 린트의 선언 전 사용 규칙(`no-use-before-define`)이나 TS로, 반환 promise의 await 누락은 린트의 `return-await` 류 규칙으로 잡을 수 있다.\
