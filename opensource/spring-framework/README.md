...
구성은 세 갈래다. `prs/`는 PR 하나당 주제 폴더 하나로, 표준 5종 문서(README 해설, tests 테스트 해설, structure 실구조, analysis 메서드 그래프·이름표 사전, gates 이해 게이트 기록)가 탭으로 배포된다. `concepts/`는 기여 중 배운 개념을 단일 문서 주제(폴더명 = md 파일명)로 담는다. `architecture/`는 프레임워크 동작을 호출 흐름 단위로 위에서 아래로 따라가는 지도다. 폴더 하나가 메서드 하나이고, 각 폴더의 README에 실제 코드와 그 흐름의 아스키 도식을 싣는다.

## 작성 규칙 (이 폴더의 계약)

- 배포 트리 규칙: 주제(리프) = 하위 폴더 없이 md만 있는 폴더. PR 주제 폴더 안에 하위 폴더를 만들지 않는다. 단일 문서 개념은 `<슬러그>/<슬러그>.md`.
- `architecture/`만 예외다. 호출 그래프를 폴더 트리로 표현하므로 모든 폴더에 `README.md`를 두고, 폴더명은 `<순번>_<클래스>.<메서드>`로 호출 순서를 나타낸다. 코드 발췌와 줄 번호는 각 흐름 루트 README에 적힌 기준 커밋에 고정한다.
