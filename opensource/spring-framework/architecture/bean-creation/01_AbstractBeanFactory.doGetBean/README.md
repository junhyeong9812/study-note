...
```text
 ...
 markBeanAsCreated 표시
      --> 이 빈이 생성 단계에 들어갔다는 표시. 캐시 대상이 아니면 병합 정의를 지워
          지금 다시 병합하게 한다 (ABF L1791)
      --> 이후 병합 정의 캐시 대상 판정, 타입 검사용 싱글톤 제거 판단에 쓰인다
      --> 정의 등록 자체는 계속 허용되며, 순회 안전을 위해 복사 후 교체 경로를 탄다
 ...
 args 를 준 getBean(name, args)
      --> 1차 싱글톤 캐시 조회만 건너뛴다 (ABF L258)
      --> 매번 새로 만들어지는 것은 프로토타입뿐이고, 이미 만들어진 싱글톤이면
          getSingleton 이 기존 인스턴스를 그대로 돌려준다
```
