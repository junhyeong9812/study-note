...
WebSocket 경로는 같은 `DispatcherHandler` 골격을 쓰지만 이 지도에서는 애노테이션 컨트롤러 경로만 따라간다. 같은 골격에 라우터 함수를 끼우는 경로는 [함수형 엔드포인트](../functional-endpoints/README.md) 흐름에 있다. 나가는 요청(`WebClient`)은 [HTTP 클라이언트](../http-client/README.md) 흐름에 있다.
